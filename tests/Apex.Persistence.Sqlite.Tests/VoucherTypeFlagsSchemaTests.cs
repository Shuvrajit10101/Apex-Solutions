...
public sealed class VoucherTypeFlagsSchemaTests
{
    ...
    public void A_migrated_v52_book_reads_both_flags_off()
    {
        ...
        try
        {
            ...
            using (var conn = Open(path)) { SchemaDowngrade.V60ToV59(conn); SchemaDowngrade.V59ToV58(conn); SchemaDowngrade.V58ToV57(conn); SchemaDowngrade.V57ToV56(conn); SchemaDowngrade.V56ToV55(conn); SchemaDowngrade.V55ToV54(conn); SchemaDowngrade.V54ToV53(conn); SchemaDowngrade.V53ToV52(conn); SqliteConnection.ClearPool(conn); }
