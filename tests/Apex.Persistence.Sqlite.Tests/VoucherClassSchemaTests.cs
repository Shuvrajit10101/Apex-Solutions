...
public sealed class VoucherClassSchemaTests
{
    ...
    public void Migration_v61_to_v62_matches_CreateV1()
    {
        ...
        try
        {
            ...
            using (var conn = Open(migratedPath)) { SchemaDowngrade.V65ToV64(conn); SchemaDowngrade.V64ToV63(conn); SchemaDowngrade.V63ToV62(conn); SchemaDowngrade.V62ToV61(conn); SqliteConnection.ClearPool(conn); }
    ...
    public void The_downgrade_drops_only_the_v62_objects_and_keeps_every_class_row()
    {
        ...
        try
        {
            ...
            using (var conn = Open(path)) { SchemaDowngrade.V65ToV64(conn); SchemaDowngrade.V64ToV63(conn); SchemaDowngrade.V63ToV62(conn); SchemaDowngrade.V62ToV61(conn); SqliteConnection.ClearPool(conn); }
