...
public sealed class InventoryCostingTrackingSchemaTests
{
    ...
    public void Migration_v57_to_v58_matches_CreateV1()
    {
        ...
        try
        {
            ...
            using (var conn = Open(migratedPath)) { SchemaDowngrade.V58ToV57(conn); SqliteConnection.ClearPool(conn); }
    ...
    public void A_migrated_v57_book_reads_every_feature_off_and_nothing_tracked()
    {
        ...
        try
        {
            ...
            using (var conn = Open(path)) { SchemaDowngrade.V58ToV57(conn); SqliteConnection.ClearPool(conn); }
