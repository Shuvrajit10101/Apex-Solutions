...
public sealed class GstRegistrationSchemaTests
{
    ...
    public void Migration_v60_to_v61_matches_CreateV1()
    {
        ...
        try
        {
            ...
            using (var conn = Open(migratedPath)) { SchemaDowngrade.V65ToV64(conn); SchemaDowngrade.V64ToV63(conn); SchemaDowngrade.V63ToV62(conn); SchemaDowngrade.V62ToV61(conn); SchemaDowngrade.V61ToV60(conn); SqliteConnection.ClearPool(conn); }
    ...
    public void A_migrated_v60_book_attributes_every_existing_voucher_to_the_companys_own_registration()
    {
        ...
        try
        {
            ...
            using (var conn = Open(path)) { SchemaDowngrade.V65ToV64(conn); SchemaDowngrade.V64ToV63(conn); SchemaDowngrade.V63ToV62(conn); SchemaDowngrade.V62ToV61(conn); SchemaDowngrade.V61ToV60(conn); SqliteConnection.ClearPool(conn); }
    ...
    public void Downgrading_to_v60_keeps_the_primary_keys_of_both_rebuilt_tables()
    {
        ...
        try
        {
            ...
            {
                SchemaDowngrade.V65ToV64(conn); SchemaDowngrade.V64ToV63(conn); SchemaDowngrade.V63ToV62(conn); SchemaDowngrade.V62ToV61(conn); SchemaDowngrade.V61ToV60(conn);
