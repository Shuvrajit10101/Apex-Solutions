...
public sealed class KarnatakaPtBackfillSchemaTests
{
        ...
        try
        {
            ...
            using (var conn = Open(path))
            {
                ...
                Exec(conn, $"""
                    ...
                    """);
                SchemaDowngrade.V58ToV57(conn); SchemaDowngrade.V57ToV56(conn); SchemaDowngrade.V56ToV55(conn); SchemaDowngrade.V55ToV54(conn);
    ...
    public void Downgrade_v55_to_v54_restores_the_v54_shape_and_keeps_the_correction()
    {
        ...
        try
        {
            ...
            using (var conn = Open(path)) { SchemaDowngrade.V58ToV57(conn); SchemaDowngrade.V57ToV56(conn); SchemaDowngrade.V56ToV55(conn); SqliteConnection.ClearPool(conn); }
