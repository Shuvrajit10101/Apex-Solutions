...
public sealed class InventoryVoucherRoundTripTests
{
    ...
    private static void DowngradeToV9(string dbPath)
    {
        ...
        SchemaDowngrade.V65ToV64(conn); SchemaDowngrade.V64ToV63(conn); SchemaDowngrade.V63ToV62(conn); SchemaDowngrade.V62ToV61(conn); SchemaDowngrade.V61ToV60(conn); SchemaDowngrade.V60ToV59(conn); SchemaDowngrade.V59ToV58(conn); SchemaDowngrade.V58ToV57(conn);
        ...
        // (the companies/ledgers rebuild below strips the v21 ALTER columns back to the v9 shape).
        ...
        // Drop the v22 reorder-definitions table + its indexes so the reopen's v21→v22 CREATE TABLE does not collide.
        ...
        // Drop the v24 Job Work tables + their indexes so the reopen's v23→v24 CREATE TABLE does not collide (the
        // companies/voucher_types rebuilds below strip the v24 ALTER columns back to the v9 shape).
