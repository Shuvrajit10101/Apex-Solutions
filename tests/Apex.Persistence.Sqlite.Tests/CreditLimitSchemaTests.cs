...
public sealed class CreditLimitSchemaTests
{
    ...
    public void A_migrated_v53_book_reads_no_credit_limit_and_both_flags_off()
    {
        ...
        try
        {
            ...
            using (var conn = Open(path)) { SchemaDowngrade.V64ToV63(conn); SchemaDowngrade.V63ToV62(conn); SchemaDowngrade.V62ToV61(conn); SchemaDowngrade.V61ToV60(conn); SchemaDowngrade.V60ToV59(conn); SchemaDowngrade.V59ToV58(conn); SchemaDowngrade.V58ToV57(conn); SchemaDowngrade.V57ToV56(conn); SchemaDowngrade.V56ToV55(conn); SchemaDowngrade.V55ToV54(conn); SchemaDowngrade.V54ToV53(conn); SqliteConnection.ClearPool(conn); }
