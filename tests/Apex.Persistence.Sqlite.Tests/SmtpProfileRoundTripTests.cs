...
public sealed class SmtpProfileRoundTripTests
{
    ...
    /// real targets, and a pre-v18 <c>voucher_types</c> (with the v10 effect flags, without the v18
    /// use_as_manufacturing_journal column) plus a pre-v18 <c>stock_items</c> (without set_components) so the
    /// v17→v18 ADD-COLUMN steps have real targets too. Kept in the test so it never drifts as the production
    /// schema advances.
    /// </summary>
