...
public sealed class MasterGapsSchemaTests
{
    ...
    public void Migration_v59_to_v60_matches_CreateV1()
    {
        ...
        try
        {
            ...
            using (var conn = Open(migratedPath)) { SchemaDowngrade.V64ToV63(conn); SchemaDowngrade.V63ToV62(conn); SchemaDowngrade.V62ToV61(conn); SchemaDowngrade.V61ToV60(conn); SchemaDowngrade.V60ToV59(conn); SqliteConnection.ClearPool(conn); }
    ...
    public void A_migrated_v59_book_has_every_flag_off_and_no_alternate_unit()
    {
        ...
        try
        {
            ...
            using (var conn = Open(path)) { SchemaDowngrade.V64ToV63(conn); SchemaDowngrade.V63ToV62(conn); SchemaDowngrade.V62ToV61(conn); SchemaDowngrade.V61ToV60(conn); SchemaDowngrade.V60ToV59(conn); SqliteConnection.ClearPool(conn); }
    ...
    public void Downgrading_to_v59_keeps_the_primary_keys_of_both_rebuilt_tables()
    {
        ...
        try
        {
            ...
            {
                SchemaDowngrade.V64ToV63(conn); SchemaDowngrade.V63ToV62(conn); SchemaDowngrade.V62ToV61(conn); SchemaDowngrade.V61ToV60(conn); SchemaDowngrade.V60ToV59(conn);
