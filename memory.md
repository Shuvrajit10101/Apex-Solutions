...
- **2026-08-23 - ORPHANED WORKING-TREE CHANGE FOUND IN THE `main` WORKTREE DURING MERGE PREP: CODE DISCARDED AS DEAD, DEFECT CARRIED FORWARD (A12).** While preparing the four working tracks for merge, `git status --porcelain` on the root worktree (`C:\Users\dkpho\OneDrive\Desktop\Apex Solutions(end)`, on `main` at `c655dc2`) showed exactly ONE modified file: `src/Apex.Desktop/ViewModels/ReportRow.cs`, mtime **2026-07-18** - uncommitted for over a month. The change was purely additive: one computed property plus its doc comment.
  - The property: `public bool IsPlainRow => !IsHeader && !IsTotal;`
  - **MEASURED DEAD, two ways.** `grep -rn IsPlainRow src tests docs plan.md memory.md` over the root worktree returns exactly ONE hit - its own declaration. `git grep IsPlainRow` over `main` and all four working tracks (`claude/apex-wrong-figures-bc45f4`, `claude/apex-tier0`, `claude/apex-edit-log`, `claude/apex-print-engine`) plus `backup/stream-a-pre-resplit` returns ZERO files. Nothing binds it from XAML, nothing calls it from C#. And `git diff --name-only a34d989..<branch> -- src/Apex.Desktop/ViewModels/ReportRow.cs` is empty for all four tracks, so the file conflicts with nothing either - this was simply the abandoned first step of a UI fix that was never finished.
