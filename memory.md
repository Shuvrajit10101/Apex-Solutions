...
- **▶ WHAT REMAINS — in priority order:**
  ...
  7. **THE PRINTED-VS-POSTED MONEY DEFECT IS ONLY HALF CLOSED.** §206C TCS rides the party leg and has no `InvoicePrintData` member, so the "printed total == posted party leg" invariant is stated for **NON-TCS** sales only (`plan.md:1697`), and **the item-path footing guard must be sequenced AFTER a TCS row lands or it would refuse every TCS invoice** (`plan.md:1700-1707`). **A supply taxable at 0% posts no leg for either pass to read** — a statutory question needing a rate snapshotted onto the posted line, i.e. **a schema change, not a print-path patch.**
  8. **THE `GstReturnJson` INVENTED KEYS** — the third instance of the statutory-payload class, dead code today, and already plan item **W0-4** (`plan.md:1532`), which is GATED on A14/R7 confirming the GSTN key schema — and the **`CostAllocationStrictness` naming debt** (~13 files across four test projects). Both are known, scoped and safe to defer — **but the first stops being safe the moment a production caller appears.**
