...
### IV-65 · Split Company Data: the two new companies must be NAMED, because our store keys a book by its file name
...
### IV-65a · The Gateway row is labelled "Split Company Data", where the vendor's menu word is "Split"
...
### IV-66 · The split books keep the SOURCE company's id, and its whole audit log
...
| **What TallyPrime does** | Assigns a **new company number** to each company a split creates (citation as IV-65). |
...
### IV-67 · A new book begins on the split date — the vendor's split pages do not say what the new company's period is
...
### IV-68 · Verify Data checks the OPEN company; the vendor lets you pick one
...
### IV-69 · A split is REFUSED where this build cannot carry a book's state forward faithfully
