...
- **24 of 24** TallyPrime predefined voucher types are seeded in the domain. `[CODE: src/Apex.Ledger/Seed/SeedVoucherTypes.cs:19-48`, with a hard count guard at `:51` and `:79-80]`. The number 24 and the enumeration are corroborated `[CORPUS-BOOK p.24: "There are 24 Pre-defined vouchers in Tally Prime", 3-column table]`.
- **23 of 24** have a real, reachable entry screen. **1 is dead data**: `VoucherBaseType.Attendance` appears **nowhere in the codebase except its enum member and its seed row** — verified by exhaustive grep across `src/` and `tests/` `[CODE: only hit is src/Apex.Ledger/Seed/SeedVoucherTypes.cs:46]`. The Attendance *screen* exists but writes `AttendanceEntry` rows, not vouchers `[CODE-H: AttendanceVoucherEntryViewModel]`.
