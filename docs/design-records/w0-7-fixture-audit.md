...
# W0-7 â€” extend `PopulatedCompanyFixture` to every voucher family

**Design agent, read-only pass. Repo read: `â€¦\.claude\worktrees\recursing-swirles-3138c6`, branch
`claude/apex-wrong-figures-bc45f4`, HEAD `3a4fcdb`.**

---

## Â§0 â€” HEADLINE: THE SLICE IS ALREADY BUILT AND COMMITTED. THIS IS NOT A DESIGN, IT IS AN AUDIT.
...
1de940e test(fixture): W0-7 â€” extend the populated fixture to every voucher family
3774695 fix(ui): one row-track fixes three critical layout defects (StockSummary, PriceList, item-invoice)
```

`1de940e` is an **ancestor of HEAD** and the file is **NOT in `git status --porcelain`** â€” i.e. it is
...
| `tests\Apex.Desktop.Tests\Fixtures\PopulatedFixtureCoverageTests.cs` | committed, clean â€” the coverage assertion Â§8 asked for |
...
and (c) the **residual gaps that are genuinely still open** â€” which is the only part that is real work.

> âš ï¸ **For the main loop:** do not re-run W0-7 as a build slice. Re-running it would re-derive a fixture
> that already exists and would collide with `1de940e`. The actionable output of this pass is Â§9
> (residual gaps) and Â§7 (the census/Â§1.3 correction), not a rebuild.

---
## Â§1 â€” GROUND TRUTH: WHAT THE FIXTURE POSTS TODAY (measured from source, not relayed)
...
| The 24th, unseeded | **`Attendance`** | `SeedVoucherTypes.cs line 59-66` â€” the row is deliberately absent (decision D24 option B); the **enum member stays**, because `voucher_types.base_type` persists as the enum **ordinal**, so deleting it would renumber `Payroll` 23â†’22 and every stored Payroll type would load as Attendance |
...
**âš ï¸ ONE STALE COMMENT FOUND, and it is exactly the W0-6 defect class.**
...
says *"Count guard: exactly 23 (was 24 â€” the dead Attendance row is gone)"*. Missed by W0-6's count
sweep. Trivial, one line, and it belongs to **W0-6's open remainder â€” not to this slice.**

> ðŸ”´ **CITE THIS ONE BY TEXT, NOT BY LINE â€” and here is the proof of why.** I read that comment at
...
> *while I was reading*. `SeedVoucherTypeSet()` moved `:61` â†’ `:75` over the same interval. This is the
...
### 1.2 The numerator â€” every base kind the fixture now posts
...
| `PostProvisionalVouchers` | `:1145-1182` | Memorandum, ReversingJournal (+ an **Optional-flagged Journal**, `optional: true` at `:1180` â€” the third exclusion mechanism, which is a *flag*, not a base kind) | 2 |
...
said, and Â§7.2 says the second with the corpus behind it.** *Right in the fixture:* nothing in **our**
product posts a `Voucher` of that kind â€” the screen writes `AttendanceEntry` rows through
...
`GOT > Voucher > F10 > Attendance` (corpus Book PDF p.29 and p.374 â€” see Â§7.2). Nothing in this pass
...
### 1.3 ðŸ”´ THE CENSUS'S TWO CLAIMS, CHECKED â€” one is now FALSE, one is STILL TRUE

| Census claim | Where | Verdict |
|---|---|---|
| *"8 of 23 base types, zero inventory/order/job-work/POS/payroll vouchers"* | `docs\full-clone-census.md:188`, and again in the prerequisite graph at `:249-251` (*"posts 15 more base types â€¦ Currently 8 of 23"*) | **FALSE at HEAD.** It is **23 of 23** since `1de940e` (2026-08-10). Both census sites are stale. |
| *"No print or export test uses `PopulatedCompanyFixture`. Every renderer is locked against thin bespoke fixtures â€¦ it is unchanged."* | `docs\full-clone-census.md:325` (finding 9) | **STILL TRUE at HEAD, verified exhaustively.** See Â§6. |
...
(`tests\Apex.Desktop.Tests\VoucherInvoicePrintViewModelTests.cs line 370`) â€” and that file contains **no
...
### 1.4 The complete consumer list â€” 8 test classes + 1 shared guard

`grep -r PopulatedCompanyFixture` â†’ **44 occurrences in 12 files**, of which 2 are docs (`plan.md` Ã—5,
`docs\full-clone-census.md` Ã—3) and 10 are code:

| Consumer (`tests\Apex.Desktop.Tests\â€¦`) | How it enters |
|---|---|
| `CaAuditLayoutLockTests.cs line 64,71` | `storage.Save(BuildRegular())` â†’ real company-select |
...
(`vm.Menu.First(m => m.Label == RegularCompanyName)`), never by index or id â€” which is why the
`Guid.NewGuid()` identifiers in Â§4 are harmless to them.

---

## Â§2 â€” THE SHAPE OF THE EXTENSION, AS BUILT (briefing item 2)
...
it is the reusable part â€” the next fixture extension will face the same table.

| Family | Posting service used | Master graph it needed | Present pre-W0-7? |
|---|---|---|---|
| ReceiptNote, DeliveryNote, RejectionIn, RejectionOut | `InventoryPostingService.Post(InventoryVoucher â€¦)` | items + godowns + batch labels + a party ledger | **yes** â€” no new masters |
| StockJournal | `InventoryVoucher.StockJournal(â€¦)` | two godowns; source and destination **must balance in the base unit** | yes |
| PhysicalStock | `InventoryVoucher.PhysicalStock(â€¦)` | item + godown; the counted qty **SETS** book qty (DP-3) | yes |
| PurchaseOrder, SalesOrder | `InventoryVoucher.Order(â€¦)` | items + godowns + party; affects **neither** stock nor accounts | yes |
| JobWorkInOrder, JobWorkOutOrder | `InventoryVoucher.JobWork(â€¦)` + `JobWorkOrder` payload | a **third-party godown** (`thirdParty: true`), a finished good, component lines tracked `PendingToIssue` / `PendingToReceive` | **godown added** at `:383` ("Bharat Zinc Electroplating Job Work Premises (3rd Party)") |
| MaterialOut | `JobWorkService.BuildMaterialOutTransfer(â€¦)` | balanced transfer + order link | yes |
| MaterialIn | `JobWorkService.BuildConsumingMaterialIn(â€¦)` | **balance-EXEMPT transform**; requires the type stamped `AllowConsumption` | **gate had to be fixed â€” see 2.1** |
| Memorandum, ReversingJournal | `LedgerService.Post(â€¦)` | ordinary ledgers; `applicableUpto` for the reversing one (`:1167`) | yes |
| Optional (a **flag**, not a base kind) | `LedgerService.Post(â€¦, optional: true)` | none | yes |
| **POS** | `LedgerService.Post(â€¦, posTenders:)` + `PosTenderService` | **a whole graph**: a second POS-flagged `VoucherType` carrying `PosConfig`, plus **four tender ledgers under the groups `PosTenderService` requires** â€” Giftâ†’Sundry Debtors, Card/Chequeâ†’Bank, Cashâ†’Cash-in-Hand (`:1200-1219`) | **ALL NEW** |
| **Payroll** | `PayrollVoucherService.Post(â€¦)` | **the largest graph**: attendance types of the right *kind*, payroll units, pay heads created **through `PayHeadService`** (attendance linkage + computation formulae), employee-scoped salary structures, and **attendance recorded first** | **partly new â€” and three pre-existing master sets were UNPOSTABLE** |
...
1. **The attendance-type *kinds* were all wrong** (`:682-704`). All four were seeded `AttendancePaid` â€”
   including *"Absent â€” Loss of Pay"* and *"Overtime Hours Worked"*, which by definition are not paid
...
2. **The pay heads were unpostable** (`:708-713`). They were built with `new PayHead(...)` straight onto
   the company, **bypassing `PayHeadService`'s validation**, and every one of them violated it â€” the
...
3. **Job Order Processing was a raw flag** (`:91-97`). Assigning `c.EnableJobOrderProcessing = true`
   ...
   unstamped. The fixture now drives `new JobWorkService(c).SetEnabled(true)` â€” the same path the real
   F11 toggle drives â€” which also stamps `UseForJobWork` / `AllowConsumption`.
...
### 2.2 Stock coherence is load-bearing â€” it is the constraint a naive extension breaks
...
outward ones so no key is ever short. This is asserted, not merely commented â€” see Â§8, test F.

---

## Â§3 â€” ODD-VALUE FIXTURES (briefing item 3) â€” SATISFIED, AND LOCKED

The standing rule (a Â±â‚¹0.50 defect survived its whole life under round-number assertions) is honoured.
The figures actually shipped, read from source:

| Family | Odd figures | `file:line` |
|---|---|---|
| ReceiptNote | 465.250 kg @ â‚¹69.35; 212.500 kg @ â‚¹248.65 | `:982-985` |
| DeliveryNote | 47 Nos @ â‚¹2.63; 63 Nos @ â‚¹17.44 | `:995-996` |
| RejectionIn / RejectionOut | 6 @ â‚¹2.63; 18.750 kg @ â‚¹248.65 | `:1004`, `:1011` |
| StockJournal | 42.750 Ltr @ â‚¹178.00, both legs | `:1018-1019` |
| PhysicalStock | counted **1068.375** Ltr | `:1026` |
| PurchaseOrder | 1,250.750 kg @ â‚¹69.35; 96.500 @ â‚¹181.27 | `:1034-1035` |
| SalesOrder | 145 @ â‚¹431.63; 320 @ â‚¹25.47 | `:1045-1046` |
| JobWorkOut / JobWorkIn | 318.625 @ â‚¹61.27; 214.375 @ â‚¹68.53; FG rates â‚¹47.86 / â‚¹21.35 | `:1062-1087` |
| MaterialOut / MaterialIn | 318.625 issued; **306.500 consumed** (deliberately â‰  issued) | `:1103-1123` |
| Memorandum | â‚¹12,473.85 | `:1149` |
| ReversingJournal | â‚¹18,942.37 | `:1159` |
| Optional Journal | â‚¹9,617.42 | `:1172` |
| POS | 34 @ **â‚¹247.99**; tax from the real `GstService`; cash tender is a **derived residual** | `:1224-1235` |
| Payroll | 8 distinct basics â‚¹31,486.50 â€¦ â‚¹21,368.90; advances â‚¹1,943.25 / â‚¹2,617.40 / â‚¹1,284.75 / â‚¹3,071.60; conveyance â‚¹1,687.50; incentive â‚¹118.25/hr | `:1325-1331`, `:1339-1340` |
...
**Two totals are DERIVED, never literal** â€” the POS bill total comes from
`GstService.ComputeInvoiceTax` and the cash tender from `PosTenderService.CashResidual(â€¦)`
(`:1228-1235`) â€” so no hand-typed grand total can drift away from the posted one.

### 3.1 ðŸ”´ The subtlest thing in the slice â€” two odd-value assertions that CANNOT FAIL

`PopulatedFixtureCoverageTests` Â§D (`:318-357`) records two near-misses, and they are the most
transferable lesson in the whole pass:

- **POS.** `HasOddPaisa(bill)` *looks* like the right check and is worthless â€” **18% of any whole-rupee
  taxable value lands on paisa**, so the posted CGST/SGST lines satisfy it whatever the counter rate is.
  Measured: rounding the rate to â‚¹248 left that form of the assertion **green**. The test therefore pins
...
**Rule to carry forward: assert on what a tidy-up would actually round â€” the hand-typed input â€” never on
a derived figure that is odd by arithmetic necessity.**

---

## Â§4 â€” DETERMINISM (briefing item 4) â€” CLEAN HERE; THE NAMED DEFECT IS ELSEWHERE AND STILL LIVE
...
- **No `DateTime.Now` / `.Today` / `.UtcNow`.** Every date derives from the two constants
  ...
  literal `2025-08-01 â€¦ 2025-08-31` (`:1290-1291`).
- **No `GetHashCode`, no `Random`.** Every identifier-shaped string is a formatted counter â€”
...
- **`Guid.NewGuid()` appears 66 times**, exclusively as entity identity. This is *bounded*
  ...
  the company by `m.Label == RegularCompanyName` (Â§1.4), and the fixture's own positional indexing
...
### 4.1 ðŸ”´ THE `Math.Abs(pin.GetHashCode())` DEFECT THE BRIEFING NAMED IS **NOT** IN THIS FIXTURE â€” IT IS LIVE IN THE W0-2b WORKING TREE
...
That file is **untracked** (`?? tests/Apex.Desktop.Tests/CompanyProfileScreenTests.cs`) â€” it is the
...
## Â§5 â€” BLAST RADIUS (briefing item 5) â€” MEASURED, NOT ESTIMATED
...
**did already exist at `1de940e`** (`git cat-file -e 1de940e:â€¦`) and were **untouched** â€” so their
absence from the diff is genuine independence, not "added later".

### 5.1 What actually moved â€” and it is the opposite of what a blast-radius forecast expects
...
|---|---|---|
| `CaAuditLayoutLockTests` | `Rows.Count > 1` â†’ `>= 8` (Batch-wise); `Existing.Count > 0` â†’ `>= 8` (employees) | **floors RAISED** â€” `> 1` was satisfied by one batch row plus a total |
| `KeyboardArbitrationTests` | stale doc-comment counts ("38 ledgers, 28 stock items, 51 vouchers") struck; the file **asserts no absolute count** â€” every arm captures its own `before` at runtime and asserts a **delta** | **doc only**; the tests were already delta-based and got *stronger* on a bigger book |
| `InventoryReportScrollReachabilityTests` | the empty-state detector was replaced (prose `"No "` prefix â†’ structural row counting via the new shared `ReportContentGuard`) | **defect fixed**, not assertion moved |
...
cheap exactly to the extent that the family is inert to the aggregates other tests read** â€” that is the
transferable rule.
...
1. **A guard that could not guard.** `IsEmptyState` matched only placeholder prose beginning `"No "`,
   ...
   sentence**. Detection is now structural â€” a row that is neither `IsHeader` nor `IsTotal` â€” and shared
...
3. **A report's as-of date is computed from `Company.Vouchers` only.** `ReportsViewModel.ComputeAsOf`
   ...
   UNFIXED and is a live carry-forward** â€” see Â§9, R-1.

---

## Â§6 â€” ðŸ”´ THE PRINT/EXPORT GAP (briefing item 6) â€” STILL OPEN. THIS IS THE REAL RESIDUAL WORK.

### 6.1 Verified: census finding 9 is unchanged at HEAD

`grep -rn PopulatedCompanyFixture tests/ src/` returns **10 code files** (Â§1.4). **Not one of them is a
...
| `VoucherPrintProjector.ProjectInvoice` | `Apex.Desktop.Tests\VoucherInvoicePrintViewModelTests.cs` | **bespoke** (confirmed â€” no fixture reference) |
| `ExportViewModel` | `Apex.Desktop.Tests\ExportViewModelTests.cs` | **`vm.LoadRobertDemo()`** â€” the thin Robert demo (`:39`) |
...
so `Apex.Desktop.Tests` has `Apex.Ledger.Io` transitively â€” and **already uses it today**
...
### 6.2 The smallest change that closes finding 9 â€” designed
...
**Test P â€” one PRINT surface.** Open a voucher of a family that only exists because of W0-7 and project
it through the shipped path. The strongest single choice is the **POS bill**, because it is the fixture's
only multi-tender document, its own renderer (`PosReceiptPdf`) had a shipped Â§10(4) defect, and its
...
precisely the invariant W0-10 was built to hold (*"â‚¹47,296.73 printed against â‚¹55,810.14 posted"*), and
today **nothing checks it on a realistic book.**

**Test X â€” one EXPORT surface.** `ExportViewModelTests` already has every seam needed â€”
...
fixture, open a report **that only has rows because of W0-7** â€” `ReportKind.MaterialInRegister` or
`OrderRegister` â€” export CSV, and assert a **known odd figure reaches the bytes**: e.g. `1068.375`
...
renderer is ever driven by a realistic book at all** â€” two tests remove that, and the census row can then
...
## Â§7 â€” RULING 5: THE FIDELITY ROW (briefing item 7) â€” DRAFTED, AND THE CORPUS *DOES* SETTLE IT
...
*"How many Types of Pre-defined Voucher in Tally Prime?"*** â€” an explicit numbered table:

> *"There are 24 Pre-defined vouchers in Tally Prime"* â€” 1 Contra Â· 2 Receipt Â· 3 Payment Â· 4 Purchase Â·
> 5 Sales Â· 6 Journal Â· 7 Memorandum Â· 8 Reversing Journal Â· 9 Rejections in Â· 10 Rejections out Â·
> 11 Credit Note Â· 12 Debit Note Â· 13 Purchase order Â· 14 Receipt Note Â· 15 Sale Order Â·
> 16 Delivery Note Â· 17 Stock Journal Â· 18 Physical Stock Â· 19 Job Work in order Â· 20 Material in Â·
> 21 Job Work out order Â· 22 Material out Â· **23 Attendance** Â· 24 Payroll

**Our `VoucherBaseType` enum is a member-for-member match with this table â€” all 24, same families.** That
is a genuine, sourced fidelity result and it has never been recorded.

### 7.2 ðŸ”´ And the corpus contradicts our decision D24 â€” which must be stated, not glossed

`SeedVoucherTypes` justifies dropping the Attendance seed row with *"nothing in the product ever posted a
Voucher of base kind Attendance â€¦ it needs no voucher type at all"*. **That is true of OUR product and
false of TallyPrime's**, and the corpus is explicit:

> **Book PDF p.374 (printed p.370), *"Part 1: Attendance Voucher"*:** *"Attendance Voucher â€” It is used
> to record Employee's attendance dates based of Attendance/Production types (Present or absent or
> Overtime by days or Hours)."* â€” entered at **`GOT > Voucher > F10 > Attendance`**, capturing Date,
...
the voucher-ness are a divergence** â€” and D24's stated rationale is a statement about our
...
### 7.3 The row to add to `docs\full-clone-census.md` Â§1.3 â€” proposed text

> 10. **Voucher families â€” the 24 pre-defined kinds, and what each affects (added with W0-7).**
...
>     `VoucherBaseType` enum is a **member-for-member match** â€” Contra, Receipt, Payment, Purchase,
...
>     â€” Book PDF p.374, *"Attendance Voucher â€¦ GOT > Voucher > F10 > Attendance"*, with manual and
>     autofill modes â€” whereas we seed no Attendance voucher type and write non-voucher
...
>     **NOT SOURCED, and deliberately so:** nothing about the fixture's own *contents* â€” the party names,
...
**Note the shape:** it follows row 9's precedent â€” what IS sourced, what is OURS, and where each
divergence is already logged â€” which is what the R12 ruling at `docs\full-clone-census.md:82` asks for.

---

## Â§8 â€” TESTS (briefing item 8) â€” THE LOCK EXISTS AND IS STRONG; TWO HOLES REMAIN
...
| C | `Regular_fixture_posts_a_payroll_voucher_carrying_per_employee_detail` | a **half-run** â€” floor pinned at the whole roster, not a token `>= 4` |
...
**The briefing's second question â€” *"what stops the fixture being extended later without its coverage
assertion being updated?"* â€” is answered well.** Test A derives its **denominator from
...
- **H-1 â€” a new enum member with NO seed row is invisible to test A.** A's denominator is
  `c.VoucherTypes.Select(t => t.BaseType)`, i.e. the *seeded* kinds. Add a `VoucherBaseType` member and
  no seed row and **nothing fails** â€” which is precisely how `Attendance` reached its current state.
  The nearest guard, `SeedTests.cs line 157` (`Assert.Equal(23, SeedVoucherTypes.Count)`), locks the seed
  count but **never compares it to `Enum.GetValues<VoucherBaseType>().Length`**. A three-line test â€”
  *every enum member is either seeded or on a named, justified exclusion list* â€” would close it and
  would document the Attendance exclusion in code rather than in a comment.
- **H-2 â€” no print or export surface is in the lock at all.** Tests G covers 21 *report* kinds; there is
  no equivalent for the renderers. Â§6 is the fix.

---

## Â§9 â€” RISK, SCOPE, AND THE RESIDUAL WORK (briefing item 9)

### 9.1 What the shipped slice deliberately left out â€” and I agree with each
...
- **Payroll's voucher type stays `IsActive = false`.** The seed ships it inactive and **nothing in the
  ...
  the flag). The pay run is posted through `PayrollVoucherService`, which bypasses the resolver â€” so the
  voucher, the payslip and the payroll register are populated **while the Alt+A picker still correctly
  omits the type.** *(Briefing item 9's "verify before designing around it" â€” verified: `SeedVoucherTypes`
  ships `new("Payroll", â€¦, false)`; `CanAddFromDayBook` excludes Attendance, and the resolver excludes
...
- **Catalogue-depth SKUs are masters only** â€” no opening stock, no movements â€” so they stress list
  geometry without perturbing any aggregate.

### 9.2 ðŸ”´ RESIDUAL WORK â€” the actionable output of this pass

| id | What | Severity | Owner |
|---|---|---|---|
| **R-1** | **`ReportsViewModel.ComputeAsOf` scans `Company.Vouchers` only, never `Company.InventoryVouchers`.** A book whose newest activity is a stock movement (goods despatched today, invoice raised next week â€” the commonest sequence in an Indian trading book) opens **every report at an as-of date earlier than its own newest stock movement**, and that movement is invisible until the operator changes the date by hand. **Reproduced** during W0-7 (the Material In on day 171 fell outside the window and the register rendered EMPTY) and **worked around in the fixture, not fixed.** | **HIGH â€” a live product defect, not a test defect** | needs a plan row; belongs with the register/report work |
| **R-2** | **The print/export gap (census finding 9) is still open.** Design in Â§6: one new file, two tests, no production change, no new project reference. | MEDIUM â€” it is what turns the fixture from a posting fixture into a regression instrument | the natural W0-7 tail |
...
| **R-4** | **H-1** â€” a new `VoucherBaseType` member with no seed row is invisible to the coverage lock. ~3-line test. | LOW | opportunistic |
| **R-5** | `CompanyFactory`'s `// 24 voucher types.` comment vs `SeedVoucherTypes.Count == 23`. | TRIVIAL | **W0-6's open remainder** |
| **R-6** | **`Math.Abs(pin.GetHashCode())` at `CompanyProfileScreenTests.cs line 329`** â€” randomised per process, and `Math.Abs(int.MinValue)` throws. **Untracked file = the in-flight W0-2b slice.** | MEDIUM | **W0-2b fix pass â€” NOT this slice** |

### 9.3 ðŸ”´ DOCUMENT CORRECTIONS OWED (R5/R6) â€” the highest-value item in this pass

**`plan.md` and the census both still describe W0-7 as unstarted, seven days after it shipped.**

| File | Text | Correction |
|---|---|---|
| `plan.md` (Phase 10.12 row list) | *"**W0-7 (S0) `PopulatedCompanyFixture` extension** â€¦ It covers **8 of 23 base types and zero inventory, order, provisional, job-work, POS or payroll vouchers**, and **no print or export test uses it at all.**"* â€” **no DONE marker** | Mark **DONE at `1de940e` (2026-08-10)**; **23 of 23**; note the print/export half (R-2) is **the one part still open** |
| `plan.md` (Â§5 ruling-6 banner, and the Phase 10.11 header) | Both argue W0-7 must ship before Phase 10.11 *"while that fixture covers 8 of 23 base types"* | The **prerequisite is SATISFIED**. Phase 10.11 is **unblocked** â€” which is the operative fact for the main loop |
| `docs\full-clone-census.md:188` | *"51 is right; 'every type' is not â€” 8 of 23 base typesâ€¦"* | Now **23 of 23** since `1de940e` |
| `docs\full-clone-census.md:249-251` (prerequisite graph, S0) | *"posts 15 more base types â€¦ Currently 8 of 23; no print/export test uses it at all"* | S0 **DONE**; the print/export clause **still stands** |
| `docs\full-clone-census.md:325` (finding 9) | *"No print or export test uses `PopulatedCompanyFixture` â€¦ it is unchanged."* | **STILL TRUE â€” leave it standing** until R-2 lands |
| `docs\full-clone-census.md` Â§1.3 | no row for W0-7 | Add **row 10** (Â§7.3) |

âš ï¸ **Both `plan.md` and `docs\full-clone-census.md` are ` M` in the working tree right now** (the W0-2b
agent holds them). **These corrections must be sequenced after that slice commits, or they will be lost
in a conflict.**

---

## Â§10 â€” BOTTOM LINE FOR THE MAIN LOOP

1. **W0-7 is DONE** â€” `1de940e`, 2026-08-10, ancestor of HEAD, clean in the working tree. **23 of 23**
   seeded base kinds posted, locked by a 550-line coverage test across a real SQLite round trip.
   **Do not rebuild it.**
2. **The stated blocker on Phase 10.11 is therefore DISCHARGED.** The ruling of 2026-08-16 â€” *"W0-7
   ships first, then Phase 10.11"* â€” was made against a `plan.md` row that was already six days stale.
...
3. **Three things are genuinely still open**, in priority order: **R-1** (the `ComputeAsOf` product
   defect â€” the only one that harms a user), **R-2** (the print/export gap, ~1 file), and the
   **document corrections in Â§9.3**.
