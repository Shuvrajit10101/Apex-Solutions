...
# Phase 10.11 â€” Voucher lifecycle (alter Â· delete Â· cancel) â€” DESIGN
...
**Gate at HEAD (given, not re-measured):** build 0W/0E Â· Ledger 1668 Â· Io 414 Â· Sqlite 231 Â· Desktop 2195 Â· schema **v51**.
...
| Â§ | Topic | State |
|---|---|---|
| 0 | Preamble â€” method, and one methodological finding that matters | done |
| 2 | R7 â€” what the corpus actually settles about Alter / Delete / Cancel | done |
| 1 | Ground truth â€” what exists in the tree today | done |
| 3 | The hard part â€” unwinding derived state | done |
| 4 | Numbering | done |
| 5 | Audit â€” the excluded Edit Log, and what its absence costs | done |
| 6 | Slice shape | done |
| 7 | Tests, including the RED-PROOF | done |
| 8 | Risk, and the ER-13 byte-identity question | done |
| 9 | Schema | done |
| 10 | RULING 5 â€” the fidelity rows for `docs/full-clone-census.md` Â§1.3 | done |
| â€” | APPENDIX â€” the ten decisions this design asks for (R12) | done |
| **11** | **LATE FINDINGS â€” read these; they correct Â§6.3, Â§10 and `plan.md`** | done |

> ## ðŸ”´ READ FIRST â€” four things that change what you were about to do
>
...
>    **This phase is three slices, not five.** Â§1.1
...
>    `plan.md:320` cites it as if sourced, via a section id that does not exist. Â§11.1
> 3. **DELETING THE HIGHEST-NUMBERED VOUCHER REUSES ITS NUMBER.** `NextNumber` is `max+1` by scan. The engine's
>    own doc says only *"may leave a gap"*. VL-2 is the slice that makes this reachable. Â§4.1
> 4. **THE WORKING TREE IS NOT CLEAN AND ANOTHER AGENT IS WRITING A PHASE 10.11 DESIGN INTO THE REPO**
>    (`docs/design-records/phase-10-11-voucher-lifecycle-design-PARTIAL.md`, written during this run). Nothing
>    in it is mine; I did not read or touch it. Reconcile before executing either. Â§11.3

---

# Â§0 â€” Preamble: method, and one methodological finding

## 0.1 What I did

- Converted the nine admissible corpus PDFs with `pdftotext -layout` into a scratchpad `corpus/` directory
  (`659947760-Tally-Prime-Short-Key.pdf` **excluded** â€” REJECTED as a source per the standing rule).
...
## 0.2 ðŸ”´ METHODOLOGICAL FINDING â€” `-layout` SILENTLY SCRAMBLES THE BOOK'S SHORTCUT TABLES; `-raw` FIXES THEM

This is the most reusable thing in this document and it invalidates at least one R7 claim already written into
`plan.md`.

`664311548-Tally-Prime-Book.pdf` pages 435â€“437 carry TallyPrime's own three-column shortcut table (Key /
...
(15 keys : 15 functions) and the pairing is recoverable. **On p.436 and p.437 they do not** â€” p.436 yields 20
keys against 21 function-fragments, p.437 yields 10 against 11 â€” and any pairing read off the `-layout` dump is
a guess.
...
**Consequence for this phase (see Â§2.4):** the `plan.md` R7 line claiming TallyPrime *"reserves **Ctrl+Enter**
...
off a scrambled `-layout` dump. **Any future R7 finding sourced from Book pp.435â€“437 must be re-derived with
`-raw`.**
...
# Â§2 â€” R7: what the corpus actually settles
...
## 2.1 SETTLED â€” how alteration is reached, and that there is no read-only voucher screen
...
â€” `664311548-Tally-Prime-Book.pdf` PDF **p.32** (Contra), **p.34** (Receipt), **p.37** (Payment), **p.42**
...
3. **`Ctrl+A` saves the altered voucher** â€” the same accept key as creation.
4. **`Alt+D` on the selected register row deletes it** â€” corroborated independently at Â§2.2.
...
The divergence is settled and is not re-litigated here â€” but Â§2.4 shows the *stated reason* for one half of it
is wrong.

## 2.2 SETTLED â€” Alt+D is Delete; Alt+X is Cancel; and the neighbours

From the Book's own shortcut table, re-extracted with `-raw` (Â§0.2), `664311548-Tally-Prime-Book.pdf`
...
- **`Alt+X` = cancel a voucher â€” CONFIRMED, and its scope is BOTH "Vouchers" and "Reports"**, not report-only.
  ...
  *"Vouchers & Reports"*. Our slice may still ship report-only â€” but it must say so as **our** scope decision,
  not as fidelity.
- **`Alt+D` = delete â€” CONFIRMED**, and the same key deletes masters from the alteration screen (Â§2.3).
...
## 2.3 SETTLED â€” the delete confirmation is a DOUBLE Yes/No, and its wording is published
...
`Enter' Button"** â€” i.e. two consecutive confirmations:

- Delete **Company**: *"Gateway of Tally > Alt+K > Alter > Alt+D > Press Two times `Enter' Button"* â€” Book PDF **p.15**
- Delete **Ledger**: *"GOT > Alter > Ledger > Select Ledger for Delete & Press Enter > Alt+D > Press Two times Enter"* â€” Book PDF **p.21**
- Delete **Voucher Type**: *"GOT > Alter > Voucher type > Select Voucher for Delete & Press Enter > Alt+D > Press Two times Enter"* â€” Book PDF **p.23**
- Delete **Ledger** (second source): *"Press Alt+D â†’ supply Yes to confirm Deletion."* â€” STUDY-GUIDE PDF **p.67**

**Finding.** The two-prompt pattern (`Delete Yes or No?` â†’ `Are you sure Yes or No?`) is **corpus-settled for
...
"Are you sure" prompt as **NOT ATTESTED FOR VOUCHERS** rather than copying it across by analogy â€” the corpus
...
## 2.4 ðŸ”´ CORRECTION TO A CLAIM ALREADY IN `plan.md` (R7)
...
`Ctrl+Enter` â†’ *"To alter a master during voucher entry or from drilldown of a report"*.

So in TallyPrime `Ctrl+Enter` is an **alteration** key, not a display key â€” for a **master**, reached either
...
1. **USER DECISION 1 survives, but half its stated reason does not.** The decision binds `Ctrl+Enter` to
   *voucher* alteration and plain Enter to a read-only column. Against the corpus, the Enter half is still a
   deliberate divergence (Tally's Enter goes straight to Show/Edit â€” Â§2.1), but the **Ctrl+Enter half is closer
...
## 2.5 SETTLED â€” a ledger with transactions cannot be deleted
...
Related, same family, Book PDF **pp.104â€“105**: *"You will not be able to delete a Cost Category in multiple
modes"* / *"â€¦a Cost Centers in multiple modes"* â€” i.e. multi-master screens offer alter but **not** delete.
Relevant if a Multi-Alter screen ever grows an Alt+D.

## 2.6 SETTLED â€” TallyPrime has Duplicate and Insert as *separate* verbs from Alter

- `Alt+2` â€” *"To create an entry in the report, by **duplicating** a voucher"* (Book PDF p.435).
- `Alt+I / Alt+A` â€” *"To **insert** or Add a voucher in a report"* (Book PDF p.435).
...
settlement gesture is `Alt+A` on the Outstandings screen) â€” the plan cites this and the citation checks out.
`Alt+2` (Duplicate) is a **named carry-forward**, not built here.

## 2.7 ðŸ”´ NOT SETTLED BY THE CORPUS â€” five questions, and they must NOT be filled by invention

| # | Question | Corpus state |
|---|---|---|
| C-1 | What does **Cancel** MEAN â€” does the voucher keep its number? | **NOT SETTLED.** The corpus's only statement about cancellation is the four words *"To cancel a voucher"* (Book PDF p.437). No corpus text anywhere describes the *effect* of cancelling. `grep -oic cancel` over all nine admissible PDFs returns **2 hits total**, one of which is *"cancelled cheque with Form 5 IF"* in the EPF chapter (Book PDF p.320). |
| C-2 | Is a cancelled voucher shown **struck through**? | **NOT SETTLED.** Zero corpus hits for `struck`, `strike through`, `strike-through`. `plan.md:267` specifies a **greyed** Day Book row â€” that is **ours**, and must be recorded as ours. |
| C-3 | The **cancellation confirmation wording**. | **NOT SETTLED** (as `plan.md` already states). The delete wording IS published (Â§2.3); the cancel wording is not. |
| C-4 | Does **un-cancel** exist? | **NOT SETTLED** (as `plan.md` already states). ORCHESTRATOR RULING 3 ships no un-cancel; that stands. |
| C-5 | What happens to the **number** of a **deleted** voucher â€” reused, retired, or a permanent gap? | **NOT SETTLED.** No corpus text. See Â§4, where our own code answers it â€” badly. |
...
*already* implements retain-the-number (`LedgerService.Cancel` sets a flag and never touches `Number` â€” Â§1.3),
and that is a good design; but it must be documented as **UNVERIFIED-BY-DESIGN â€” our choice, corpus silent**,
in exactly the shape R7 already uses elsewhere. Do not write "as TallyPrime does" anywhere near it.

---

# Â§1 â€” Ground truth: what actually exists at `3e968b3`

## 1.1 ðŸ”´ THE BIGGEST FINDING â€” TWO OF THE FIVE PLANNED SLICES HAVE ALREADY SHIPPED

`plan.md`'s Phase 10.11 lists five slices S1â€¦S5. **S1 and S2 are merged ancestors of HEAD and were verified in
the code, not just in the log:**

| Slice | Commit | Date | Verified in code |
|---|---|---|---|
| **S1 â€” the Alt+D modifier hole** (VL-2 step 1) | `6a28d15` *"fix(desktop): quick jumps and the accept prompt stop answering modifier chords (10.11 S1)"* | 2026-08-07 | âœ… `src/Apex.Desktop/Views/MainWindow.axaml.cs line 1096-1097` now reads `=> vm.IsMenuScreen && !IsTyping(e) && e.KeyModifiers == KeyModifiers.None;` |
| **S2 â€” settlement off Ctrl+B** (VL-4 / IV-5) | `f2abdbb` *"fix(desktop): settlement comes off Ctrl+B â€” Alt+A now OPENS a voucher instead of posting one (10.11 S2)"* | 2026-08-07 | âœ… `MainWindow.axaml.cs line 385-410` is now a RESERVED-DO-NOT-BIND comment block where the arm stood; `MainWindow.axaml.cs line 621-627` is the Alt+A Outstandings arm calling `vm.OpenSettlementVoucherFromOutstandings()`; `BillSettlementService.cs line 19` records *"`SettleAndPost` is therefore deleted"* |

**`plan.md` is stale on this and must be corrected.** Two specific sentences in the Phase 10.11 row are now
false:

1. *"**Also closes the modifier hole** â€” `CanQuickJump` never tests `e.KeyModifiers`, so **Alt+D already opens
   the Day Book today**."* â€” **FALSE at HEAD.** It tests `e.KeyModifiers == KeyModifiers.None`. The hole is shut.
2. The VL-4 warning that leaving the button-bar row *"would paint a red badge that fires nothing â€” the IV-31
   defect"* â€” **did not happen.** `MainWindow.axaml.cs line 1201-1202` `OnSettleBillsClick` was **repurposed**, not
...
**Consequence for the slice plan: this phase is now THREE slices, not five** (Â§6). Anyone starting from
`plan.md` alone would re-do 10.11 S1 and S2.

## 1.2 The aggregate â€” `Company`

`src/Apex.Ledger/Domain/Company.cs line 8` â€” `public sealed class Company`. Framework-agnostic; holds the posted set
...
- `Company.cs line 13` `private readonly List<Voucher> _vouchers = new();` â†’ `Company.cs line 420` `public IReadOnlyList<Voucher> Vouchers => _vouchers;`
- `Company.cs line 26` `private readonly List<InventoryVoucher> _inventoryVouchers = new();` â†’ `Company.cs line 461` `public IReadOnlyList<InventoryVoucher> InventoryVouchers => _inventoryVouchers;`

**âš ï¸ Trap.** Both properties return the **live `List<T>` instance**. `((List<Voucher>)company.Vouchers).Add(v)`
...
| **`RemoveVoucher(Voucher)`** | **`public`** | `:971` | bare `List.Remove`, **no guards** â€” used by import roll-back and the Desktop save-failure undo |
...
## 1.3 The posting service â€” `LedgerService`

`src/Apex.Ledger/Services/LedgerService.cs`, **162 lines, six public members**. Full text read.

- `:30` `public Voucher Post(Voucher voucher)` â†’ delegates with `CostAllocationStrictness.Strict`.
- `:42` `public Voucher Post(Voucher voucher, CostAllocationStrictness)` â€” the only guarded accounting entry
  ...
  â€” **mutates `Number`**; `:54` `_company.AddVoucherInternal(voucher)`; `:61` returns the **same instance**
  (no clone).
- **`:92` `public void Cancel(Guid voucherId)`** â€” EXISTS. Body is three lines: find-or-throw, then
  `v.Cancelled = true;` (`:96`). **Nothing else.** Number retained, row retained, position retained.
- **`:103` `public void Delete(Guid voucherId)`** â€” EXISTS. find-or-throw, then
  `_company.RemoveVoucherInternal(v);` (`:107`). No reversal voucher, no tombstone, no audit row.
- `:120` `ConvertToRegular(Guid, Guid)` â€” Memorandum â†’ real. **Read this one: it is the closest existing
...
- `:154` `public int NextNumber(Guid voucherTypeId)` â€” `max(Number) + 1` by linear scan. **Does not skip
  cancelled vouchers** (correct â€” a cancelled number must stay burned) **and cannot see deleted ones** (Â§4).
...
### 1.3b The inventory twin â€” `InventoryPostingService`
...
Cancel analogue is deferred"*) is a **UI** deferral, not an engine one â€” the engine method is there.

## 1.4 ðŸ”´ CAN ANYTHING TODAY REMOVE OR AMEND A POSTED VOUCHER? â€” the precise answer
...
1. `src/Apex.Ledger/Services/GstSetOffService.cs line 315-320` â€” the **only production call of
   `LedgerService.Delete` in `src/`**. Idempotent period replace: delete the prior set-off voucher(s) for the
   period, then re-post.
2. `src/Apex.Ledger.Io/ApplyJournal.cs line 234-236` â€” transactional import roll-back, undoing a failed apply in
   reverse dependency order via the public `RemoveVoucher`/`RemoveInventoryVoucher`.
3. **Desktop save-failure undo** â€” `VoucherEntryViewModel.cs line 3112`, `:4267`, `:4854`, `PosBillingViewModel.cs line 724`,
   all `undo.Push(() => _company.RemoveVoucher(posted));`. The pattern (`VoucherEntryViewModel.cs line 4251-4272`)
   is **`Post` â†’ `try { _storage.Save(_company); } catch { _company.RemoveVoucher(posted); â€¦ }`**.
...
- `VoucherBaseType.ReversingJournal` + `Voucher.ApplicableUpto` (`Voucher.cs line 101`) â€” a what-if voucher *type*, not an action.
- `GstReversalService` â€” **ITC** reversal (Rule 37/37A/42/43, Â§17(5)); returns an `ItcReversal` audit row.
- `RcmService` "reverse charge" â€” GST liability direction.
- `EInvoiceService.Cancel(record, â€¦)` (`:193`) / `EWayBillService.Cancel(record, â€¦)` (`:281`) â€” cancel the **IRP/NIC artefact**, never the voucher.
- `PayrollVoucherService.Rollback()` (`:484`) â€” undoes auto-created **ledgers**, not vouchers.
- `MainWindowViewModel.CancelVoucher()` (`:4981`) â€” **Esc/Alt+X abandon-the-entry-screen**, not voucher cancellation. Its own doc: *"Alt+X: cancel the in-progress voucher (no save) and pop its page column."*

**The accounting-correct amendment path that DOES exist** is a Â§34 note:
`CreditDebitNoteService.BuildCreditDebitNote` (`:88`) registers a `GstCreditDebitNoteLink` back to the original,
which is never touched. Â§34(2) time limit guarded at `:108-114`.

## 1.5 The voucher model

`src/Apex.Ledger/Domain/Voucher.cs line 7` â€” **`public sealed class Voucher`. A class, not a record. Mutable.**

| Member | Declaration | line | Mutable |
|---|---|---|---|
| `Id` | `public Guid Id { get; }` | `:14` | **no** |
| `TypeId` | `public Guid TypeId { get; }` | `:17` | **no** |
| `Number` | `public int Number { get; set; }` | `:20` | yes â€” set by `Post` |
...
| **`Cancelled`** | `public bool Cancelled { get; set; }` | **`:88`** | yes â€” **the status field** |
...
1. **The status field is `Cancelled` (a plain `bool`), NOT `IsCancelled`, and there is no enum.** There is **no
   `Status`, no `PostedAt`, no `CreatedAt`, no `ModifiedAt`, no user stamp** â€” on the object or in the table
   (`Schema.cs line 941-961`: no `created_at`, no `modified_at`, no `deleted_at`). Â§5 is about exactly this.
2. **`Date`, `TypeId` and `IsAccountingInvoice` are get-only.** So *"altering a voucher's DATE is
   warn-and-proceed"* (ORCHESTRATOR RULING 2) **cannot be done by mutating the voucher** â€” it forces
...
3. There is **no `VoucherNumber` string**. The raw sequence is `int Number`; the rendered document number is
   `Company.FormatVoucherNumber(Voucher)` (`Company.cs line 1004`) â†’ `VoucherNumberFormatter.Render(type, number, date)`
   = prefix ++ zero-padded number ++ suffix, with date-selected affix rows. **A pure projection â€” nothing to
   unwind.** Inventory overload at `Company.cs line 1015`.

`EntryLine` (`src/Apex.Ledger/Domain/EntryLine.cs line 22`) is **fully immutable** â€” every property `{ get; }`. Its
...
`InventoryVoucher` (`Domain/InventoryVoucher.cs line 21`) is deliberately a separate class â€” a stock/order voucher
posts no accounting entry (DP-5) and cannot satisfy Î£ Dr = Î£ Cr. `Cancelled` at `:49`; **no `Optional`**.

## 1.6 Persistence â€” the store is a SNAPSHOT, not the system of record

`src/Apex.Persistence.Sqlite/SqliteCompanyStore.cs line 1790` `public void Save(Company company)`:
one transaction â†’ `ReadStoredSourceOrders` â†’ **`DeleteCompanyRows`** (delete-all) â†’ ~40 `Insert*` calls
re-inserting the whole aggregate in FK order â†’ commit. **A save is O(whole book).**
...
Inventory vouchers are rehydrated *directly* (`:1586-1587`, `company.AddInventoryVoucher(iv)`) â€” an asymmetry
explained in-source at `:1584-1585`.

**ðŸ”´ AND THE ORDER IS PERSISTED.** `ReadVouchers` selects `FROM vouchers WHERE company_id = $cid ORDER BY rowid`
(`SqliteCompanyStore.cs line 4082`), and `Load` re-posts in that order, appending. **So the in-memory list position of
a voucher survives saveâ†’load and is a real, user-visible property** (the Day Book order of same-dated vouchers).
This is the hard evidence for `plan.md`'s "list position" identity requirement â€” it is not a nicety.

**ðŸ”´ A LATENT DEFECT FOUND WHILE MAPPING â€” `IVoucherRepository.Remove` IS INCOMPLETE.**
`SqliteCompanyStore.cs line 1919` `public void Remove(Guid companyId, Guid voucherId)` deletes, in one transaction:
`bill_allocations` â†’ `cost_allocations` â†’ `bank_allocations` â†’ `entry_lines` â†’ `vouchers`. It does **NOT** delete
`tds_lines` (`Schema.cs line 990`), `tcs_lines` (`:1007`), `payroll_lines` (`:1824`) â€” all FK `entry_lines` â€” nor
...
implementer either fixes it or deliberately does not touch it. See Â§8.

## 1.7 Day Book and registers â€” what exists
...
**â‡’ Cancellation is ALREADY honoured by every balance and every stock figure.** `Cancel` works today at the
engine level; only the UI and the *visual* treatment are missing. That makes S3 by far the cheapest of the
three remaining verbs (Â§6).
...
- Alt+A on the Day Book already opens an add-voucher picker beside the live report:
  ...
  â€” which Â§2.6 independently re-confirms.
- The app-wide **Alt+X** arm at `MainWindow.axaml.cs line 350-355` currently calls `vm.CancelVoucher()`, i.e.
  **abandon the entry screen**. It is guarded only by `e.KeyModifiers.HasFlag(KeyModifiers.Alt)` â€” **no
  `!IsPickerOpen`, no screen scope.** `plan.md`'s VL-3 description of this arm is accurate.

## 1.8 The regression surface is genuinely wide â€” W0-7 verified

`1de940e` *"test(fixture): W0-7 â€” extend the populated fixture to every voucher family"*, **2026-08-10**, is an
ancestor of HEAD (`git merge-base --is-ancestor 1de940e HEAD` â‡’ true). The lock lives at
`tests/Apex.Desktop.Tests/Fixtures/PopulatedFixtureCoverageTests.cs` and asserts coverage **as data, not as a
comment**: `SeededBaseTypes(c)` is `c.VoucherTypes.Select(t => t.BaseType).ToHashSet()` â€” derived from the
company's own seed â€” and `PostedBaseTypes(c)` unions **both** `c.Vouchers` and `c.InventoryVouchers`. POS
...
**The prerequisite is discharged. The reasoning it was built on still binds** â€” see Â§7.4.

---

# Â§3 â€” THE HARD PART: what amending a posted voucher means for derived state
...
call**. There is no cache, no materialised view, no running-balance table, no persisted cost-layer stack â€” I
...
about it**. And the plan's phrasing â€” *"the old voucher's derived records must be unwound before the
replacement re-derives"* â€” is right for half of them and **actively destructive for the other half**, because
half of them are not derived from the voucher's content at all. They are **relationships to the outside world**
(an IRN the IRP issued, a challan the bank stamped, a bank date a human ticked, a Â§34 link to another voucher).
Re-deriving those means **inventing** them; unwinding them means **losing** them.

**â‡’ The single most important design rule in this phase: classify every voucher-attached record into
RE-DERIVE / CARRY-FORWARD / REFUSE before writing a line of `Replace`.** Â§3.3 does that classification.

## 3.1 COMPUTED ON THE FLY â€” nothing to unwind (verified, with the recompute site)

| Derived figure | Recompute site | Verdict |
|---|---|---|
| Ledger closing balance | `Reports/LedgerBalance.cs line 95-106` â€” `SignedOpening + Î£ line.Signed` over `company.Vouchers`, every call | ON-FLY |
...
| Stock on-hand | `Services/InventoryLedger.cs line 135-180` â€” replays opening + every movement; Physical Stock acts as a checkpoint (`:193-207`) | ON-FLY |
| **Stock valuation, incl. FIFO/LIFO layers** | `Services/StockValuationService.cs line 274-318` â€” **I opened this myself**: `BuildLayers` opens with `var layers = new List<Layer>(); // oldest-first` and replays the movement stream into a **local**. `Layer` is a `private readonly record struct` (`:505`) with no table and no id. Moving average `RunAverage` `:329-363`; issue costing `IssueValue` `:114-147` rebuilds from scratch | ON-FLY |
| Outstandings / bill ageing | `Reports/Outstandings.cs line 124-196` â€” transient `Dictionary<string, BillState>`; `BillState` is a local `private sealed class` (`:245-253`) | ON-FLY |
| Cost centre / category | `Reports/CostReports.cs line 136`, `:167` | ON-FLY |
| GSTR-1 / GSTR-3B | `Reports/Gstr1.cs line 217`, `Reports/Gstr3b.cs line 130` â€” re-read off posted `EntryLine.Gst` every call | ON-FLY (over stored line tax â€” see Â§3.2) |
...
| Bank Reconciliation **statement** | `Reports/BankReconciliation.cs line 78-104` | ON-FLY (but see Â§3.3 for the stored bank *date*) |
| Saved report views | config only â€” `Schema.cs line 1561-1563` states in-source that a saved view *"can never go stale (ER-9)"* | ON-FLY |
...
book â€” so a Balance Sheet is roughly quadratic in book size, and every lifecycle test that asserts a Balance
Sheet pays that. (b) **Nothing here will tell you the unwind was wrong**; a stale figure is impossible, so the
tests in Â§7 must attack Â§3.2 and Â§3.3, not Â§3.1.

## 3.2 STORED **ON** THE VOUCHER'S OWN LINES â€” dies with the voucher, but MUST be RE-COMPUTED, never copied
...
**ðŸ”´ THE INVERSION TRAP â€” "the posted `Voucher.Lines` is NOT what the operator keyed".** `plan.md` names this
...
`Replace` must **invert the carve to recover the gross**, and on Accept **re-carve from the restored gross** â€”
not re-apply the stored carve to a new base. Get this wrong and the amount drifts by exactly the carve.
`plan.md`'s driving test (2) is precisely this, and Â§7 keeps it with an **odd-paise** carve so a Â±â‚¹0.50 drift
cannot hide.

**âš ï¸ A SECOND INVERSION THAT `plan.md` DOES NOT NAME.** `PayrollComputationService` and `GstService` are the
other two writers whose output is on the line rather than in the operator's head. Any `ForAlter` rehydration
must have a stated answer for each of the six rows above â€” "invert it", "re-key it", or "refuse alteration for
...
## 3.3 ðŸ”´ STORED **BESIDE** THE VOUCHER â€” the real unwind surface, classified

Each of these is a separate object on `Company`, pointing at the voucher by `Guid`. **This table is the
deliverable of Â§3.** The right column is the design decision, not an observation.
...
| **`EWayBillRecord`** | `Company.cs line 504`; `eway_bills` `Schema.cs line 502` | `SourceVoucherId`, `DocumentNumberUpper` | **`ConsignmentValuePaisa`** (`EWayBillRecord.cs line 52` â€” *I opened this*: *"The Rule-138 consignment value in integer paisa (computed off the posted lines, Â§1.3), **stored for audit**"*) | **CARRY + WARN.** âš ï¸ **Highest silent-divergence risk in the phase**: amend the amounts and the EWB states a consignment value the invoice no longer supports, with nothing to detect it. |
| **`TdsChallan` / `TcsChallan` + `ChallanVoucherLink` / `TcsChallanVoucherLink`** | `Company.cs line 663-678`, `:681-706`; `tds_challans` `Schema.cs line 1022`, `challan_voucher_links` `:1036` | `(ChallanId, VoucherId)` pair | ChallanNo, BsrCode, DepositDate, Amount, Section | **CARRY.** âš ï¸ Note the asymmetry I verified at `Reports/ChallanReconciliation.cs line 85-92`: `ChallanHasLiveVoucher` drops a challan whose booking voucher `is { Cancelled: false }` fails â€” so **cancel and delete SELF-HEAL the report; amend does NOT.** Amending the booking voucher's amount leaves a challan whose frozen `Amount` no longer matches, and the reconciliation simply reports the wrong Remaining. |
| **`GstCreditDebitNoteLink`** | `Company.cs line 508` | `CdnVoucherId`, `OriginalInvoiceVoucherId` (both `Guid`, `GstCreditDebitNoteLink.cs line 23`, `:29`) | `OriginalInvoiceDate` (drives the Â§34(2) 30-Nov cut-off), `ReasonCode`, `Is9BTarget` | **CARRY â€” and this is exactly what `plan.md`'s driving test (3) protects.** Preserving the **Guid** is what preserves the link; the object is immutable and needs no touching, *provided* the replacement keeps the Id. |
| **`GstSetoffLine`** | `Company.cs line 530`; `gst_setoff_lines` `Schema.cs line 623` | `VoucherId` | period set-off paisa | **RE-DERIVE â€” but not by us.** `Services/GstSetOffService.cs line 317-321` already deletes prior lines + their voucher and re-posts on a period re-run. Amending a source sales/purchase voucher afterwards leaves the set-off **stale until the operator re-runs the period**. **Named gap, not fixed here.** |
...
| **`Gstr2bReconResult`** | `Company.cs line 526` | `MatchedVoucherId` (`Gstr2bReconResult.cs line 22-40`) | `TaxableVariancePaisa`, `TaxVariancePaisa`, `ReconciledAt` | **CARRY + named gap** â€” a frozen variance against a voucher that just changed. |
...
| **`BankAllocation.BankDate`** | `EntryLine.BankAllocation` â†’ `BankAllocation.cs line 39` `public DateOnly? BankDate { get; set; }` | it IS a line child | the human's reconcile tick | **ðŸ”´ CARRY â€” AND THIS ONE IS NOT IN `plan.md` AT ALL.** See Â§3.4. |
| `InventoryVoucher.OrderLinks` / `material_order_links` | `InventoryVoucher.cs line 87`; `Schema.cs line 1555` | `Guid` list / FK to `inventory_vouchers` | orderâ†”delivery linkage | **CARRY**; and **deleting an order orphans the link** â€” a VL-2 guard question, Â§6. |

## 3.4 ðŸ”´ THE DEFECT `Replace` WILL CREATE IF NOBODY READS THIS PARAGRAPH â€” the bank reconciliation date
...
the voucher entry screen**. A `Replace` implemented the obvious way â€” rehydrate the entry VM from the voucher,
let the operator edit, rebuild `EntryLine`s from the VM, swap them in â€” **silently destroys every bank
...
own test** (Â§7.3, T-6). It is the clearest example of the Â§3.0 rule: a voucher-attached fact that is not a
function of voucher content.

## 3.5 What Cancel and Delete need â€” much less, and it is worth saying why
...
`plan.md:267`), **referential** (the Â§34 and advance pickers that filter on base type only and would still
offer a cancelled invoice as the original supply â€” `plan.md` names `BuildSection34Pickers()` /
`BuildAdvancePickers()`), and **statutory** (an e-invoiced document â€” Â§6).

**Delete needs the Â§3.3 table read in the DELETE direction**: the voucher goes, and every row pointing at it by
...
refuse to delete a voucher that is the `OriginalInvoiceVoucherId` of a live Â§34 note, or that carries a
`Generated` e-invoice, or that is linked to a challan â€” **with the count of blockers**, which is what `plan.md`
already specifies and which Â§2.5's corpus rule (*"You cannot delete any ledger, if any transaction(s) has been
...
## 4.1 What the code does today â€” and it is worse than "leaves a gap"
...
|---|---|
| **Cancel** | Voucher stays in `_vouchers`, so it still counts toward `max`. **Number retained, sequence unbroken, no reuse.** âœ… Correct, and it is correct today by accident of the flag design rather than by a rule anybody wrote down. |
| **Delete a MID-sequence voucher** (say #7 of 1â€¦10) | `max` is still 10. Next post takes 11. **Permanent gap at 7.** |
| **ðŸ”´ Delete the HIGHEST-numbered voucher** (say #10 of 1â€¦10) | `max` drops to 9. **The next post REUSES 10.** Two different documents, at different times, with the same tax-invoice number â€” and the first one no longer exists to prove which was which. |
...
## 4.2 What the corpus says â€” nothing

**NOT SETTLED (C-5, Â§2.7).** No admissible corpus PDF discusses what happens to a deleted voucher's number.
The nearest thing is the Book's delivery-challan rule, PDF **p.81**: *"It must be serially numbered and number
does not exceed 16 characters. This can be in a single series or in multiple series."* â€” which speaks to the
form of a series, not to reuse after deletion. **Do not fill this gap by invention.**

## 4.3 What the project's own doctrine already says â€” and it contradicts the engine

This is the important half, because the doctrine is already **in the shipped code**, written during Phase 10.7.
`src/Apex.Desktop/ViewModels/VoucherNumberingConfigViewModel.cs line 399-409` â€” I opened it:

> *"True when a voucher carries a filed statutory document whose number is **legally frozen**
> (numbering-design-v2 Â§5.4). For e-invoicing the frozen signal is any status that REACHED the IRP: GENERATED
> (IRN issued) OR CANCELLED (the IRN was reported and is **permanently burned â€” a cancelled doc-no is never
> reusable**, Â§2.5)."*
...
1. **ðŸ”´ THE DOCTRINE AND THE ENGINE DISAGREE.** The doctrine says a filed document number is *permanently
   ...
   same slice.
2. **A NAMED SOURCE THAT IS NOT IN THE REPOSITORY.** `numbering-design-v2 Â§5.4` and `Â§2.5` are cited by
...
3. **A SMALL INCONSISTENCY WITH `plan.md`.** ORCHESTRATOR RULING 2 says *"Refuse alteration only when
   `EInvoiceStatus.Generated`, not `Pending`"* â€” silent on `Cancelled`. `IsFiledDocument` treats
...
## 4.4 Statute â€” what I will and will not claim
...
facts are **web-verified against official sources, never asserted from memory** â€” and no corpus PDF settles it.

**â‡’ USER/ORCHESTRATOR DECISION REQUIRED (R12), and it is cheap to state:** should a deleted voucher's number
be **retired** (never reissued) rather than reused? My recommendation and its reason are in Â§4.5.

## 4.5 Recommendation â€” the minimum change, and why it is minimal
...
- **(a) PREFERRED â€” refuse the delete, offer the cancel.** If the voucher carries a filed statutory document
  ...
  cancel, Â§2.2), it needs no new state, and cancelling **already** preserves the number by Â§4.1. It also
  matches Â§2.5's corpus rule in spirit: Tally refuses a destructive act rather than silently doing something
  lossy.
- **(b) FALLBACK, only if (a) is judged too strict** â€” teach `NextNumber` a floor. There is no place to store
  one without schema, so this would need v54 (Â§9). **Do not do this in the first pass.**
...
accepted behaviour in the census row (Â§10), not as a silent one.**

---

# Â§5 â€” AUDIT: the exclusion is confirmed, and here is what it costs

## 5.1 Confirmed â€” Edit Log / Tally Audit is excluded by standing user decision

Verified in `plan.md`, four independent places:

- `plan.md:39` â€” *"**Phase 11 and the REST of Phase 10 â€” TallyVault, Security Control / roles, Edit Log / Tally Audit, â€¦**"* in the exclusion banner.
- `plan.md:111-112` â€” the excluded-capability list: *"**Security & administration:** TallyVault, Security Control, user roles, password policy, **Edit Log / Tally Audit** (audit trail)."*
- `plan.md:1787` â€” Phase 10.11's own scope fence: *"it builds **NO audit trail, NO Edit Log, NO security roles, NO user attribution.**"*
- `plan.md:1872` USER DECISION 3 â€” *"**Alter and delete ship with NO audit trail**, by the earlier decision that excluded Phase 10."*
...
> â€” **writing half of it here would leave an audit log no one can query or protect.** Deferred to Phase 10
> deliberately; nothing in this file records history."*

`plan.md:3899-3900` additionally records that these are **two** deliverables, not one: *"**Edit Log** =
field-level before/after on every master/voucher; **Tally Audit** = the reviewer's audit-summary report. Build
both; don't conflate."*

**â‡’ Design the three verbs with no audit trail. Confirmed, not assumed.**

## 5.2 What that costs â€” stated plainly, because a gate has to acknowledge it

The domain has **no place to put history even if we wanted to**. Verified in Â§1.5: `Voucher` carries no
...
2. **A deleted voucher leaves nothing behind at all** â€” not a tombstone, not a gap marker, not a number
   reservation (Â§4). The only trace is a hole in the sequence, and Â§4.1 shows even that disappears when the
   deleted voucher was the highest-numbered one.
3. **A cancelled voucher is the ONLY one of the three that leaves evidence** â€” the row survives with
   `Cancelled = true`. **That is an argument for making Cancel the default gesture and Delete the exceptional
   one**, and it is a free argument: it costs nothing and it aligns with Â§4.5(a).
4. **There is no "who".** No user identity exists in the product at all (Security Control is excluded), so
   attribution is not merely unrecorded â€” it is unrecordable.
...
- **(a) Prefer Cancel over Delete in the UI.** Where both are offered, Cancel is the default and Delete is the
  one that has to be chosen. Free (Â§5.2 item 3), corpus-consistent (TallyPrime ships both verbs, Â§2.2), and it
  converts the commonest destructive act into an evidence-preserving one.
- **(b) Refuse Delete on a filed statutory document and offer Cancel instead** â€” Â§4.5(a). This is the
...
# Â§6 â€” THE SLICE SHAPE

## 6.1 The corrected starting point

**Two of the five planned slices are already merged** (Â§1.1). What remains is **S3 Cancel, S4 Delete, S5 Alter**.
`plan.md`'s slice list must be amended to say so, or the first implementer re-does work.

I recommend one further change: **split S5.** `plan.md` sizes it *"XL / HIGH â€” last and largest; the only slice
...
| # | Slice | Size / risk | What it proves â€” the one sentence a gate can check |
|---|---|---|---|
...
| **S5a** | **`LedgerService.Replace` â€” ENGINE ONLY, no UI** | M / **HIGH** | The three identities (Guid Â· Number Â· list position) survive, a **rejected** replacement leaves the original **byte-identical and at its index**, and an altered book equals a directly-posted book on **every** derived figure. |
| **S5b** | **`ForAlter` rehydration â€” simple families only** | L / med | A posted voucher re-opens **pre-filled** and re-accepts unchanged to a byte-identical book; every family that cannot yet round-trip is **refused with a named message**, never silently. |
...
## 6.3 S3 â€” Cancel. Contents, and the one trap that will bite
...
2. **ðŸ”´ RENAME, DO NOT DELETE, the abandon verb.** `plan.md` says *"delete `CancelVoucher()` rather than
   ...
   *abandon-the-entry-screen*, wired to **six** click handlers (`MainWindow.axaml.cs line 1118`, `:1153`, `:1274`,
   `:1284`, `:1300`, `:1310`). Rename it **`AbandonEntry()`** â€” the compile still breaks at every stale caller,
   ...
   `tests/Apex.Desktop.Tests/KeyboardArbitrationTests.cs` (one reference each â€” measured).
3. **New Alt+X arm**, narrowly gated: report-row context only, `!IsTyping`, `!IsPickerOpen`, no Ctrl.
4. **Confirmation** â€” our own string, recorded **UNVERIFIED-BY-DESIGN** (Â§2.7 C-3). Single prompt.
5. **`IsCancelled` on `ReportRow`, a `CancelledRowToBrushConverter`, the greyed Day Book row** (`plan.md:267`).
   Record the greying as **ours** â€” the corpus does not attest struck-through (Â§2.7 C-2).
6. **Print overprint "CANCELLED"** on the two Io print DTOs.
7. **Close the two picker leaks in the SAME slice** â€” `BuildSection34Pickers()` / `BuildAdvancePickers()` filter
   on base type only, so a cancelled invoice would be offered as the original supply a Â§34 note adjusts. These
   go live the moment Cancel is reachable.

**NOT in S3:** un-cancel (RULING 3); the pure-inventory Cancel **UI** (the engine method
`InventoryPostingService.Cancel` at `:128` exists â€” record that the deferral is UI-only, which `plan.md`
currently implies is an engine gap); anything about alteration.

## 6.4 S4 â€” Delete. Contents, and the two guards that are not optional

1. Y/N confirmation on the **one** confirmation channel, keeping `ConfirmMasterAccept` / `DismissMasterAccept`
   by name (called by tests and the dispatcher). **Single prompt**, per Â§2.3 â€” the corpus's double
   *"Delete Yes or No?" â†’ "Are you sure Yes or No?"* is attested for a **group company**, not a voucher.
2. A new **pure** `Services/MasterDeletionRules.cs` on the `MasterAlterationRules` shape (throws, never mutates).
3. **Referential guard, from Â§3.3, refusing with the COUNT of blockers** â€” a voucher that is the
...
4. **Master-side guard, corpus-mandated** â€” *"You cannot delete any ledger, if any transaction(s) has been
   already made with that ledger"* (STUDY-GUIDE PDF p.67, Â§2.5). Refuse with the count.
5. **ðŸ”´ The numbering guard (Â§4.5a)** â€” refuse Delete on a filed statutory document (`IsFiledDocument`,
   `VoucherNumberingConfigViewModel.cs line 404`) and **offer Cancel**. Without it, **this slice creates the
   number-reuse defect Â§4.1 describes**, because it is the slice that makes `Delete` reachable.
6. Routing from Day Book / register drill / voucher detail / Chart of Accounts / Stock Item list.

**NOT in S4:** **company deletion** (USER DECISION 2 â€” split out; `CompanyStorage.Delete` swallows `IOException`
and `Company = null` appears nowhere); the `SqliteCompanyStore.Remove` completeness fix (Â§1.6) â€” **decide
explicitly**: my recommendation is *leave it and add a `// DO NOT USE â€” incomplete` comment*, because fixing it
invites someone to route delete through it instead of through `Save`.

## 6.5 S5a â€” `Replace`, engine only. The contract
...
1. **Ordering: validate the replacement BEFORE removing the original.** `Post` mutates before it can fail
   ...
   is accepted"*. `Replace` must do the same in spirit: `EnsureValid` â†’ then swap **in place**.
2. **Preserve the `Guid`.** 25+ tables `REFERENCES vouchers(id)`, and `GstCreditDebitNoteLink.OriginalInvoiceVoucherId`
   / `EInvoiceRecord.SourceVoucherId` / `ChallanVoucherLink.VoucherId` are all Guid pointers (Â§3.3). **The Guid
...
4. **Preserve the list index.** `List.Remove` + `Add` moves the voucher to the end; Â§1.6 proves the index is
...
**Also in S5a: the `DerivedStateSnapshot` helper** (Â§7.2). It is a test artefact but it belongs to this slice
because S5a is the slice whose correctness it defines.

## 6.6 S5b / S5c â€” and the refusals
...
**refused with a named message** â€” including, temporarily, any voucher carrying `EntryLine.Gst`, `.Tds` or
...
**Â§3.4 bank-date carry-forward**, and the Â§3.3 CARRY/REFUSE table.
...
`Pending` e-invoice. **My addition (Â§3.3):** warn-and-proceed on an **active e-Way bill** too, because
`ConsignmentValuePaisa` is a frozen amount that will silently diverge.

## 6.7 ðŸ”´ WHAT I WOULD NOT BUILD IN THE FIRST SLICE â€” the explicit list

Un-cancel Â· any alteration at all (S3 is Cancel only) Â· any audit trail, Edit Log, role or user attribution Â·
company deletion Â· **Duplicate (`Alt+2`) and Insert (`Alt+I`)** â€” both corpus-attested (Â§2.6) and both out of
scope Â· the `SqliteCompanyStore.Remove` completeness fix Â· Basis of Values (Ctrl+B stays **reserved and
unbound**) Â· pure-inventory Cancel/Delete **UI** Â· a numbering floor / counter table (Â§4.5b) Â· any schema
change (Â§9).
...
## 7.1 The RED-PROOF â€” one test, runnable on the current tree, that FAILS

**`VoucherLifecycleRedProofTests.CorrectingAPostedVoucherLosesItsIdentity`** â€” project `Apex.Ledger.Tests`.
...
Book A (the only correction available today):
    post Sales #1 â€¦ #9, then Sales #10 with the WRONG total â‚¹1,84,733.45
    post Sales #11 (a later, unrelated invoice)
    Delete(#10)
    Post(a corrected Sales for â‚¹1,84,731.95)        // -â‚¹1.50, odd paise
Book B (the reference â€” the same book, keyed right the first time):
    post Sales #1 â€¦ #9, then Sales #10 for â‚¹1,84,731.95, then Sales #11

ASSERT: the corrected voucher's Number      == B's  â†’  FAILS today (it takes max+1 = 12)
ASSERT: the corrected voucher's index in company.Vouchers == B's â†’ FAILS today (it is last, not 10th)
ASSERT: DerivedStateSnapshot(A) == DerivedStateSnapshot(B)       â†’ FAILS today (both of the above leak into the Day Book and the register)
```

**A second red-proof, cheaper and even more direct â€” `DeletingTheHighestNumberedVoucherReusesItsNumber`:**
post Sales #1â€¦#10, `Delete` #10, post a new Sales, assert `Number == 11`. **It will fail, returning 10** â€”
proving Â§4.1's reuse defect on the current tree, before VL-2 makes it reachable from the keyboard.

Both are **kept green afterwards**, rewritten to use `Replace` / to assert the Â§4.5(a) refusal. Neither is
deleted â€” a red-proof that is deleted after it goes green proves nothing about the next regression.

## 7.2 ðŸ”´ THE EQUIVALENCE TEST â€” and the helper that makes it possible
...
A **canonical, ordered, paisa-exact text dump** of the entire derived surface â€” so a diff names the divergence
instead of a boolean hiding it. It must cover, in a fixed order:

1. Trial Balance â€” every ledger, signed closing (`Reports/TrialBalance.cs line 31`)
2. Balance Sheet â€” every group total **and** closing stock (`Reports/BalanceSheet.cs line 65`, `:133`)
3. Profit & Loss â€” every line (`Reports/ProfitAndLoss.cs line 71`)
4. Stock â€” **on-hand AND closing valuation for every item and every godown** (`InventoryLedger.cs line 135`, `StockValuationService.cs line 67`)
5. Batch on-hand and batch valuation (`BatchStockService.cs line 56`, `:135`)
6. Outstandings â€” every bill: reference, pending, **ageing bucket** (`Outstandings.cs line 95`, `:213`)
7. Cost â€” category summary, cost-centre breakup, ledger breakup (`CostReports.cs line 185`, `:208`, `:264`)
8. GSTR-1 â€” every section row (`Gstr1.cs line 217`); GSTR-3B â€” every box (`Gstr3b.cs line 130`)
9. Electronic cash/credit ledgers (`ElectronicLedgersView.cs line 50`)
10. Challan Reconciliation â€” every section's Deducted / Deposited / Remaining (`ChallanReconciliation.cs line 43`)
...
12. **The voucher identity vector** â€” for every voucher: `Id`, `Number`, rendered number via
    `Company.FormatVoucherNumber`, `Cancelled`, and **its index in `company.Vouchers`**

**`T-1 â€” AlterEqualsDirectPost`:** build A (post-wrong â†’ `Replace`) and B (post-right) from the **same
`PopulatedCompanyFixture` seed**; assert `Snapshot(A) == Snapshot(B)`; then **`Save` both to real SQLite,
`Load` both, and assert again**. The second half is not ceremony: Â§1.6 proves list order is persisted via
...
|---|---|---|
| **T-2** | **A rejected `Replace` leaves the original byte-identical and still at its index.** Feed an unbalanced replacement; catch `UnbalancedVoucherException`; assert `Snapshot(company)` is unchanged **string-for-string**. | `plan.md` driving test (1). Guards the Â§6.5(1) ordering clause â€” the one `Post`'s mutate-before-fail shape makes easy to get wrong. |
| **T-3** | **A TDS-carved purchase re-derives from the RESTORED GROSS**, at a carve rounding to odd paise. Gross â‚¹47,239.55 â†’ alter to â‚¹47,241.05 â†’ assert the carve, the net party credit and the `tds_lines` detail all match a directly-posted book to the paisa. | `plan.md` driving test (2). The Â§3.2 inversion trap. |
| **T-4** | **A Â§34 Credit Note altered with `ShowSection34Details` false KEEPS its `GstCreditDebitNoteLink` and its GSTR-1 9B row.** | `plan.md` driving test (3). The Â§3.3 CARRY column â€” the hidden-sub-form rule is inverted for vouchers. |
| **T-5** | **Alt+X on a â‚¹1,84,733.45 invoice: greyed in the Day Book, "CANCELLED" overprinted, number still in sequence, and every balance moves by exactly the invoice.** | `plan.md` driving test (4). |
| **ðŸ”´ T-6** | **Bank reconciliation date survives an alteration.** Post a bank Payment, `BankReconciliation.SetBankDate(...)`, then alter the **narration only**; assert `BankDate` is unchanged. Then alter the **amount**; assert `BankDate` is cleared **and a warning was raised**. | **Â§3.4 â€” this defect is not in `plan.md` and will ship silently without this test.** |
| **T-7** | **Delete is REFUSED on a filed statutory document, with Cancel offered**; and **`NextNumber` never returns a number already used by a filed document.** | Â§4.5(a). Prevents S4 from creating the Â§4.1 reuse defect. |
| **T-8** | **ER-13 byte-identity** â€” Â§8.3. | |
...
| **T-10** | **Delete's referential guard names the COUNT of blockers**, for each blocker class in Â§3.3. | |

## 7.4 ðŸ”´ FAMILIES BEYOND THE ACCOUNTING EIGHT â€” non-negotiable, and why

The prerequisite that made W0-7 a blocker is discharged (Â§1.8) **but the reasoning still binds**: this is the
...
**Every test above that can be family-parameterised must be** â€” `[Theory]` over the base kinds
...
- **Item-invoice Sales/Purchase** (`Voucher.InventoryLines`) â€” the accounts+stock atomic case
- **Stock Journal** â€” source/destination balance is still *enforced* (`InventoryPostingService.cs line 88-89`)
- **ðŸ”´ Physical Stock** â€” `InventoryLedger.cs line 193-207` makes a count a **checkpoint that RESETS the running
  ...
  just a `[Theory]` row.
- **Purchase Order / Sales Order** + **Delivery Note / Receipt Note** â€” `OrderFulfilment`, `InventoryVoucher.OrderLinks`
- **Job Work In/Out Order, Material In/Out** â€” `material_order_links` (`Schema.cs line 1555`)
- **Credit Note / Debit Note** â€” T-4
- **Memorandum, Optional, Post-dated, Reversing Journal** â€” all four are excluded by
...
- **POS and Payroll** â€” the **refusal** tests, T-9

## 7.5 Odd values â€” the standing rule, made concrete

A Â±â‚¹0.50 defect survived this project's life under round numbers, and `PopulatedFixtureCoverageTests` already
...
- invoice total **â‚¹1,84,733.45**, amended to **â‚¹1,84,731.95** (a âˆ’â‚¹1.50 delta that a rupee-rounded assertion would still see, and a âˆ’â‚¹0.50 variant that it would not â€” **include both**)
- TDS gross **â‚¹47,239.55** at a rate whose carve does not land on a whole paisa before rounding
- quantity **3.75** at rate **â‚¹1,010.33**; a second line at **12.125** units to exercise 6-dp quantity
...
## 8.1 What could break â€” ranked

1. **ðŸ”´ `Replace` renumbers a mid-sequence voucher.** `Post` assigns when `Number <= 0` (`LedgerService.cs line 51-52`).
   A `Replace` built by calling `Post` on a fresh `Voucher(Guid.NewGuid(), â€¦)` gets **both** the Guid and the
   Number wrong, and every Â§3.3 link breaks at once. *Mitigation:* Â§6.5 clauses 2â€“3; T-1 and T-2.
2. **ðŸ”´ `Replace` moves the voucher to the end of the Day Book.** `List.Remove` + `Add`. *Mitigation:* Â§6.5(4);
   T-1's **post-round-trip** half is the assertion that catches it.
3. **ðŸ”´ A rejected `Replace` destroys the original.** `Post` mutates at `:46` and `:51-52` before it can throw
   at `:48`. *Mitigation:* validate-then-swap; T-2 asserting byte-identity, not just "still present".
4. **ðŸ”´ Silent loss of `BankAllocation.BankDate`** â€” Â§3.4. No existing test would notice. *Mitigation:* T-6.
5. **ðŸ”´ S4 creates the number-reuse defect** â€” Â§4.1. It exists in the engine today but is unreachable; VL-2 is
   what reaches it. *Mitigation:* the Â§4.5(a) guard **in the same slice**; T-7.
6. **Copying old tax lines forward instead of re-deriving** â€” Â§3.2. *Mitigation:* T-3, plus S5b's temporary
   blanket refusal of GST/TDS/TCS-carrying vouchers so this cannot ship half-done.
7. **Someone routes delete through `SqliteCompanyStore.Remove`** (Â§1.6), which does not delete `tds_lines`,
   `tcs_lines`, `payroll_lines`, `voucher_inventory_lines` or `pos_tender_allocations`. *Mitigation:* Â§6.4's
   explicit decision + an in-source `DO NOT USE` note.
8. **The live-list cast** â€” `((List<Voucher>)company.Vouchers)` compiles (Â§1.2). *Mitigation:* a review grep.
9. **Deleting `CancelVoucher()` literally, destroying the abandon-entry behaviour** on six buttons. *Mitigation:*
   Â§6.3(2) â€” rename to `AbandonEntry()`.
10. **Test runtime.** `TotalClosingStockValue` (`StockValuationService.cs line 98-104`) loops every item, each
    ...
    snapshots per test Ã— a family `[Theory]` could dominate the Ledger suite. *Mitigation:* build the snapshot
    once per book, not once per assertion; if it bites, gate the stock section behind a flag for the
    accounts-only families â€” **but never for the item-invoice or Physical Stock tests.**

## 8.2 What currently passes that would start failing

Measured, not estimated:

- **`CancelVoucher` is referenced in exactly two test files** â€” `tests/Apex.Desktop.Tests/InventoryVoucherEntryViewModelTests.cs`
...
- **Nothing in `Apex.Ledger.Io` or `Apex.Persistence.Sqlite` should move at all.** A moved Io or Sqlite count is
  a **red flag**, not a pass â€” see Â§8.3.

**Per-project gate discipline (Â§6.2 of `plan.md`): predict all four counts before each merge and treat an exact
match as evidence the merge is semantically clean.** Baseline at HEAD: **Ledger 1668 Â· Io 414 Â· Sqlite 231 Â·
Desktop 2195**. âš ï¸ `plan.md`'s Phase 10.11 exit gate still quotes the **stale** baseline
*"Ledger 1294 Â· Io 368 Â· Sqlite 214 Â· Desktop 1836"* â€” **correct it before the first merge** or the gate
compares against a figure four phases old.

## 8.3 ER-13 â€” a book that never uses these verbs must be byte-identical

**This is satisfied by construction, and here is the proof rather than the assertion:**

- **No schema change** (Â§9). `vouchers.cancelled` already exists (`Schema.cs line 941-961`) and already round-trips.
...
- **Persistence is a pure function of the in-memory graph** â€” `SqliteCompanyStore.Save` (`:1790`) is
  delete-all + full re-insert. A company whose voucher list is untouched serialises identically.

**T-8, the test that turns that into evidence â€” three assertions:**
...
2. Round-trip the same company through `Apex.Ledger.Io` canonical XML and assert byte-identity â€” `plan.md` is
...
- `Schema.CurrentVersion = 51` â€” verified at `src/Apex.Persistence.Sqlite/Schema.cs line 159`.
...
- `SqliteCompanyStore.Save` (`:1790`) re-inserts the whole aggregate in one transaction, so persistence is a
  **pure function of the in-memory `Company` graph** â€” nothing to migrate, nothing to back-fill.
- **Io: none for the canonical model** â€” but that is an **assertion to be tested** (Â§8.3 T-8), not assumed.

**The version-allocation position, stated plainly as required:**

- `plan.md:1636-1638` â€” the binding allocation is **WF-1 = v51, WF-2 = v52, WF-3 = v53**, and
  **ðŸ”´ "THE ALLOCATION ENDS AT v53. NOTHING IS RESERVED BEYOND IT."**
- **v54 is contested and reserved for nobody.** Two rows were each promised it by different sentences â€” WF-8's
...
- **â‡’ Phase 10.11 takes NO version and reserves none.** The only design choice in this document that could
  require one is Â§4.5's **fallback (b)** â€” a stored numbering floor. **Recommendation (a) exists precisely so
  ...
  free number, and **amend `plan.md`'s allocation line in the same commit** â€” with columns byte-identical in
...
# Â§10 â€” RULING 5: the fidelity rows owed to `docs/full-clone-census.md` Â§1.3

RULING 5 (R12, 2026-08-16) requires **every slice** to end with a fidelity row in `docs/full-clone-census.md`
Â§1.3, or a record of why the corpus cannot settle the question. Â§1.3 is a numbered list of capabilities with
...
**"What IS sourced â€” each with the page it comes from, and nothing else claimed"** vs **"What is OURS, or
unsettled â€” separated deliberately."**

**Owed rows, one per slice.** Drafted here so the implementer copies rather than composes.

### Item 10 â€” Voucher cancellation (S3)
- **SOURCED:** `Alt+X` = *"To cancel a voucher / To cancel a voucher from a report"*, scope *"Vouchers &
  Reports"* â€” Book PDF **p.437** [printed p.433], **re-extracted with `-raw`** because `-layout` scrambles this
  table (Â§0.2).
- **OURS / UNSETTLED â€” and this is most of the row:** *what cancellation MEANS* is **not in the corpus at all**
  â€” `grep -oic cancel` over all nine admissible PDFs returns **2 hits**, one of them an EPF cancelled cheque
...
### Item 11 â€” Voucher and master deletion (S4)
- **SOURCED:** `Alt+D` = *"To delete an entry from a report"* â€” Book PDF **p.435** [printed p.431]. The
  per-family register recipe *"For Delete Entry Press `Alt+D' on Selected Entry"* â€” Book PDF **pp.32, 34, 37,
  42, 47, 49, 64, 71** and the inventory families. Master deletion is `Alt+D` from the **Alter** screen â€”
  ...
  has been already made with that ledger."* Multi-master screens offer alter but **not** delete â€” Book PDF
  **pp.104-105**.
- **OURS / UNSETTLED:** the **single** prompt for a *voucher* â€” the double prompt is attested for a **group
  ...
  gap otherwise) is **ours** â€” and our **refusal to delete a filed statutory document, offering Cancel
  instead**, is ours, taken from the project's own numbering doctrine at
  `VoucherNumberingConfigViewModel.cs line 399-409` (whose cited source `numbering-design-v2 Â§2.5/Â§5.4` **is not in
  the repository**). Company deletion is out of scope.

### Item 12 â€” Voucher alteration (S5a/b/c)
- **SOURCED:** the register drill-down **is** the alteration screen â€” *"How to Show/Edit \<X> Voucher Entry in
  Tally Prime? â€¦ Select Month & **Show/Edit Entry**"*, Book PDF **pp.32, 34, 37, 42, 47, 49, 64, 71** and the
  inventory families; saved with **`Ctrl+A`** (Book PDF **pp.51, 53, 56, 58**). **TallyPrime has no separate
  read-only voucher screen** â€” one action is named, not two. `Ctrl+Enter` = *"To alter a master during voucher
  entry or from drilldown of a report"*, Book PDF **p.436** [printed p.432]. `Ctrl+D` removes a **line** inside
  voucher entry (same page) â€” a different granularity from `Alt+D`.
- **OURS / UNSETTLED:** **our key bindings are a deliberate divergence** â€” plain Enter keeps a read-only
  VoucherDetail column and **`Ctrl+Enter` opens voucher alteration**, to preserve the Miller-column cascade
  (USER DECISION 1, with a follow-up to reconsider). **ðŸ”´ The `plan.md` R7 line that Tally *"reserves
  `Ctrl+Enter` for display-only drill-down"* is WRONG and must be amended** â€” the corpus says Ctrl+Enter is an
  *alteration* key for a **master**; our extension of it to **vouchers** is a **smaller** divergence than the
  ...
  p.435) and NOT BUILT** â€” named carry-forward, not a silent omission.

### Item 13 â€” the method note (not a capability row; propose it as a Â§0.2-style footnote to Â§1.3)
**`pdftotext -layout` scrambles the Book's three-column shortcut tables on pp.436-437** (key count â‰  function
...
# APPENDIX â€” the decisions this design asks the orchestrator/user to take (R12)

| # | Decision | My recommendation |
|---|---|---|
| D-1 | `plan.md` Phase 10.11 says five slices; **S1 and S2 are already merged** (`6a28d15`, `f2abdbb`). Amend the row? | **Yes â€” before any work starts.** Two false sentences are named in Â§1.1. |
| D-2 | Split S5 into **S5a (engine) / S5b (rehydration, simple families) / S5c (carves + carry)**? | **Yes.** Â§6.1. |
| D-3 | **Should a deleted voucher's number be retired?** | **Refuse Delete on a filed statutory document and offer Cancel** (Â§4.5a). No schema, no counter, corpus-shaped. Do **not** build a numbering floor. |
| D-4 | `plan.md` R7 claims Tally reserves `Ctrl+Enter` for **display-only** drill-down. **The corpus says it is an ALTER key.** | **Amend the R7 line.** USER DECISION 1 stands; its stated reason was half wrong (Â§2.4). |
| D-5 | `plan.md` R7 says Tally **scopes Alt+X to cancelling from a report**. The corpus scope is **"Vouchers & Reports"**. | Ship report-only if we choose, but **record it as our scope decision**, not fidelity (Â§2.2). |
| D-6 | Voucher delete confirmation: **one** prompt or **two**? | **One.** The double prompt is attested only for a group company / masters (Â§2.3). |
| D-7 | `SqliteCompanyStore.Remove` is **incomplete** (Â§1.6) and off the live path. Fix, or fence? | **Fence** â€” add a `DO NOT USE â€” incomplete` note. Fixing it invites routing delete through it. |
| D-8 | `plan.md`'s 10.11 exit gate quotes the **stale** baseline *Ledger 1294 Â· Io 368 Â· Sqlite 214 Â· Desktop 1836*. | **Correct to Ledger 1668 Â· Io 414 Â· Sqlite 231 Â· Desktop 2195** before the first merge (Â§8.2). |
| D-9 | `numbering-design-v2 Â§2.5/Â§5.4` is cited by shipped code and **is not in the repository**. | Land it, or restate its rule in-repo. Owed to the post-merge documentation slice (Â§4.3). |
| D-10 | `plan.md` implies the **pure-inventory Cancel** deferral is an engine gap; `InventoryPostingService.Cancel` (`:128`) **exists**. | Re-word: the deferral is **UI-only** (Â§6.3). |

---

# Â§11 â€” LATE FINDINGS (written last; they correct two things above and one thing in `plan.md`)

## 11.1 ðŸ”´ THE "CANCELLED VOUCHER KEEPS ITS NUMBER AND IS GREYED" CLAIM IS **MODEL-KNOWLEDGE**, AND THE PROJECT'S OWN VERIFICATION REPORT SAYS SO
...
> can gap numbering (**verification Â§A14**)."*

Chasing "verification Â§A14": `docs/tally-feature-catalog-verification-report.md` has **no section `A14`**. The
...
1. **The belief that a cancelled voucher keeps its number and appears greyed/struck through is asserted from a
   model's memory. It has never been sourced.** My independent corpus sweep (Â§2.7, C-1/C-2) found the corpus
   silent â€” `grep -oic cancel` over all nine admissible PDFs returns **2 hits**, one an EPF cancelled cheque.
   **Two independent lines of evidence now agree that this is unsourced.**
2. **It has been propagating as if sourced.** `plan.md:320` cites it as *"(verification Â§A14)"* â€” a citation
...
3. **This is precisely what R7 forbids** (*"never assert a TallyPrime behaviour from memory"*). The design
   above is unaffected in substance â€” Â§2.7 already refused to treat it as settled and Â§4.1 shows our engine
   implements retain-the-number for its own good reasons â€” but **every place that cites it must be relabelled.**

**Actions (owed to the post-merge documentation slice, R5/R6):**

- **`plan.md:320`** â€” replace *"(verification Â§A14)"* with an honest tag: the numbering/greying behaviour is
  **UNVERIFIED â€” model-knowledge, flagged for spot-check by the verification report itself (line 68, line 177
  item 5); corpus silent (2 hits, neither relevant)**. Our implementation is **ours**, chosen on the merits.
- **Census Â§1.3 item 10** (Â§10 above) â€” add this provenance to the "OURS / UNSETTLED" column. It strengthens
...
- **A general sweep is owed**: line 177 names **five more** model-knowledge claims travelling under the same
  flag â€” the single-entry-mode F12 toggle path, Payroll/Job-Work-requires-F11 availability, Bank Allocation vs
  Stat-Payment challan split, Stock-in-Hand derived balance, and rename-in-place semantics. **Anything in
  `plan.md` citing "verification Â§Ann" should be checked for the same defect.** Out of scope here; named so it
  is not lost.

**One genuinely sourced fact found in the same file, worth keeping** â€” `docs/tally-feature-catalog-verification-report.md:162`
records *"Online e-Invoice & e-Way Bill (Rel 1.1 / 2.0; **cancel-on-alteration 7.0**)"* with an official source
(`help.tallysolutions.com` release notes). That supports Â§3.3 / Â§6.6's e-invoice interlock being a real
TallyPrime concern, though it does not settle our behaviour.

## 11.2 CORRECTION TO Â§6.3 AND Â§10 â€” `plan.md:267` IS A STALE POINTER

`plan.md`'s VL-3 bullet says the greyed Day Book row is *"`plan.md:267` already specifies"*. **Line 267 is the
tech-stack comparison section** (*".NET (C#) + Avalonia/WPF + SQLiteâ€¦"*). The real specification is
**`plan.md:320`**, quoted in Â§11.1.

I repeated the stale pointer in Â§6.3 item 5 and in Â§10 item 10 because I quoted `plan.md`. **Read `plan.md:320`,
not `:267`** â€” and note that what it "specifies" turns out to be model-knowledge (Â§11.1). Add the pointer fix to
the documentation slice.

## 11.3 ðŸ”´ OPERATIONAL â€” THE WORKING TREE IS NOT CLEAN, AND ANOTHER AGENT IS WRITING A PHASE 10.11 DESIGN INTO THE REPO

The brief stated the checkout was **CLEAN**. At the end of my run `git status --porcelain` in
`â€¦\worktrees\recursing-swirles-3138c6` reports:
...
- **`docs/design-records/phase-10-11-voucher-lifecycle-design-PARTIAL.md`** â€” 33 082 bytes, mtime **09:17
  today**, i.e. **written during my run**. ðŸ”´ **Somebody else is producing a Phase 10.11 design at the same
  time as this one.**
- `docs/design-records/w0-2b-company-screen-design.md` (92 279 B) and `w0-7-fixture-audit.md` (45 069 B), same mtime.
- `tests/Apex.Ledger.Tests/DocumentCodeAgreementTests.cs` â€” **+9 lines**, adding
...
**I did not read, merge, or touch the other design file** â€” deliberately. Absorbing an unverified concurrent
draft would defeat the point of independent grounding, and editing another agent's in-flight file in a shared
worktree is the two-agents-one-worktree failure this project has already been bitten by.

**For the main loop, three things follow:**

1. **Reconcile the two Phase 10.11 designs before either is executed.** Mine is at the scratchpad path in this
   document's header; the other is in-repo at `docs/design-records/phase-10-11-voucher-lifecycle-design-PARTIAL.md`.
   Where they disagree, **the file:line and PDF-page citations in this document are the ones I opened myself**
   and can be re-checked in minutes.
2. **The gate figures in my Â§8.2 assume HEAD `3e968b3` with a clean tree.** With `DocumentCodeAgreementTests.cs`
...
3. **A new top-level directory `docs/design-records/` is being introduced**, and a test allow-list has been
   amended to accommodate it. That is a repo-convention change nobody has recorded in `plan.md` â€” worth a
