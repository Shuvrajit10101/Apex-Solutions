...
# W0-2b â€” Company Create/Alter screen: DESIGN

**Slice:** W0-2b (census S2 / T1-6; the WRITE half of T0-8, the CGST Rule 46(a) blank supplier address).
**Baseline:** worktree `â€¦\.claude\worktrees\recursing-swirles-3138c6`, branch `claude/apex-wrong-figures-bc45f4`,
...
**Governing ruling:** R12, 2026-08-15 â€” **INHERIT** (`plan.md`, W0-2b block, RULING 3).
**Prior art that must not be re-derived:** `docs/w0-2-company-screen-grounding.md` (the R7 corpus pass).

---

## 1. GROUND TRUTH â€” every inherited claim, re-verified at `3a4fcdb`

Verdict up front: **the brief's five traps all hold. Two claims in the grounding doc have drifted, and â€” the
find that changes the design â€” one mechanism the ruling appears to require is a silent data-loss trap in the
store as it stands today.** Details below; the trap is Â§1.9 and it drives Â§3.

### 1.1 âœ… HOLDS â€” `Company.EnsureValid()` has exactly one call site in `src/`, and it is the canonical import
...
| **`src/Apex.Ledger.Io/ImportPlan.cs line 1203`** | **`Company` â€” the only one** |
...
(`cmd.Parameters.AddWithValue("$pin", (object?)c.Pin ?? DBNull.Value)`). **This slice is what breaks it** â€” it is
the first UI that assigns `Company.Pin`. See Â§5.

### 1.2 âœ… HOLDS â€” `MasterGstDetails.EnsureValid` is reachable on 1 of 5 write paths
...
on exactly one of five write paths â€” the canonical import"*, and it is listed again under **"What this review
did NOT close"**. The same file's `:71` names *"the `Company.EnsureValid` limit recorded for W0-2a"* as the
sibling defect. **W0-2b must not add a sixth unguarded path** â€” see Â§5.

### 1.3 âœ… HOLDS â€” the create screen captures exactly one field, and there is no Alter screen

- `src/Apex.Desktop/Views/MainWindow.axaml line 228-244` is the whole create form: one `TextBox` bound to
  `NewCompanyName` (`:236`), a **static** hint `"Financial year begins 1-Apr; base currency â‚¹ INR."` (`:238-239`),
  and a `Button` `"Create (Ctrl+A)"` â†’ `OnCreateCompanyClick` (`:240-242`). The `Border` is gated on
  `ConverterParameter=CreateCompany` (`:230`).
- `src/Apex.Desktop/ViewModels/MainWindowViewModel.cs line 815-823` `ShowCreateCompany()` â€” sets
  ...
  `BuildButtonBar()`.
- `src/Apex.Desktop/ViewModels/MainWindowViewModel.cs line 827-839` `CreateCompany()` â€” trims, refuses blank
...
- The entry point is a menu row, not a cascade: `MainWindowViewModel.cs line 808`
  ...
  screen builder (`:795` sets `ScreenTitle = "Company Info â€” Select Company"`).
- **`grep -rn "AlterCompany" src/ --include=*.cs --include=*.axaml` returns ZERO hits.** The `Screen` enum
  ...
  `RestoreCompany`, `GstConfig`, â€¦ and **no `AlterCompany`**.

### 1.4 âœ… HOLDS â€” `Company.State` has no assignment site in `src/Apex.Desktop`, and no consistency guard exists

The only writers of `Company.State` anywhere are the canonical import (`ImportPlan.cs line 1198`) and the journal
snapshot/rollback pair. The store round-trips it â€” write `SqliteCompanyStore.cs line 4693`
...
### 1.5 âœ… HOLDS â€” the 11 profile fields exist in the domain and in the schema; no migration for them

`src/Apex.Ledger/Domain/Company.cs` â€” `:59` `Id`, `:62` `Name`, `:65` `MailingName`, `:67` `Address`,
`:76` `Country = "India"`, `:85` `State`, `:89` `Pin`, `:104` `FinancialYearStart`, `:107` `BooksBeginFrom`,
`:109` `BaseCurrencySymbol = "â‚¹"`, `:110` `BaseCurrencyName = "INR"`, `:111` `DecimalPlaces = 2`,
`:112` `DecimalUnitName = "Paisa"`.

> âš ï¸ **Line drift against the grounding doc Â§7.1** (which was written at `fa651ae` and re-anchored at
...
> The **claim** stands exactly â€” eleven profile fields excluding `Name` â€” only the numbers moved (W0-2a added
> the `Country`/`State`/`Pin` doc comments and `EnsureValid`). Do not copy Â§7.1's numbers into anything new.

`src/Apex.Persistence.Sqlite/Schema.cs line 179-194` â€” the `companies` DDL opens at `:179` and carries `name`,
...
### 1.6 âœ… HOLDS â€” Save is a delete-and-reinsert full rewrite
...
> âš ï¸ **Line drift against grounding Â§7.6**, which gives `:4489` DELETE / `:4492` InsertCompany / `:4497` INSERT /
...
### 1.7 âœ… HOLDS â€” the print path never reads `Company.State`
...
to `PostalAddressText` (`:822-831`), which emits Address â†’ Country â†’ `"PIN: " + pin`, each skipped when blank.
`StateText(string? code)` (`:846-850`) resolves through `IndianState.FromCode` and returns `""` for an
unrecognised code. `SplitAddress` (`:855-866`) splits on **newlines only** â€” `"Pune, Maharashtra 411001"` is
ONE line, deliberately.

### 1.8 ðŸ”´ DRIFTED CLAIMS FOUND â€” two shipped code comments still call the R12 gate OPEN
...
1. **`src/Apex.Ledger/Domain/Company.cs line 79-84`** â€” the `State` doc comment ends: *"Its relationship to
   `GstConfig.HomeStateCode` is an **open R12 user gate** â€” see `plan.md` W0-2b."* **It is RESOLVED (INHERIT,
   2026-08-15).** The same comment's *"No print path reads this"* is still correct.
2. **`src/Apex.Desktop/Services/VoucherPrintProjector.cs line 698-707`** â€” `SellerBlock`'s doc comment: *"The
   capture question â€” expose both / suppress the postal one / wire one to the other â€” **remains an open R12 user
   gate** (`plan.md`, W0-2b)"*. Also resolved. The rest of that paragraph â€” that this method is independent of
   the answer, and that `A_company_whose_postal_State_disagrees_with_its_GST_State_prints_the_GST_one` pins it â€”
...
2026-08-16 and the W0-2b block is at `plan.md:1830-1940` at `3a4fcdb` (row heading at `:1830`, RULING 3 at
`:1910`), consistent with what the doc claims.

### 1.9 ðŸ”´ðŸ”´ NEW, MEASURED, AND IT CHANGES THE DESIGN â€” the store *drops* a GST home State on a GST-off company

This is the single most important finding of this pass, and nothing in the grounding doc or `plan.md` records
it. The obvious reading of RULING 3 â€” *"the GST home State DEFAULTS FROM the postal State **at creation**"* â€”
...
- **Write** (`SqliteCompanyStore.cs line 4704-4709`): `$gsten` is `gst is { Enabled: true } ? 1 : 0`, but
  `$gsthome` is `(object?)gst?.HomeStateCode ?? DBNull.Value` â€” **the home state is written whenever a
  `GstConfig` object exists, regardless of `Enabled`.**
- **Read** (`SqliteCompanyStore.cs line 1348-1350`): `if (r.GetInt64(16) != 0) { company.Gst = new GstConfig { â€¦ } }`
  â€” the entire config, `HomeStateCode` included, is reconstructed **only when `gst_enabled = 1`**. With GST off,
  `company.Gst` comes back **null** and the persisted `gst_home_state` is unreachable.

So a creation-time stamp writes a value that the very next `Load` throws away â€” and worse, it lingers in the
...
is enabled; the delete-and-reinsert then fabricates â€¦ over the migration's `UPDATE`."* Same root cause, same
...
**seed-on-demand at the moment the GST screen is opened / GST is enabled** â€” which is, independently, *exactly
what the corpus describes*: TallyPrime's GST Details State *"by default **shows** the State name as selected in
the Company Creation screen"* (Book PDF p.177, grounding Â§4.1). The code constraint and the corpus converge on
the same mechanic. Â§3 builds it that way.

### 1.10 âœ… HOLDS â€” the schema numbers, re-read rather than quoted

`src/Apex.Persistence.Sqlite/Schema.cs line 159` `public const int CurrentVersion = 51;` (doc comment at `:119`
agrees). `plan.md:1495` carries the binding allocation *"Schema (v50 â†’ v53) â€” binding allocation â€¦ WF-1 = v51,
WF-2 = v52, WF-3 = v53"*, and `plan.md:1841-1846` records the 2026-08-16 correction: **nothing is reserved
beyond v53**; whoever needs v54 takes it **and amends the allocation line in the same commit**; expected â€” not
binding â€” outcome `W0-2b = v54`. Â§2 decides whether W0-2b needs a number at all.

### 1.11 âœ… HOLDS â€” the bridge from a postal State name to a GST state code already exists

`src/Apex.Ledger/Domain/IndianState.cs line 107-109` `FromName(string? name)` â€” case-insensitive
(`StringComparison.OrdinalIgnoreCase`) lookup over `All` (`:52-89`: codes 01â€“38 plus 97 "Other Territory";
**neither 96 nor 99**, deliberately â€” `:35-49`). `FromCode` at `:103-104`, `IsValidCode` at `:100`. So the
INHERIT mapping `Company.State` â†’ `GstConfig.HomeStateCode` is a one-liner **provided** the postal State is
constrained to that same list â€” which the corpus independently requires (grounding Â§2 field 6: *"State (from a
list)"*).

### 1.12 âœ… HOLDS â€” the GST screen already has a second, competing seed for the same field
...
GSTIN-derived fill and the new postal-State seed** â€” Â§3 does.

### 1.13 âœ… HOLDS â€” `CompanyFactory` already accepts the dates the UI never passes
...
the default is 1-Apr of the **current calendar year**, so a company created in Januaryâ€“March is stamped an
FY-start three months in the **future** relative to the live Indian FY.

### 1.14 âš ï¸ A claim in the brief that needs narrowing
...
a company where the user types **State and PIN but no Address** prints **nothing at all** â€” not even the PIN.
That is correct ER-13 behaviour and must not be "fixed" here, but it is a behaviour the new screen makes newly
reachable, and Â§6 tests it.

---

## 2. THE SCHEMA QUESTION â€” decision: **W0-2b ships ZERO schema change and takes NO version number**

`plan.md`'s W0-2b row instructs: *"**check first whether W0-2b needs a migration at all** â€” the row's own
...
Â§1.5 and Â§1.6 above. All fourteen `companies` columns behind the eleven profile fields are in the **v1 create
DDL** (`Schema.cs line 179-194`), and Save is delete-and-reinsert (`SqliteCompanyStore.cs line 4602` â†’ `:4628`), so an
Alter path is `company.X = â€¦; _storage.Save(company);` and nothing else. Read path `:1306`, mapped `:1318-1341`.
...
RULING 3 is explicitly **additive â€” "BOTH COLUMNS ARE KEPT. NO DESTRUCTIVE MIGRATION"** (`plan.md:1924-1925`).
...
### 2.3 The five Tally contact fields â€” **OUT of this slice**, with reasons

Telephone No Â· E-Mail Â· Mobile No Â· Fax No Â· Website. **Verified absent at `3a4fcdb`:**
...
**(a) ðŸ”´ The corpus cannot tell us what order to put them in â€” so shipping them means inventing.**
This is the decisive reason, and it is an R7 reason rather than an effort one. Grounding Â§2.1 and Â§9 item 9
record that the two PRIMARY sources give **different** contact-block orders (Study Guide p.59 prose:
Telephone â†’ E-Mail â†’ Mobile â†’ Fax â†’ Website; Book p.13: Telephone â†’ Mobile â†’ Fax â†’ E-Mail â†’ Website) â€” **and the
...
fidelity. The grounding doc's Â§9 list exists precisely to stop that.

**(b) They are FIDELITY, not COMPLIANCE.** CGST Rule 46(a), quoted first-party from CBIC in grounding Â§5.4, is
*"name, address and Goods and Services Tax Identification Number of the supplier"*. Grounding Â§5.5 states the
scoping fact outright: *"Pin Code, Telephone, Mobile, Fax, E-Mail and Website are Tally-fidelity fields, not
compliance fields. A design that trims scope should trim there."* This slice's reason for existing is T0-8 â€” a
...
`SchemaDowngrade.V51ToV50` *"is not the true inverse it is documented as"* â€” a populated round trip loses
...
**(d) Taking no version number leaves the v54 allocation untouched.** `plan.md:1495-1512` ends the binding
allocation at **v53** and makes v54 a first-come claim that **"MUST amend this line in the same commit"**, with
`plan.md:1506` naming the two claimants: W0-2b and WF-8's fallback closure flag. By shipping schema-clean,
W0-2b **does not consume v54 and does not have to amend the allocation line** â€” removing a coordination hazard
...
Recommendation for `plan.md`: a **W0-2c** row â€” "Company contact block (Telephone, E-Mail, Mobile, Fax,
Website)" â€” carrying **two** explicit preconditions: **(i)** an R7/A14 pass that either settles the field order
from the corpus or records it as UNVERIFIED-and-chosen with the choice argued (grounding Â§9 item 9 is the open
...
The Alter screen will not be able to edit a company's phone number or e-mail â€” the very three things the Book
gives as Alter's *purpose* (grounding Â§3, Book p.15: companies *"will alter or edit their information when they
have changed company **address** or **contact number** or **email**"*). **One of those three â€” address â€” is the
statutory one and ships here; the other two do not.** That is a real fidelity gap for a screen whose corpus
rationale names them, and it is recorded in Â§7 and Â§8 rather than glossed.

---

## 3. THE INHERIT MECHANICS â€” concretely, with named call sites

RULING 3 in four clauses (`plan.md:1912-1925`): postal `Company.State` is the **source of truth**; the GST home
State **defaults from it at creation** and **stays editable**; a guard **warns** on divergence; **both columns
kept**. Below is how each clause becomes code, and where clause 2 has to be read carefully to avoid Â§1.9.
...
hosting the profile fields and used by **both** Create and Alter. One view model, two modes â€” because Create and
Alter differ in exactly two ways per the corpus (grounding Â§3: Alter adds only `Alt+D`; Create adds only
...
**The State control is a picker, not a free-text box.** Corpus: grounding Â§2 field 6 â€” *"State (from a list)"*.
...
`GstConfig.HomeStateCode` is the 2-digit code â€” `"27"`, asserted at
...
**ðŸ”´ ER-13 requirement â€” the picker must tolerate a value it does not recognise.** Canonical import assigns
...
"correct" a stored State** â€” that is a silent data change on a field the canonical round-trip asserts
...
is **not built** â€” `plan.md:672-676` records that S5 shipped type-to-**JUMP** and that *"No filtering
infrastructure exists anywhere in `src/`"*, and `plan.md:712-715` puts KB-3 behind a measurement spike. **Do not
invent it here.** The picker inherits whatever the shared widget already does, and nothing more.

### 3.2 How the GST home State seeds from it â€” NOT at creation; at the GST screen

> ðŸ”´ **This is the clause that has to be read against the store.** Taking *"defaults from it at creation"*
> literally â€” building a `GstConfig` in `CreateCompany()` and stamping `HomeStateCode` â€” **writes a value the
> next `Load` discards** (Â§1.9: the write at `SqliteCompanyStore.cs line 4706` is unconditional on `Enabled`; the
...
**The seed is a DISPLAY default at the moment the GST screen is populated** â€” which is precisely what the corpus
says: the GST Details State *"by default **shows** the State name as selected in the Company Creation screen"*
(`664311548-Tally-Prime-Book.pdf` PDF p.177, grounding Â§4.1). "Shows", not "stores".

**Exact site: `src/Apex.Desktop/ViewModels/GstConfigViewModel.cs`, immediately after `:563`.** Today `Load` runs
`Gstin = cfg?.Gstin ?? string.Empty;` (`:562`) â€” which fires `OnGstinChanged` (`:607-616`) and may set
`HomeState` from the GSTIN's leading two digits â€” then `:563`
...
when no config is stored**. The seed is one guarded statement appended after that line, in spirit:
    ...
    // State, exactly as the corpus describes. A DISPLAY default only â€” nothing is persisted until the user
...
**Precedence ladder, highest first â€” every rung but the last is an explicit user act:**
...
| 3 | The **postal `Company.State`** (new) | The corpus default â€” applies only when 1 and 2 are absent. |
| 4 | `null` â†’ the screen refuses to enable (`:1586-1591`) | Unchanged. |
...
Â§1.9 trap is avoided by construction rather than worked around.

### 3.3 What happens when a user later edits either

| Action | Effect on the other | Rationale |
|---|---|---|
| Postal State edited on **Alter**, GST **off** | none â€” there is no `GstConfig` to diverge from | The next GST enable seeds from the new value (3.2, rung 3). |
...
| Home State edited on the **GST screen**, away from the postal State | postal `Company.State` untouched | RULING 3 clause 2: the GST State *"stays EDITABLE for the rare genuine divergence â€” a registration in a State other than the postal one."* |
...
### 3.4 The consistency guard â€” what it says and where it appears

**Shape:** a computed, read-only advisory string on the view model, rendered as a normal text line whenever it is
non-empty. **A warning, never a refusal** â€” RULING 3 clause 3: *"A warning, not a refusal: divergence is legal,
silence about it is not."* It never gates Accept, never throws, and never mutates.

**Where â€” two places, because either screen can create the divergence:**

1. **`CompanyProfileViewModel`** (Alter and Create) â€” visible while the divergence exists, so it is present the
   moment an Alter screen opens on an already-divergent book, not only after an edit.
2. **`GstConfigViewModel`** â€” the same computation, so moving the Home State away from the postal State warns
...
**Computation** â€” one shared helper, so the two screens cannot drift (a small `CompanyStateConsistency` static
...
- **GST off** â†’ silent. There is no second State.
- **Postal State blank** â†’ silent. Nothing was claimed.
- **Postal State non-blank but unrecognised by `IndianState.FromName`** â†’ **its own, different message**, not
  the divergence one. Comparing an unresolvable name against a code and declaring "they differ" would report a
  divergence that may not exist. This case is reachable today from canonical import (Â§3.1).
...
- Divergence â€” `Postal State 'Kerala' differs from the GST registration State 'Maharashtra (27)'. Printed invoices and tax calculation use the GST State.`
- Unrecognised postal State with GST on â€” `Postal State 'WB' is not a recognised State/UT, so it cannot be checked against the GST registration State 'West Bengal (19)'.`
...
is untouched; accepting without editing re-saves identical values (Save is a full rewrite of the same fields â€”
Â§1.6 â€” so the row comes back byte-identical). This is required by ER-13 and it is testable today, because **the
fixture already exists**: `tests/Apex.Desktop.Tests/VoucherInvoicePrintViewModelTests.cs line 355` builds exactly this
company â€” `c.State = "Kerala"` against a GST home State of 27 â€” and its own comment reads *"postal State (code
32) â€” deliberately NOT the GST home State (27)"*. The print-side assertion
`A_company_whose_postal_State_disagrees_with_its_GST_State_prints_the_GST_one` must stay green **unchanged**;
Â§6 adds the capture-side sibling rather than editing it.

### 3.6 The change list â€” every call site this slice touches
...
| 4 | `MainWindowViewModel.cs line 827-839` `CreateCompany` | Read the view model; pass `financialYearStart` / `booksBeginFrom` into the **existing** `CompanyFactory.CreateSeeded` overload (`CompanyFactory.cs line 17-20`); assign the postal fields; `EnsureValid()` (Â§5); `_storage.Save`; `OpenCompany`. |
| 5 | `MainWindowViewModel.cs` â€” new `ShowAlterCompany()` | A page column via the established pattern `OpenPageColumn(new GatewayColumn(title, page), Screen.AlterCompany, title, () => AlterCompany = page)` â€” the shape `ShowGstConfig` uses at `:3641-3649`. |
| 6 | `MainWindowViewModel.cs line 912-978` `BuildRootColumn` | One new section header + one item (Â§4.4). |
| 7 | `src/Apex.Desktop/Views/MainWindow.axaml line 228-244` | Replace the one-field create form; add the Alter page column. |
| 8 | `src/Apex.Desktop/ViewModels/GstConfigViewModel.cs`, after `:563` | The seed (Â§3.2). |
| 9 | `GstConfigViewModel.cs` (guard surface) | The shared divergence advisory (Â§3.4). |
| 10 | `src/Apex.Ledger/Domain/Company.cs line 79-84` | Correct the stale "open R12 user gate" doc comment (Â§1.8). |
| 11 | `src/Apex.Desktop/Services/VoucherPrintProjector.cs line 698-707` | The same correction; **keep** the rest of that paragraph verbatim (Â§1.8). |
...
Order and section headings are the Study Guide's (grounding Â§2, p.57-60, headings literal), corroborated by the
Book p.13. **Shipped in this slice = the 11 that already have a domain member and a column** (Â§1.5). Everything
else is listed so the omissions are visible rather than silent.

| # | Corpus section | Corpus label | Our member | In this slice? |
|---|---|---|---|---|
| 1 | *(ungrouped)* | Directory | â€” | **NO â€” architectural.** One `.db` per company, path chosen by `CompanyStorage.PathForName` (`src/Apex.Desktop/Services/CompanyStorage.cs line 63-64`). Grounding Â§7.4 calls this "a deliberate architectural difference, not a gap". |
| 2 | *(ungrouped)* | **Name** | `Name` | **YES on Create. DISPLAY-ONLY on Alter** â€” see 4.3. |
| 3 | Primary Mailing Details | **Mailing Name** | `MailingName` | **YES.** The Rule 46(a) supplier *name* (grounding Â§5.5; Book p.13: *"Type Company's Short name here for Show in Invoice/Bill"*). |
| 4 | Primary Mailing Details | **Address** | `Address` | **YES.** Multi-line free text. The Rule 46(a) supplier *address*. |
| 5 | Primary Mailing Details | **State** | `State` | **YES.** Picker over `IndianState.All` (Â§3.1). |
| 6 | Primary Mailing Details | **Country** | `Country` | **YES.** Book's label; the Study Guide calls the same field "Statutory Compliance for". |
| 7 | Primary Mailing Details | **Pin Code** | `Pin` | **YES.** Validated (Â§5). |
| 8-12 | Contact Details | Telephone No Â· E-Mail Â· Mobile No Â· Fax no Â· Website | â€” | **NO â€” Â§2.3.** |
| 13 | Books and Financial Year Details | **Financial year begins from** | `FinancialYearStart` | **YES on Create** (fixes T1-6, Â§1.13). Alter: see 4.3. |
| 14 | Books and Financial Year Details | **Books beginning from** | `BooksBeginFrom` | **YES on Create.** Alter: see 4.3. |
| 15-16 | Security Control | Vault Password Â· User Access Control | â€” | **NO.** No domain member; security is in the excluded phase. |
...
| 19-21 | Base Currency Information | Suffix Symbol to Amount Â· Add space between amount and symbol Â· Show amount in Millions | â€” | **NO.** No domain member; and grounding Â§9 item 6 records that their *defaults are UNVERIFIED* â€” the Book shows the author's walkthrough values, not documented defaults. Shipping them means inventing defaults. |
...
| 24 | Base Currency Information | No of decimal places for amount in words | â€” | **NO.** No domain member. |

**Sections rendered:** *Primary Mailing Details*, *Books and Financial Year Details*, *Base Currency
Information* â€” the corpus's own headings, with the empty ones (Contact Details, Security Control) simply not
drawn. Nesting under headings is also the standing UI convention for this project.

#### 4.1.1 ðŸ”´ The State/Country order conflict â€” RESOLVED, and the reasoning recorded

Grounding Â§2.1 item 2 records that the two primaries invert this pair: **Book p.13** gives Address â†’ **State** â†’
**Country** â†’ Pin Code; **Study Guide p.58-59 prose** gives Address â†’ **Country** â†’ **State** â†’ Pin Code.
**Chosen: the Book's order (State before Country)**, for a reason that is evidence and not preference â€” the
**Study Guide's own worked example at p.268 sides with the Book against the Study Guide's own prose**
(grounding Â§5.1 reproduces it: `Address:` / `State: West Bengal` / `Country: India` / `Pincode: 700039`). Two
concrete renderings agree; one piece of prose dissents. This is the *identical* self-contradiction pattern the
grounding records for the contact block (Â§2.1 item 1), and it resolves the same way.
...
**This does not touch the PRINT order.** Grounding Â§9 item 11 records that we *print* Address â†’ Country â†’ PIN â†’
State, and that changing it would move the shipped recipient block too. **Capture order and print order are now
deliberately different, and Â§8 records that as an open item rather than pretending it is resolved.**

### 4.2 Create-only

- **Directory** â€” not applicable (row 1 above).
- **`Alt+R` â†’ Group Company** (grounding Â§3, SG p.267 / Book p.14, whose creation screen adds *Member
  Companies*, SG p.268) â€” **OUT.** No domain support for a group company exists; this is a feature, not a field.

### 4.3 Alter-only, and the two fields Alter must NOT edit

**`Alt+D` Delete (corpus: Book p.15, `Alt+K > Alter > Alt+D > Enter twice`) â€” OUT of this slice.**
...
also consistent with how the project already treats destructive actions â€” the Restore panel *"refuses it unless
the archive has been examined AND the confirmation is ticked (NFR-8)"* (`MainWindow.axaml.cs line 233-236`).
Recorded, not dropped.

> ### ðŸ”´ðŸ”´ 4.3.1 `Name` MUST be display-only on Alter â€” a book-eater otherwise
>
...
> the old file untouched** â€” two entries in Company Select, the same `Company.Id` in both, and every subsequent
...
> (grounding Â§5.5), and Mailing Name **is** editable here. Tally itself says Mailing Name is the name that
> prints on the invoice.

> ### âš ï¸ 4.3.2 `Financial year begins from` and `Books beginning from` on Alter â€” editable, with a warning
>
> Grounding Â§9 item 2 is explicit: **no corpus source states that any company field becomes read-only after
> creation**, and grounding Â§3 says the corpus documents **no** Alter-only field and **no** locking. So making
...
> **Chosen middle: editable, with an advisory shown whenever the company already has vouchers** â€” the same
...
> **The alternative â€” locking them â€” must not be chosen silently**, because the corpus does not support it;
> Â§8 records the question rather than closing it.

### 4.4 Where it hangs off navigation

**Create â€” unchanged.** It stays the `Create Company` row on the Company-Select menu (`MainWindowViewModel.cs line 808`,
hint `"F3"`), because Alter needs an open company and Create must work with none.

**Alter â€” a new cascade page column under a new `Company` section.** In `BuildRootColumn`
...
> ðŸ”´ **The corpus route cannot be reproduced, and this is a recorded deviation, not an oversight.** The corpus
> route is `Gateway of Tally > Alt+K (Company menu) > Alter` (grounding Â§3/Â§6, Book p.15, SG p.61/p.267).
> **`Alt+K` is already bound in this app** â€” to the Saved Views list (RQ-8), `MainWindow.axaml.cs line 651-653`,
...
> also ships with an empty hint (`:919`). **Inventing a different chord would be worse** â€” it would be an
> unsourced keystroke presented as fidelity. Extended into Â§8.

### 4.5 Accept and Escape

**Accept â€” both corpus forms work, using shipped machinery.** Grounding Â§6 records that the corpus gives **both**
...
- **`Ctrl+A` â†’ saves outright.** Existing arm, `MainWindow.axaml.cs line 211-256`, which falls through to
...
- **`Enter` â†’ `Accept Company? (Y/N)` â†’ `Y`.** Add `Screen.CreateCompany` and `Screen.AlterCompany` to
  ...
  the prompt can never drift from the shortcut â€” the property that method's own comment exists to protect.
  âš ï¸ **This CHANGES today's behaviour on the Create screen** (Enter currently creates immediately, via the
  `case Key.Enter when !IsPickerOpen(e)` arm at `MainWindow.axaml.cs line 900-903`). Named in Â§7. If a headless test
  turns out to depend on Enter-creates-immediately and the team prefers not to move it, the fallback is to add
  **only** `Screen.AlterCompany` â€” recorded here so the choice is explicit rather than discovered.
- **A picker being open must not raise the prompt.** Already handled: both Enter arms carry `!IsPickerOpen(e)`
  ...
  free â€” **do not add a new Enter arm.**

**Escape â€” `Back()`, i.e. pop the column; two presses when a picker is open.** Existing arm,
`MainWindow.axaml.cs line 931-934`. With a dropdown open the first Escape closes the dropdown (the `ComboBox` does it
once the arm yields) and the second pops â€” the settled contract, stated at `:925-930`.
ðŸ”´ **This is OUR convention, not fidelity.** Grounding Â§9 item 5: **Escape behaviour on Company Creation is
UNVERIFIED** â€” the Book's p.435 table does not list `Esc` in the company region, and the only source that does is
the **rejected** Short-Key PDF (grounding Â§1.1). Consistency with the other ~157 form columns is the entire
justification, and it must be written down as such.

**F11-after-save is OUT.** Grounding Â§2.3 (SG p.60) records that TallyPrime opens the Company Features screen
after a save. Our `CreateCompany` ends in `OpenCompany(company)` (`:838`) â†’ the Gateway. Changing that would
alter the landing screen for 153 test-suite bootstraps (Â§7). Recorded as a fidelity gap, deferred.

---

## 5. THE ENSUREVALID WIRING

`plan.md:1795-1800` states the obligation in W0-2a's own row: *"`Company.EnsureValid()` has exactly ONE call
site in `src/`, and it is the canonical import. Nothing calls it on save â€¦ **The day W0-2b's screen ships, that
stops being true**, so W0-2b must call `Company.EnsureValid()` on its save path (or the store must), and it must
ship the test that proves a bad PIN typed into the screen is refused."* Re-verified at `3a4fcdb` in Â§1.1.

### 5.1 Where it goes â€” **`CompanyStorage.Save`**, and the reason is measured
...
before `PathForName` is computed:
    ...
    public void Save(Company company)
    {
        ...
        // written â€” the canonical importer has had this since W0-2a; the UI had nothing.
        company.EnsureValid();
...
- `grep -rn "_storage\.Save(\|storage\.Save(" src/Apex.Desktop --include=*.cs` â†’ **98 call sites**.
- `grep -rn "new SqliteCompanyStore" src/Apex.Desktop --include=*.cs` â†’ **9 hits, ALL of them inside
...
reproduce, one level up, the very defect the review recorded for `MasterGstDetails.EnsureValid` â€”
...
It is one layer deeper and would also cover the engine and the test fixtures â€” a wider blast radius than this
...
### 5.3 The friendly message in front of it â€” the established two-layer pattern
...
The screen pre-validates and shows a message, exactly as the stock-item master does â€” its comment at
...
1. **`CompanyProfileViewModel.Accept()`** checks `IndianPinCode.IsValidOrBlank(Pin)` and, on failure, sets a
   message such as `PIN code must be 6 digits.` and **returns without saving** â€” no exception, no aggregate
   mutation, the operator's typing intact.
2. **`CompanyStorage.Save` â†’ `company.EnsureValid()`** is the floor that catches any path that skipped step 1.
...
### 5.4 A trap in the ordering â€” mutate-then-save, and rollback
...
disk does not have â€” the "wrong-figures divergence" that comment names, on the postal block instead of the GST
one. Reuse `SaveFailure.IsReportable`; do not write a fourth private copy of the list.

### 5.5 How the wiring is PROVED to fire â€” three levels, mutation at the top

**(1) A behavioural test that goes red today** â€” the RED-PROOF, Â§6.1.
...
tests that went red. This is the standing method in this project: `plan.md:1808-1821` records W0-2a's three
...
assertion** â€” lens 2 finding 1 of the owed review is precisely the case where *"the only red was a hardcoded
...
constructs a `SqliteCompanyStore` outside `CompanyStorage` â€” i.e. that the choke point is still a choke point.
...
`plan.md:1808-1821` records W0-2a's three.

### 6.1 THE RED-PROOF

**RP-1 (behavioural â€” the real one).**
...
and `invoice.Seller.Gstin` is the GSTIN â€” the three particulars of CGST Rule 46(a) (*"name, address and Goods and
Services Tax Identification Number of the supplier"*, grounding Â§5.4, first-party CBIC text). Then assert the
...
`MainWindowViewModel` exposes no postal property â€” Â§1.3/Â§1.4 measured that `Company.State`, `Address` and
...
**RP-2 (structural â€” observably red on the UNMODIFIED tree, no new production code needed).**
...
`LedgerMasterViewModel.cs line 582, 803, 965, 988, 1152` are the **party** mailing block and must be excluded â€” the
grounding doc Â§7.2 records that exact confusion).
...
It is deliberately paired with RP-1 and **is not a substitute for it** â€” a structural test proves reachability,
never behaviour.
...
a statutory value â€” the wrong-tax-head class.
...
default clobber the GSTIN-derived code). Pins precedence rung 2 over rung 3 (Â§3.2).

**T-4 ðŸ”´ `A_GST_home_State_is_never_written_onto_a_GST_off_company`.** The Â§1.9 regression guard. Create through
...
*Mutation:* implement the seed as a creation-time stamp (`company.Gst = new GstConfig { HomeStateCode = â€¦ }` in
...
*Mutation:* make the guard a refusal (return before save). Must redden â€” RULING 3 clause 3 is *"A warning, not a
refusal."*

**T-6 `Matching_States_raise_no_warning` / `A_GST_off_company_raises_no_warning` / `A_blank_postal_State_raises_no_warning`.**
The three silences of Â§3.4.
*Mutation:* drop the `gstCode is not null` term â€” the GST-off case must redden. A guard that fires on every
...
âš ï¸ **Not "the two agree with each other" â€” that is lens 2 finding 5's failure mode** (*"asserts the
...
unchanged.
*Mutation:* delete the view-model pre-check (Â§5.3 step 1). Must redden â€” and must redden with a *message*
...
(`Company.cs line 99-100` words it *"Company PIN code 'â€¦' is not a valid 6-digit Indian PIN code."*, deliberately
distinct from the party message â€” `Company.cs line 93-96`).
*Mutation:* **delete `company.EnsureValid();` from `CompanyStorage.Save`.** This is the Â§5.5(2) mutation and its
red count goes in the commit body.

**T-11 `Every_desktop_save_path_goes_through_the_one_guarded_store_opener`.** The Â§5.5(3) reach assertion â€” no
...
**Odd-value PIN fixtures â€” chosen against the actual rule** (`src/Apex.Ledger/Domain/IndianPinCode.cs line 16-29`:
exactly six ASCII digits, first digit 1â€“9, blank allowed):

| Value | Expected | Why this one |
|---|---|---|
| `"700039"` | accept | The corpus's own worked value (SG p.268, grounding Â§5.1) â€” the fixture is sourced, not invented. |
...
| `"Ã Â¥Â­Ã Â¥Â¦Ã Â¥Â¦Ã Â¥Â¦ Ã Â¥Â©Ã Â¥Â¯"` (Devanagari digits) | **reject** | `char.IsAsciiDigit`, not `char.IsDigit`. Nothing in the repository walks this today; without it the ASCII half of the rule is unprovable. |
| `null` / `""` | accept | Unset is not invalid. |

### 6.5 End-to-end: Rule 46(a) through the screen

**T-12 `A_company_created_through_the_screen_prints_a_Rule_46a_compliant_supplier_block`** â€” RP-1, promoted to the
permanent suite. **Odd-value fixtures, deliberately awkward and each for a reason:**

- **Mailing Name â‰  Name.** `Name = "Bright Traders Private Limited"`, `MailingName = "Bright Traders"`. Asserts
  the printed supplier name is the **Mailing Name** (grounding Â§5.5 maps 46(a)'s supplier *name* to that field,
  and `CompanyDisplayName` at `VoucherPrintProjector.cs line 676-677` implements the fallback). A fixture where the
  two are equal cannot tell the two apart â€” that is exactly the enum-zero blindness of lens 2 findings 2 and 3.
- **A three-line address with a blank middle line**, taken from the corpus's own example (grounding Â§5.1):
...
- **An address line containing a comma** â€” `"13A, Picnic Garden Road"`. Pins the newline-only split that
  `SplitAddress`'s own comment insists on (*"'Pune, Maharashtra 411001' is one address line, not two"*,
  `:850-852`).
- **A non-ASCII character in the address** (e.g. `"BengalÃ…Â«ru"`). The printer runs everything through
  `ReportPrintProjector.Ascii`; this pins that the screen does not bypass it.
- **Postal State `"Kerala"` with a Maharashtra GSTIN and home State 27** â€” the divergence, so the E2E case and
...
screen â€” see Â§7.2 for why the existing one must stay green **unchanged**.
...
**T-15 ðŸ”´ `Opening_Alter_on_a_company_whose_stored_State_is_not_in_the_list_and_accepting_changes_nothing`.**
Store `State = "West Bengal "` (trailing space) â€” a value canonical import can produce (`ImportPlan.cs line 1198`
assigns verbatim, no list check) â€” open Alter, accept without touching the control, reload, assert the string is
...
This is the Â§3.1 ER-13 requirement and the only test standing between an Alter screen and silent data loss on
canonical-imported books.

### 6.7 Documentation gates

**T-16.** Any `file.cs:NN` this slice writes into a `.md` must resolve â€” `DocumentCodeAgreementTests` scans every
...
sentences this slice will edit â€” `"GST one** â€” \`src/Apex.Desktop/Services/VoucherPrintProjector.cs line 726\` is"`
(`:47-50`) and `"**â–¶ The load-bearing guard â€” \`SupplierPostalAddressText\`"` (`:65-68`), both in `plan.md`.
...
yet."* The natural anchor is the `CompanyStorage.Save` guard â€” document line context phrase â†’ `CompanyStorage.cs`
â†’ required token `company.EnsureValid()`.
...
`StackPanel` (`:314-315`); and **no internal build jargon in any user-facing string** (`:1275-1276`) â€” so no
...
### 7.1 ðŸ”´ The single largest risk: 97 test files bootstrap through `CreateCompany()`
...
still governs. **Fixing the Januaryâ€“March future-FY bug (Â§1.13) by changing that default is a separate change
with its own regression surface â€” it would move the FY of every one of those 153 fixtures.** Recommendation:
...
|---|---|---|
| `GatewayHierarchyTests.Gateway_exposes_the_sections_with_their_items_nested` (`tests/Apex.Desktop.Tests/GatewayHierarchyTests.cs line 47-67`) | It asserts the **exact** header list and order: `Assert.Equal(new[] { "Masters", "Statutory", "Transactions", "Reports", "Data" }, HeaderLabels(vm));` (`:54`). Adding a `Company` section (Â§4.4) makes it red. | **Update deliberately** to `{ "Company", "Masters", "Statutory", "Transactions", "Reports", "Data" }` and add `Assert.Contains("Alter Company", items)`. The test is doing its job. |
...
| `A_freshly_created_company_prints_no_supplier_address_lines_at_all` (`tests/Apex.Desktop.Tests/VoucherInvoicePrintViewModelTests.cs line 472-499`) | It drives the **real** `CreateCompany()` path and asserts `Address` blank, `Country == "India"`, `Pin == null`. Its own doc comment says *"if `CreateCompany()` ever starts capturing an address, the probe assertions fail here first."* | **ðŸ”´ It must stay GREEN, UNCHANGED.** That is an acceptance criterion, not an accommodation: the create screen must default the postal block to blank and must not write `Country` to anything other than `"India"`. If this test needs editing, the design has drifted into changing every existing book's printed output. |
| `A_company_with_no_address_still_prints_exactly_as_before` (`:448`) and `The_Rule_46a_name_and_GSTIN_pair_is_delivered_but_the_address_half_is_not` (`:319`) | The latter's **name asserts a defect that this slice closes** for newly-captured companies. | The behaviour it pins â€” a company with no captured address prints no address â€” is still correct and must stay green. **Its name and doc comment need rewording**, because "the address half is not delivered" stops being a statement about the product and becomes a statement about that fixture. Reword; do not delete. |
| `KeyboardArbitrationTests` / `MenuHotKeyAndAcceptTests` / `QuickJumpModifierGuardTests` | They assert `"Accept Ledger? (Y/N)"` and `IsMasterAcceptScreen` on **ledger/voucher** screens only. | Unaffected by adding two screens to the list â€” **verify, do not assume.** `KeyboardArbitrationTests.cs line 1133` asserts `IsMasterAcceptScreen` is **false**, but on `Screen.VoucherEntry` (`:1132`), which is untouched. |
| Everything that saves a company through `CompanyStorage.Save` | Â§5.1 adds `company.EnsureValid()` there. Any fixture holding an invalid `Company.Pin` would now throw. | **Measured risk: low.** No UI writes `Company.Pin` today (Â§1.1), so no Desktop fixture can hold one. **But run the full Desktop suite before and after adding that single line and report both counts** â€” this is exactly the kind of "obviously safe" line that lens 1 finding 1 was. |
...
today â€” only its side effect (a company object) is exercised, 153 times. So adding `Screen.CreateCompany` to
`IsMasterAcceptScreen` (Â§4.5) breaks no test, **and that is not reassurance** â€” it means the behaviour change
would ship unobserved. The slice must add the coverage it is changing.

### 7.3 ER-13 â€” a company that never touches these fields must be byte-identical
...
1. **Printed output.** Guaranteed by construction: the slice does not touch `SellerBlock`
   ...
   prints â€” the state `A_freshly_created_company_prints_no_supplier_address_lines_at_all` already pins.
2. **The stored row.** No schema change (Â§2), and `InsertCompany` binds the same columns from the same members.
...
| Left out | Reason |
|---|---|
| **The 5 contact fields** (Telephone, E-Mail, Mobile, Fax, Website) | Â§2.3 â€” the corpus cannot settle their order (one primary contradicts itself), they are fidelity not compliance, and they cost a migration + a downgrade leg on a downgrade path already recorded as broken. Deferred to a proposed **W0-2c** with two named preconditions (Â§2.4). |
| **Any schema change / a v54 claim** | Â§2 â€” nothing needs one, and declining v54 leaves `plan.md`'s allocation line untouched (`plan.md:1495-1512`). |
| **`Alt+D` company delete** | Â§4.3 â€” destructive, operates on the open company's own `.db`, needs a confirmation design of its own. |
| **Company RENAME** | Â§4.3.1 â€” the `.db` filename **is** the company name (`CompanyStorage.cs line 63-64, 70-75, 48-59`). Renaming without a file move produces two books. `Name` is display-only on Alter; the statutory name (Mailing Name) is editable. |
| **`Alt+R` Group Company** | Â§4.2 â€” no domain support; a feature, not a field. |
| **F11-opens-after-save** | Â§4.5 â€” corpus-attested (grounding Â§2.3) but changes the landing screen for 153 fixtures. |
| **The corpus `Alt+K` company-menu route** | Â§4.4 â€” `Alt+K` is bound to Saved Views (`MainWindow.axaml.cs line 651-653`). Rebinding is KB-4 arbitration work; inventing a different chord would be unsourced fidelity. |
| **KB-3 prefix type-to-filter on the new pickers** | Â§3.1 â€” not built anywhere in `src/`, three design rounds failed, and `plan.md:712-715` puts it behind a measurement spike. |
| **Matching the corpus's PRINT order** (Address â†’ State â†’ Country â†’ Pin Code) | Grounding Â§9 item 11 â€” it would move the shipped WI-4 **recipient** block too, a second statutory-document change. **Note this slice now makes capture order and print order differ deliberately** (Â§4.1.1); Â§8 records it. |
| **Changing `CompanyFactory`'s FY default** | Â§7.1 â€” moves the FY of 153 fixtures. The screen makes the date typeable, which is what T1-6 asks. |
| **Wiring `EnsureValid` into `SqliteCompanyStore.Save`** | Â§5.2 â€” a wider blast radius than this slice has evidence for. |

### 7.5 Residual risks that survive this design

- **The GST-off â†’ GST-on transition.** The seed only fires when the GST screen is opened. A user who types a
  postal State, never opens the GST screen, and later imports a canonical document with a GST config gets no
  seeding. **Correct** â€” the imported config is authoritative â€” but it means "the GST State always derives from
...
### 7.6 ðŸ”´ A SECOND unguarded invariant this screen makes reachable â€” found during this pass, recorded nowhere

`Company`'s constructor enforces two rules (`src/Apex.Ledger/Domain/Company.cs line 557-569`):

    if (string.IsNullOrWhiteSpace(name))     throw new ArgumentException("Company name is required.", â€¦);   // :559-560
    if (booksBeginFrom < financialYearStart) throw new ArgumentException("BooksBeginFrom must be â‰¥ FinancialYearStart.", â€¦);   // :561-562
...
properties can persist a company that `Company`'s own constructor would refuse â€” the same shape as the recorded
...
`ImportPlan.cs line 1203` and the two date assignments are at **`:1204-1205`** â€” **the guard runs BEFORE the dates are
...
**Why it matters to W0-2b:** Â§4.3.2 makes both dates editable on Alter, which is the first UI that assigns them
...
1. `CompanyProfileViewModel.Accept()` pre-validates `BooksBeginFrom >= FinancialYearStart` and refuses with a
   message (the Â§5.3 pattern), so the screen cannot produce the state; and
...
call. That changes the canonical import's behaviour on documents that are accepted today â€” a compatibility
...
This **extends** `docs/w0-2-company-screen-grounding.md` Â§9 (items 1â€“11). Items 1â€“9 and 11 remain **open** and
...
- **Item 9 (the contact-block field order) is now load-bearing.** It is the primary reason the five contact
  fields are out of scope (Â§2.3a). If a later session closes item 9, the W0-2c precondition is met; if it
  cannot, the fields stay out. **Do not ship them by picking an order.**
- **Item 2 (which fields become non-editable after creation) is why Â§4.3.2 chose "editable with a warning".**
  Locking the two date fields would be inventing a restriction the corpus does not state; silently allowing the
  edit on a book with vouchers would be a wrong-figures hazard. The middle course is a project decision â€” **and
...
### New entries â€” 12 to 20

**12. Whether TallyPrime warns when the postal State and the GST State disagree, and what it says.**
The corpus attests the **default** â€” the GST Details State *"by default shows the State name as selected in the
Company Creation screen"* (Book PDF p.177) â€” and it attests a standing exhortation on the same page, *"In
company creation time State must be selected right."* **It says nothing about what happens when the two
diverge.** The consistency guard of Â§3.4, its two-screen placement and its wording are **entirely a project
...
The corpus says only *"State (from a list)"* (grounding Â§2 field 6) and never enumerates the Company Creation
list. Â§3.1 binds the postal picker to `IndianState.All` â€” the GST state-code master, codes 01â€“38 plus 97, and
...
anything about a warning either.** Â§4.3.2's advisory is a project decision.
...
other any information"* (p.15) â€” it neither includes nor excludes the Name. Â§4.3.1 makes `Name` display-only for
...
p.268) and **Enter-then-Enter** (SG p.60) and, per grounding Â§6, *"does not reconcile them"*. This app's shipped
WI-11 convention is `Ctrl+A` **or** Enter â†’ **`Accept â€¦? (Y/N)`** â†’ `Y`. **The second key is `Y`, not `Enter`,
and that divergence is ours across all 24 existing master screens** (`MainWindowViewModel.cs line 4881-4890`). Â§4.5
extends it to the company screens for consistency. **UNVERIFIED whether TallyPrime's company screen has a
Y/N prompt at all.**

**17. The route to Company Alter when `Alt+K` is unavailable.** The corpus route is `Alt+K` â†’ Company menu â†’
Alter (Book p.15; SG p.61/p.267). `Alt+K` is bound here to Saved Views (`MainWindow.axaml.cs line 651-653`). Â§4.4
places Alter in the Miller cascade under a new **Company** section **with no accelerator**. **The corpus offers
no alternative route to copy** â€” the placement is ours, and no chord should be invented to make it look
attested.

**18. Whether TallyPrime's printed supplier block follows its capture order.** Grounding item 11 already records
that **the corpus contains no supplier-block print specimen**, so the capture orders (Address â†’ State â†’ Country â†’
Pin Code) are indicative only. **This design adds a new fact to that entry:** Â§4.1.1 puts **State before
...
block too â€” the reason item 11 deferred it in the first place.

**19. What "Mailing Name" does on ALTER when it was never separately set.** The corpus says it auto-fills from
Name **at creation** and is editable (grounding Â§2 field 3). Our constructor does the same â€”
`MailingName = name` (`Company.cs line 566`). **It is silent on whether TallyPrime re-syncs Mailing Name when the
Name changes on Alter.** Moot in this slice (Name is display-only on Alter, Â§4.3.1) â€” **it stops being moot the
...
Creation and one on GST Details, and describes the second as defaulting from the first â€” it never says whether
...
1. **Whether `Screen.CreateCompany` joins `IsMasterAcceptScreen`** alongside `AlterCompany` (Â§4.5). It changes
   an untested behaviour (Â§7.2). Recommendation: **yes, both**, with new coverage; fallback: Alter only.
2. **Whether the two date fields are editable on Alter at all** (Â§4.3.2). Recommendation: editable + advisory,
   because locking them is unattested (grounding item 2). A user or reviewer may prefer read-only-with-vouchers.
3. **The exact section placement of the `Company` gateway header** â€” first, or after `Masters` (Â§4.4).
   Recommendation: first. It changes `GatewayHierarchyTests.cs line 54` either way.
4. **Whether W0-2c (the contact block) is created as a plan row now or when item 9 is closed** (Â§2.4).
...
| # | Finding | Where |
|---|---|---|
| F1 | **The store drops a GST home State written onto a GST-off company** â€” the naive reading of RULING 3 produces a silently-lost value. Same root cause as the owed review's lens 1 finding 1. | Â§1.9, Â§3.2, test T-4 |
| F2 | **The `.db` filename IS the company name**, so a rename through Alter would fork the book into two files with no error. | Â§4.3.1, Â§7.4 |
| F3 | **`CompanyStorage.Save` is the single Desktop write choke point** â€” 98 call sites, and the only constructor of `SqliteCompanyStore` in the Desktop layer. One `EnsureValid()` there covers every UI path, present and future. | Â§5.1 |
| F4 | **A second unguarded invariant**: `BooksBeginFrom >= FinancialYearStart` is constructor-only, `EnsureValid` does not check it, and the import calls the guard at `:1203` **before** writing the dates at `:1204-1205`. | Â§7.6 |
| F5 | **97 test files / 153 call sites bootstrap through `CreateCompany()`**, so `NewCompanyName` and the parameterless signature are a binding compatibility constraint. | Â§7.1 |
| F6 | **Two shipped code comments still describe the R12 gate as OPEN** (`Company.cs line 79-84`, `VoucherPrintProjector.cs line 698-707`). | Â§1.8, change list rows 10-11 |
| F7 | **The State/Country capture order is resolvable from the corpus after all** â€” the Study Guide's worked example sides with the Book against the Study Guide's prose, the same self-contradiction pattern as the contact block. | Â§4.1.1 |
| F8 | **The slice needs no schema change and should decline v54**, leaving `plan.md`'s allocation line untouched. | Â§2 |
| F9 | **The create screen has zero test coverage of its navigation/keyboard behaviour** â€” 153 fixtures exercise only its side effect. | Â§7.2 |
| F10 | **Line drift in the grounding doc Â§7.1 and Â§7.6** (`Company.cs` and `SqliteCompanyStore.cs` numbers). Claims stand; numbers moved. | Â§1.5, Â§1.6 |
