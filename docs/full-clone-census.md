...
| 14 | TallyPrime-only capabilities | 10 | 1 | 7 | 2 | 0 |
...
| | **TOTAL** | **221** | **52** | **125** | **44** | **0** |

**A full clone requires 221 named capabilities. We have 52 complete, 125 partial, 44 absent, 0 undetermined.**
...
**▶ 🔴 EXPECTED RUN FROM 2026-09-07 (RULINGS 19 AND 20) ONWARD:
`TOTAL rows=221 C=52 P=122 A=47 U=0 sum=221`.** This is the literal output, transcribed after the edit and
...
| 5.5 | Insert Voucher (Alt+I) | ABSENT | 🔴 **CLAIMED BY A TRACK ON 2026-09-06 AND NOT BUILT — THE GRADE DOES NOT MOVE.** `claude/apex-f2-verbs-limits` (PR #62) carried this row in its brief; the first-time review found **zero lines of its three-dot diff touch it**, and A12 confirmed that before landing the branch for its schema-v54 half. The two comments that discuss the row (`MainWindowViewModel.cs:6629-6637`, `MainWindow.axaml.cs:349-356`) were **already on main** before that branch and are not its work; they state, correctly, that `Alt+I` is spent on the POS tender-mode toggle and that insert-renumbering collides with the IRN/challan freezes in `VoucherAlterationEligibility` — *"needs a user ruling, not a keystroke"*. **Recorded here rather than left to the branch's own summary, because an over-claimed row is the failure mode this census exists to catch** (cf. the 2026-09-04 three-rows-wrong-about-themselves entry). ▼ *Original evidence, unchanged and still true:* Alt+I is spent on the POS tender-mode toggle. No insert-at-position code of any kind, no Screen member, no menu row. Corpus-attested and not built. |
...
#### Area 14 — TallyPrime-only capabilities · 10 rows · 1 complete / 7 partial / 2 absent
