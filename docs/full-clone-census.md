...
| 2 | Accounting masters | 13 | 0 | 12 | 1 | 0 |
...
| 16 | Formerly excluded by decision (security, audit, data structure) — **was §4** | 7 | 0 | 5 | 2 | 0 |
| | **TOTAL** | **221** | **52** | **160** | **9** | **0** |

**A full clone requires 221 named capabilities. We have 52 complete, 160 partial, 9 absent, 0 undetermined.**
...
#### Area 2 — Accounting masters · 13 rows · 0 complete / 12 partial / 1 absent
...
#### Area 8 — Banking · 13 rows · 1 complete / 10 partial / 2 absent
...
| 8.11 | Connected Banking — connect a bank account, then **Get Balance** and **Get Statement** from inside the product | ABSENT | 🔴 **ROW ADDED 2026-09-07 BY USER RULING 19. IT HAD NO ROW AT ALL** — zero hits by name in this census and zero in `src/`. **Vendor pages:** `help.tallysolutions.com/connected-banking/`, `…/get-balance/`, `…/get-statement/`, `…/bank-connections/`, `…/enable-connected-banking/`. `Alt+Z` (Exchange) → **Get Balance**, and `Alt+Z` → *All Banking Options* → **Get Statement** with a period. `ConnectedBanking`, `GetBankBalance` and `GetStatement` → **zero** in `src/`; the phrase *"connected banking"* → **zero**. ⚠️ **A SCOPE QUESTION IS ATTACHED AND IS NOT ANSWERED HERE:** the vendor gates this on a **Tally.NET ID and a valid TSS subscription**, and §1.1 rule 4 excludes *edition/subscription features (Tally.NET, Remote Access, …)* from the denominator. Ruling 19 counted it in scope, so it is counted; **if the user later rules it a subscription feature it leaves the denominator, and that is a ruling, not an agent's call.** |
| 8.12 | Payment Request — generate and share a UPI/gateway **payment link or QR code** for a transaction, a party or a whole report | ABSENT | 🔴 **ROW ADDED 2026-09-07 BY USER RULING 19. IT HAD NO ROW AT ALL** — zero hits by name in this census and zero in `src/`. **Vendor pages:** `help.tallysolutions.com/payment-request/` and `…/tally-prime/banking-utilities/payment-request-faq/`; enabled at `F11` → *Show more features* → **Enable Payment Request to share payment link/QR code**, with a UPI ID / Merchant Name profile. `PaymentRequest` → **zero** in `src/`; the phrases *"payment request"* and *"payment link"* → **zero**. 🔴 **THE NEAR-COLLISION, REJECTED WITH A REASON, BECAUSE A RE-RUN WILL HIT IT:** `\bUPI\b` returns **22** hits and **not one is this feature** — they are the **GST B2C dynamic QR** on an invoice (`GstConfig.B2cDynamicQrEnabled`, the payee VPA and its `name@handle` validator) plus one `BankTransactionType` doc comment listing IMPS/UPI as transfer kinds. A supplier-side invoice QR under GST is a different capability from a Banking payment request, and counting those 22 would report this row PARTIAL on phantom hits — the same shape of trap Area 15 records for `TIN` inside `GSTIN`. |
...
#### Area 16 — Formerly excluded by decision (security, audit, data structure) · 7 rows · 0 complete / 5 partial / 2 absent
...
#### 1.2d 🔴 STRUCK BY USER RULING 20 (2026-09-07) — OUT OF SCOPE, AND **NOT** COMPLETE
...
> **As of 2026-09-07 (user rulings 19 and 20 moved §1.2's denominator; NOT ONE ITEM GRADE CHANGED), against §1.2's 221 denominator:
> 21 shipped-and-compared · 22 grounded · 200 uncompared as shipped · 199 with no sourced verification of any
> kind.**
