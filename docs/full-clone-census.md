...
|---|---|---:|---:|---:|---:|---:|
| 1 | Company creation & configuration (F11/F12) | 9 | 1 | 8 | 0 | 0 |
| 2 | Accounting masters | 13 | 0 | 13 | 0 | 0 |
| 3 | Inventory masters | 15 | 2 | 13 | 0 | 0 |
| 4 | Voucher types (7.2's classic eighteen) | 18 | 5 | 13 | 0 | 0 |
| 5 | Voucher behaviours & edit verbs | 15 | 6 | 9 | 0 | 0 |
| 6 | Statutory, current law (GST, TDS/TCS, salary IT) | 42 | 18 | 23 | 1 | 0 |
| 7 | Payroll | 26 | 6 | 20 | 0 | 0 |
| 8 | Banking | 11 | 1 | 10 | 0 | 0 |
| 9 | Inventory / manufacturing / job work (post-7.2) | 9 | 3 | 6 | 0 | 0 |
| 10 | Accounting features (post-7.2) | 2 | 0 | 1 | 1 | 0 |
| 11 | Reports | 17 | 5 | 12 | 0 | 0 |
| 12 | Printing | 9 | 1 | 8 | 0 | 0 |
| 13 | Data management (import/export/backup/e-mail) | 10 | 3 | 7 | 0 | 0 |
| 14 | TallyPrime-only capabilities | 10 | 1 | 9 | 0 | 0 |
| 15 | Statutory, obsolete by law (pre-GST) — **was §3** | 6 | 0 | 4 | 2 | 0 |
| 16 | Formerly excluded by decision (security, audit, data structure) — **was §4** | 7 | 0 | 6 | 1 | 0 |
| | **TOTAL** | **219** | **52** | **162** | **5** | **0** |

**A full clone requires 219 named capabilities. We have 52 complete, 162 partial, 5 absent, 0 undetermined.**

> **▶ 🔴 RE-SUMMED 2026-09-14 BY A12 ON MERGE OF WAVE T (PRs #102, #103, #104) — AND THE DENOMINATOR MOVED,
> SO READ THE TWO HALVES SEPARATELY BEFORE READING THE TOTAL.** Literal `awk` output now:
> **`TOTAL rows=219 C=52 P=162 A=5 U=0 sum=219`**; immediately before (main at `9a83de6`, after PR #101),
> `TOTAL rows=221 C=52 P=160 A=9 U=0 sum=221`. **Every integer was re-derived by re-running §1.2a's own
> `awk` before and after; not one was edited to fit.**
>
> **The delta is `−2 rows, +0 complete, +2 partial, −4 absent`, and it is TWO DIFFERENT EVENTS added together:**
>
> 1. 🟢 **TWO ROWS WERE BUILT AND MOVED `ABSENT` → `PARTIAL`** — **2.6 Voucher Class** (PR #102) and
>    **16.1 The company vault** (PR #104). Area 2 `0 / 12 / 1` → `0 / 13 / 0` and Area 16 `0 / 5 / 2` →
>    `0 / 6 / 1`. 🔴 **AREA 2 REACHES `0 absent` FOR THE FIRST TIME**, and 2.6 was the last absent row in it.
>    **Both grades are A12's own, not a reviewer's**, and both cells say so.
> 2. 🔴 **TWO ROWS WERE STRUCK AND LEFT THE DENOMINATOR WITHOUT ANYTHING BEING BUILT** — **8.11** and
>    **8.12**, by **user ruling 24**, to §1.2d. Area 8 `13 rows · 1 / 10 / 2` → `11 rows · 1 / 10 / 0`.
>    **`221 − 2 = 219` and `9 − 2 − 2 = 5`.** **Area 8's `0 absent` is NOT an achievement** — it is what is
>    left after its only two absent rows were struck, and §1.2d says so on the rows themselves.
>
> ⚠️ **SO THE HEADLINE "4 FEWER ABSENT" IS HALF EARNED AND HALF ARITHMETIC. Two rows were built; two were
> removed from the question.** This is the same warning §1.2d was created to carry, and it is repeated here
> because this is where a reader meets the number first.
>
> 🔴 **AND A THIRD ROW MOVED IN THE OPPOSITE DIRECTION WITHOUT CHANGING A STATE TOKEN. PR #103 (W-T4) shipped
> real work against SEVEN Area 6 rows — 6.9, 6.11, 6.12, 6.31, 6.38, 6.42 and a re-measurement of 6.29 — and
> NOT ONE OF THEM CHANGED STATE, because every one was already `PARTIAL` and stayed `PARTIAL`.** Among that
> work is the closure of **T0-26**, a wrong-money defect in which the GSTR-3B screen captioned one partial sum
> *"Total output tax"* (Table 3.1(a) only, dropping the whole 3.1(d) reverse-charge liability) and another
> *"Total eligible ITC"* (Table 4(A)(5) only, dropping 4(A)(2), 4(A)(3) and every 4(B) reversal), so an
> operator keying a return off that screen **under-declared output tax and over-claimed input credit at the
> same time.** **A census that only counts state tokens cannot see that, and this note exists so the record
> does not imply wave T's third PR did nothing.**
>
> **▶ 🟢 RE-SUMMED 2026-09-14 BY A12 ON MERGE OF PR #100 (W-Q2) — ONE ROW MOVED, AND IT IS PROGRESS.**
> Literal `awk` output now: **`TOTAL rows=221 C=52 P=160 A=9 U=0 sum=221`**; immediately before (main at
> `34833bb`, after PR #99), `TOTAL rows=221 C=52 P=159 A=10 U=0 sum=221`. The delta is exactly
> **`+0 rows, +0 complete, +1 partial, −1 absent`**, and one area heading moved: Area 1 `1 / 7 / 1` →
> `1 / 8 / 0`. 🔴 **AREA 1 REACHES `0 absent` FOR THE FIRST TIME, AND 1.8 WAS THE LAST ABSENT ROW IN IT.**
> The row is **1.8** (Global settings — F1 (Help) → Settings), `ABSENT` → `PARTIAL`, from PR #100.
> **THE GRADE IS A12'S**, and the evidence is A12's own: the F12 regression was re-derived by reading
> `ButtonBarItem.Invoke` rather than accepting the track's account of it, and the three tests the grade
> rests on were **mutation-proved on the merged tree** (report arm removed → 2 failures including
> `Clicking_the_lit_F12_badge_on_a_report_opens_the_report_configuration`; print-preview arm removed → 1;
> `RowOpacity` collapsed to a constant → 1). **Every integer was re-derived by re-running §1.2a's own
> `awk` before and after; not one was edited to fit.**
>
> 🔴 **AND THE SAME MERGE CLOSED A DEFECT THAT WAS NOT IN ANY ROW: THE BADGE TOLD THE TRUTH TO THE
> KEYBOARD AND LIED TO THE MOUSE.** The first cut of ruling 21's gate lit F12 on `IsReportContext` and
> `Screen.PrintPreview` — the two contexts the window's KEY TUNNEL handles — while `F12Configure()` had no
> arm for either. `ButtonBarItem.Invoke` is a `RelayCommand` over the row action, so a **mouse click never
> enters the tunnel**: clicking the lit badge on a report opened nothing and answered *"this screen has no
> configuration"*. The branch was **correctly withheld** for it once, and this is why a track's own green
> gate is not the gate — the old tests exercised only the key route.

> **▶ 🟢 RE-SUMMED 2026-09-14 BY A12 ON MERGE OF PR #97 (W-R1) — ONE ROW MOVED, AND IT IS THE KIND THAT IS
> PROGRESS.** Literal `awk` output now: **`TOTAL rows=221 C=52 P=159 A=10 U=0 sum=221`**; immediately
> before (main at `973d933`, after PR #96), `TOTAL rows=221 C=52 P=158 A=11 U=0 sum=221`. The delta is
> exactly **`+0 rows, +0 complete, +1 partial, −1 absent`**, and one area heading moved: Area 7
> `6 / 19 / 1` → `6 / 20 / 0`, **reaching `0 absent` for the first time**. The row is **7.19** (Labour
> Welfare Fund deduction), `ABSENT` → `PARTIAL`, from PR #97 at schema **v63**. **THE GRADE IS A12'S**,
> and it is NOT transcribed from a track report — the W-R1 report explicitly said the LWF feature was
> *held back*, so A12 read the landed diff and the reachability tests and graded it directly. It moves
> because schema v63 supplies the effective-from/effective-to window **this row itself named as the
> blocker**, and a user can reach it on the Pay Head master screen. **Every integer was re-derived by
> re-running §1.2a's own `awk` before and after; not one was edited to fit.**
>
> 🔴 **AND THE PROSE LINE ABOVE WAS ALREADY STALE BEFORE THIS EDIT, WHICH IS THE DEFECT THIS BANNER
> EXISTS TO CATCH.** It read ~~*"52 complete, 156 partial, 13 absent"*~~ while the table beside it read
> `158 / 11` — the PR #93 re-sum moved the table and left the sentence behind. Both now carry the awk's
> own output. **The sentence is a transcription of the table, never an independent count.**
>
> 🔴 **ROW 6.35 WAS RE-READ IN THE SAME MERGE AND DELIBERATELY HELD AT `PARTIAL`.** PR #98 (W-P2) closed
> the open item that row names in its own last sentence — the notified Form 26Q spells the §194-I codes
> `4-IA`/`4-IB` and this product was emitting `4IA`/`4IB` into a filed return — and it cleared the **G3**
> boundary gate for §192A and §194EE. That is real, and it is recorded in the row. It is **not** a grade
> move: Form 27Q, §197 lower/nil certificates, §234E and Form 16B/26QB remain unstarted, which is most of
> what the row is named for. **One of two candidates moved, not two of two.**

> **▶ 🟢 RE-SUMMED 2026-09-09 BY A12 ON MERGE OF PR #93 (WAVE P) — TWO ROWS MOVED OFF `ABSENT`, AND THE
> THIRD ROW OF THE SAME BRIEF WAS DELIBERATELY HELD.** Literal `awk` output now:
> **`TOTAL rows=221 C=52 P=158 A=11 U=0 sum=221`**; immediately before (main at `53d43a1`, after PR #92),
> `TOTAL rows=221 C=52 P=156 A=13 U=0 sum=221`. The delta is exactly **`+0 rows, +0 complete, +2 partial,
> −2 absent`**, and one area heading moved: Area 6 `18 / 21 / 3` → `18 / 23 / 1`. **Every integer was
> re-derived by re-running §1.2a's own `awk` before and after; not one was edited to fit.** The rows are
> **6.23** (Multiple GSTIN registrations) and **6.25** (GST Classification master), both `ABSENT` → `PARTIAL`,
> both from PR #93 at schema **v61**. **THE GRADES ARE A12'S**, recorded from the track's own report; the
> five migration pieces (`CurrentVersion`, the additive migration body, the `version == 60` ladder step, the
> byte-identical `CreateV1` declarations, and the wired `V61ToV60` downgrade) were each verified first-hand
> at the branch head **before** the merge rather than taken on report, because a half-written migration
> poisons every book created after it.
>
> 🔴 **THE HONEST OTHER HALF, AND IT IS THE LOAD-BEARING PART OF THIS ENTRY.** **6.24 (ISD) DID NOT MOVE AND
> WAS NOT ATTEMPTED** — it is credit *distribution* across registrations, it was sequenced behind 6.23, and
> 6.23 landed without voucher-side attribution, so there was no flow to project a return from. It stays
> `ABSENT` rather than becoming an invented `PARTIAL`. **And 6.23 itself is `PARTIAL` in a way that matters
> more than the grade suggests: nothing in the UI writes `Voucher.GstRegistrationId`, so a second
> registration's returns are structurally EMPTY.** The scoping engine, the refusal guard and the masters are
> real and tested; the data that would flow through them cannot yet be entered. Read row 6.23's cell before
> planning anything that depends on this.
>
> 🔴 **A SECOND BRANCH IN THIS WAVE (`claude/apex-p2-tds-tail`, census 6.35) WAS COMMITTED BUT NOT MERGED AND
> NOT GRADED** — its track returned no report, so there is no gate and no mutation evidence for it. **No row
> moved for it.** The work is preserved on origin as an explicitly-labelled WIP commit.

> **▶ 🟢 RE-SUMMED 2026-09-08 BY A12 ON MERGE OF PRs #90 AND #91 (WAVE O) — FOUR ROWS MOVED OFF `ABSENT`,
> AND CODE WAS WRITTEN TO MOVE EVERY ONE OF THEM.** Literal `awk` output now:
> **`TOTAL rows=221 C=52 P=156 A=13 U=0 sum=221`**; immediately before (main at `6e9b10c`, after PR #89),
> `TOTAL rows=221 C=52 P=152 A=17 U=0 sum=221`. The delta is exactly **`+0 rows, +0 complete, +4 partial,
> −4 absent`**, and three area headings moved: Area 2 `0 / 11 / 2` → `0 / 12 / 1`, Area 3 `2 / 11 / 2` →
> `2 / 13 / 0`, Area 6 `18 / 20 / 4` → `18 / 21 / 3`. **Every integer was re-derived by re-running §1.2a's own
> `awk` before and after; not one was edited to fit.** 🔴 **AREA 3 REACHES `0 absent` FOR THE FIRST TIME** —
> the fourth area to do so, after 4, 5 and 14. The rows are **2.2** and **3.6** and **3.13** (PR #90, schema
> v60) and **6.35** (PR #91, no schema). **THE GRADES ARE A12'S**, recorded from each track's own report; row
> 2.2's fix of defect **T1-31** was verified first-hand against `6e9b10c` before the merge rather than taken on
> report, and the `AffectsGrossProfits` arm of `ProfitAndLoss` — the wave's only wrong-money surface — was
> mutation-verified by A12 independently of the track's own evidence.
>
> 🔴 **AND THE HONEST OTHER HALF: SEVEN BRIEFED ROWS DID NOT MOVE, AND SIX OF THEM ARE NOT MERELY "UNBUILT".**
> Recording only the four that moved would leave the next wave to rediscover all of this at cost.
> **STORAGE-BLOCKED, DEFERRED TO SCHEMA v61 — do not attempt in a wave with no migration budget:** **6.23**
> Multiple GSTIN registrations (the architecture question is now *settled*: the vendor does this with multiple
> GSTINs **inside one company**, not separate companies, so it needs a `gst_registrations` collection **and** a
> registration reference on **every voucher**, because returns are filed per registration and attribution
> cannot be derived); **6.24** ISD, which is blocked *on 6.23* — credit distribution across registrations is
> meaningless with one registration, and building it anyway would fabricate a flow the data model cannot hold;
> **6.25** GST Classification master (the 5 `GstClassification` hits in `src/Apex.Desktop` are all the
> engine-managed `LedgerGstClassification` value object — **a different thing, not a partial version**);
> **7.19** Labour Welfare Fund, blocked because `pay_head_computation_slabs` has **no date column** and
> `PayrollComputationService.EvaluateRaw(PayHead)` takes **no period argument**, so the vendor's *"deducted only
> for the month specified"* is not even *expressible* — shipping it on today's mechanism would deduct an ANNUAL
> ₹20 twelve times, a 12× over-deduction strictly worse than the Karnataka PT bug that already cost a Tier 0 fix
> plus the v55 back-fill. **NOT storage-blocked, but each needs its own wave:** **2.6** Voucher Class (extend
> the existing `voucher_type_classes` table with child tables — do **not** build beside it) and **10.2** Multi
> Address (must own its wave's only migration, and the printed document is the part that makes it more than a
> master). **AND 1.8 IS NOT AN ENGINEERING BLOCK AT ALL — IT IS MIS-TITLED AND NEEDS A USER RULING (R6/R12).**
> The vendor states verbatim that *"Button F12 has been removed from Menu context"* and *"The menu Configuration
> has been removed"*, the items having moved to F1 (Help) → Settings. **TallyPrime has no global F12
> configuration tree**, so the row as written asks us to invent a Tally.ERP 9 artefact into a TallyPrime clone.
> This is the **third wave in a row** to decline it; it will be declined a fourth time unless the title is
> ruled on. ⚠️ **A live dead-knob defect sits underneath it either way:** `MainWindowViewModel.cs:11245` adds
> the F12 *Configure* button **unconditionally and with no `hasCompany` predicate** — unlike the F11 line
> immediately above it — so it is offered before any company is open, where it sets a status string and nothing
> else. Zero-schema fix, and it is the honest core of what 1.8/1.9 is really about.
>
> **▶ 🟢 RE-SUMMED 2026-09-08 BY A12 ON MERGE OF PRs #86, #87 AND #88 (WAVE N) — SIX ROWS MOVED OFF `ABSENT`
> AND CODE WAS WRITTEN TO MOVE EVERY ONE OF THEM.** Literal `awk` output now:
> **`TOTAL rows=221 C=52 P=152 A=17 U=0 sum=221`**; immediately before (main at `2bd551f`, after PR #85),
> `TOTAL rows=221 C=52 P=146 A=23 U=0 sum=221`. The delta is exactly **`+0 rows, +0 complete, +6 partial,
> −6 absent`**, and three area headings moved: Area 5 `6 / 8 / 1` → `6 / 9 / 0`, Area 7 `6 / 18 / 2` →
> `6 / 19 / 1`, Area 15 `0 / 0 / 6` → `0 / 4 / 2`. **Every integer was re-derived by re-running §1.2a's own
> `awk` before and after; not one was edited to fit.** 🔴 **AREA 15 HAD NEVER SCORED A SINGLE NON-ABSENT ROW
> BEFORE THIS WAVE** — it went from `0 / 0 / 6` to `0 / 4 / 2` on one branch.
> 1. **THE SIX:** **15.1** State VAT, **15.2** the VAT masters, **15.5** VAT Computation and **15.6** Central
>    Sales Tax (PR #86, schema **v59** — the wave's only migration); **5.5** Insert Voucher (Alt+I) (PR #87);
>    and **7.18** the NPS pay head (PR #88, **no schema cost at all**).
> 2. 🔴 **FOUR ROWS WERE CARRIED IN A BRIEF AND DELIBERATELY NOT BUILT, AND EACH SAYS SO IN ITS OWN CELL** —
>    **15.4** (no vendor page attests a VAT *composition scheme* at all), **15.8** (the RG 23D column set is
>    published only as screenshots, so building it meant inventing a statutory register), **1.8** (the "global
>    F12 tree" is a Tally.ERP 9 artefact TallyPrime explicitly removed), and **7.19** (LWF needs a month gate
>    this product's pay-head computation does not have — shipping it today would deduct the annual figure
>    **twelve times**). **Three of the four are sourcing stops and one is a storage stop. None is a capacity
>    stop, and none was papered over.**
> 3. **A12 did not inherit any of these grades.** Each was re-derived first-hand before it was written: the
>    five schema pieces read out of `Schema.cs`/`SqliteCompanyStore`/`SchemaDowngrade`; reachability read out
>    of the **shell wiring** rather than a view model; three vendor pages re-fetched and quoted; and **three
>    independent mutations** run, one per branch, each reddening exactly the assertion it should and each
>    restored byte-identical with an unchanged CR count.
> 4. ⚠️ **STILL OWED TO THE USER AND NOT DECIDED HERE (R6/R12):** the **15.2 retitle** (the row names
>    `Input VAT @ 4%` classifications the vendor's current screens do not ship), the **1.8 retitle**, and the
>    **U-6 chord ruling** that 5.5 ships scoped around rather than answering.

> **▶ 🟢 RE-SUMMED 2026-09-08 BY A12 ON MERGE OF PRs #81, #82 AND #83 (WAVE K) — SEVEN ROWS MOVED OFF `ABSENT`
> AND CODE WAS WRITTEN TO MOVE EVERY ONE OF THEM.** Literal `awk` output now:
> **`TOTAL rows=221 C=52 P=146 A=23 U=0 sum=221`**; immediately before (main at `2e162e7`, after PR #80),
> `TOTAL rows=221 C=52 P=139 A=30 U=0 sum=221`. The delta is exactly **`+0 rows, +0 complete, +7 partial,
> −7 absent`**, and three area headings moved: Area 1 `1 / 6 / 2` → `1 / 7 / 1`, Area 8 `1 / 8 / 4` →
> `1 / 10 / 2`, Area 9 `3 / 2 / 4` → `3 / 6 / 0`. **Every integer was re-derived by re-running §1.2a's own
> `awk` before and after; not one was edited to fit.**
> 1. **THE SEVEN:** **9.6** Job Costing, **9.7** Item Cost Tracking, **9.8** Tracking Numbers and **9.9**
>    Voucher Classes on Stock Journal (PR #81, schema **v58** — the wave's only migration, so the next owns
>    **v59**); **8.10** e-Payments and **8.13** Auto-create vouchers (PR #83, no schema); **1.7** the F11
>    Accounting Features group (PR #82, no schema). **Area 9 now has no `ABSENT` row at all.**
> 2. 🔴 **NOT ONE OF THE SEVEN IS `COMPLETE`, AND EACH ROW NAMES ITS OWN REASON RATHER THAN A SHARED ONE.**
>    Three are worth reading as a pattern: **9.9 shipped only its Transfer Class** because no vendor page
>    enumerates the Job-Costing Consumption Class's fields; **8.10's instruction-file layout is OURS and is
>    labelled as ours in the source**, because the vendor publishes no format and refers the reader to their
>    own bank; and **two whole vendor sections of 8.10 are absent rather than faked** because nothing in this
>    schema records what happened after an export. Source silence was recorded as silence in all three.
> 3. 🔴 **THE MOST IMPORTANT EVIDENCE IN THE WAVE IS A MUTATION THAT DID *NOT* REDDEN.** Deleting the Tracking
>    No. field from the item-invoice **bill** grid left all eight of row 9.8's reachability tests green — goods
>    keyed, bills not, so Purchase Bills Pending would have reported every receipt as permanently unbilled,
>    which is **wrong rather than merely empty**. A ninth test was written against the live mutation before the
>    XAML was restored. **A green reachability suite is not proof a feature is reachable in both halves.**
> 4. **ROW 1.8 DID NOT MOVE, AND THE REASON IS THAT THE ROW IS MIS-SPECIFIED, NOT THAT THE WORK WAS SKIPPED** —
>    see the row for the vendor quotations. Re-titling it is an R6/R12 user decision and is **owed to the user**.
> 5. **THE DENOMINATOR IS STILL UNDERSTATED AND THE 221 IS NOT THE HONEST FIGURE.** 8.11 Connected Banking and
>    8.12 Payment Request remain `ABSENT` by scope rule (they need a live vendor subscription) and PR #83
>    deliberately gave them **no menu row**, with a test asserting the Banking column offers neither — a row
>    leading to a screen this build cannot honour is how a cell gets graded present while it is absent.

> **▶ 🟢 RE-SUMMED 2026-09-07 BY A12 ON MERGE OF PR #80 (wave J1) — FIVE ROWS MOVED OFF `ABSENT` AND CODE WAS
> WRITTEN TO MOVE EVERY ONE OF THEM.** Literal `awk` output now: **`TOTAL rows=221 C=52 P=139 A=30 U=0 sum=221`**;
> immediately before (main at `f7a54b7`, after PR #79), `TOTAL rows=221 C=52 P=134 A=35 U=0 sum=221`. The delta is
> exactly **`+0 rows, +0 complete, +5 partial, −5 absent`**, and Area 7's heading moved `6 / 13 / 7` → `6 / 18 / 2`.
> **Every integer was re-derived by re-running §1.2a's own `awk` before and after; not one was edited to fit.**
> 1. **THE FIVE:** **7.22** Attendance Sheet, **7.23** Pay Head Employee Breakup, **7.24** Employee Pay Head
>    Breakup, **7.25** Payroll Statutory Summary and **7.26** Income Tax Computation — the five rows user ruling 19
>    added on 2026-09-07, all five of which had no row and no feature. They were ABSENT for less than a day.
> 2. 🔴 **NOT ONE OF THE FIVE IS `COMPLETE`, AND THE REASON IS ONE SHARED STRUCTURAL GAP RATHER THAN FIVE
>    OVERSIGHTS.** The vendor drills on `Enter` in all four report rows; **the payroll matrix in this product
>    carries no drill stack**, so each detail level is rendered as a SECOND BAND under its report. Every figure the
>    drill would show is on screen — nothing was dropped — but **the navigation shape is ours, not the vendor's**,
>    and on this census's own bar that is PARTIAL. Building a drill stack for the payroll matrix would close all
>    four at once and is the single highest-leverage item left in Area 7.
> 3. **NO NEW ARITHMETIC WAS WRITTEN FOR ANY OF THEM.** 7.26 reads `Form24Q.BuildAnnexureII` (the call backing
>    Form 16 Part B) and 7.25 reads posted vouchers rather than recomputing PF/ESI/PT — so a report cannot drift
>    from the return or the register it summarises. ⚠️ **That also means 7.26 makes T1-26 and the unverified 4%
>    cess VISIBLE without making them VERIFIED.** Surfacing a figure is not sourcing it.

> **▶ 🟢 RE-SUMMED 2026-09-07 BY A5 (wave J2, schema v57) — TWO ROWS MOVED OFF `ABSENT` AND THIS TIME CODE WAS
> WRITTEN TO MOVE THEM.** Literal `awk` output now: **`TOTAL rows=221 C=52 P=134 A=35 U=0 sum=221`**; immediately
> before, `TOTAL rows=221 C=52 P=132 A=37 U=0 sum=221`. The delta is exactly **`+0 rows, +0 complete,
> +2 partial, −2 absent`**, and Area 8's heading moved `1 / 6 / 6` → `1 / 8 / 4`. **Every integer was re-derived
> by re-running §1.2a's own `awk` before and after; not one was edited to fit.**
> 1. **THE TWO:** **8.5** Cheque Register and **8.6** Deposit Slip, both reached by arrow navigation from
>    `Transactions > Banking` and both asserted that way by `BankingStorageReachabilityTests`.
> 2. 🔴 **AND ONE ROW THAT DID NOT MOVE BUT CHANGED MEANING ENTIRELY: 8.4.** It stays **PARTIAL**, and the
>    reason it stays is no longer the reason it stated. It used to be partial because **the cheque leaf could not
>    be printed by anybody** — `Ledger.ChequeLayout` had zero writers in `src/` and nothing persisted it, so
>    ~625 lines of correct, tested rendering code were unreachable. Schema **v57** and the bank ledger master's
>    Cheque Dimensions block close that. What keeps 8.4 partial now is the named remainder in its own cell: one
>    format instead of the vendor's subscription-served table, no UI route to the calibration sheet, and a
>    `printed` flag with storage and no writer.
> 3. **NEITHER NEW ROW IS `COMPLETE`, and that is the measurement rather than modesty** — each names what it does
>    not hold (no `Alt+B` from inside the register, no `Alt+R`; no telephone number and no cash-denomination
>    block on the slip, both because no admissible source and no stored field exists for them).

> **▶ 🔴 RE-SUMMED 2026-09-07 BY A15 AT `origin/main` `cb8cccb`, SCHEMA v56 — SEVEN ROWS MOVED OFF `ABSENT`
> AND NOT ONE LINE OF CODE WAS WRITTEN TO MOVE THEM. READ THIS BEFORE EVERY BANNER BELOW IT.**
> The table read ~~*"TOTAL … 52 · 125 · 44"*~~; areas 2, 5 and 14 read ~~`0|8|5`~~, ~~`6|6|3`~~ and ~~`1|7|2`~~.
> **Every integer above was re-derived by re-running §1.2a's own `awk` before and after; not one was edited to
> fit.** Literal output now: **`TOTAL rows=221 C=52 P=132 A=37 U=0 sum=221`**; immediately before,
> `TOTAL rows=221 C=52 P=125 A=44 U=0 sum=221`. The delta is exactly **`+0 rows, +0 complete, +7 partial,
> −7 absent`**.
> 1. **THE SEVEN, WITH THE PR THAT SHIPPED EACH AND NOBODY MOVED THE ROW:** **2.4** Voucher Type master ·
>    **5.10** numbering methods · **5.11** voucher-type user flags (all three, **PR #52**); **14.1** Go To
>    (Alt+G) · **14.5** Basis of Values / Scale Factor (**PR #53**); **2.12** Multi Ledger / Multi Group ·
>    **2.13** the derived Show-Unused view (**PR #54**). 🔴 **THIS IS BOOKKEEPING, NOT PROGRESS** — the same
>    shape as the ten bolded cells of 2026-09-07, and the same direction: the published split **understated
>    our own landed work**, which is the direction nobody double-checks. `complete` did not move.
> 2. **EVERY ONE WAS MOVED ONLY AFTER THE CAPABILITY WAS DRIVEN TO BY A USER GESTURE** — a menu row in a
>    `Build*Column`, a `switch (item.Label)` case, a `Key.*` arm in `MainWindow.axaml.cs`'s `OnKeyDown`, and a
>    realised control in `MainWindow.axaml`. A view model alone moved nothing.
> 3. 🔴 **ALL SEVEN ARE `PARTIAL`. NOT ONE IS `COMPLETE`, AND THAT IS THE MEASUREMENT, NOT MODESTY** — each
>    row names the fraction of the vendor capability we hold: 2.4 ships **7 of ~20 fields**, 14.5 **2 of 8
>    options**, 2.12 **2 of 6 families and no Multi Alter**, 2.13 **ledgers only and view-only**.
> 4. 🔴 **AN EIGHTH CANDIDATE WAS RE-MEASURED AND LEFT WHERE IT WAS. Row 5.5 (Insert Voucher, Alt+I) IS STILL
>    GENUINELY `ABSENT`** — `Alt+I` is the POS tender-mode toggle, there is no insert-at-position code, and
>    both surviving notes say so in the product's own source. **A stale ABSENT costs less than a wrong
>    PARTIAL**, and this document has been corrected in BOTH directions before.
> 5. 🔴 **A NEW DEFECT WAS FOUND WHILE PROVING 5.11 AND IS FILED IN THAT ROW: two of its three flags are
>    WRITE-ONLY.** `PrintAfterSaving` and `ProvideNarrationForEachLedger` are settable, persisted and
>    exported, and **nothing anywhere reads them** — checkboxes an operator can tick that change no behaviour.
>    That is why 5.11 is `PARTIAL` and not `COMPLETE`, and it is the fourth dead-capability of this shape.
> 6. **Area 14 reaches `0 absent` for the first time.** Areas 11, 12 and 13 were already there.

> **▶ 🔴 THIS SENTENCE WAS STALE BEFORE THIS EDIT, AND THE LOCK COULD NOT SEE IT.** Re-running §1.2a's own
> `awk` on `origin/main` at `4780066`'s parent gave `TOTAL rows=221 C=52 P=123 A=46 U=0 sum=221`, while the
> sentence above still read *"52 complete, 122 partial, 47 absent"* — one row out, in the direction that
> **understated** our own work, which is the direction nobody double-checks.
> `CensusDerivationDriftLockTests` reconciles the area headings and the §1.2 **table** (including the TOTAL
> row) against the rows, and both of those were right; the **prose restatement underneath the table is not a
> surface it parses**, so it drifted silently. Corrected here to the derived figures. **A fourth derived
> surface is now known to exist and to be unguarded** — worth a lock of its own, and a `WAVE 13` doc comment
> in that test file still quotes the old `C=52 P=122 A=47` for the same reason.

> **▶ 🔴 THE DENOMINATOR MOVED ON 2026-09-07 BY TWO USER RULINGS, AND IT IS NOT A RE-COUNT OF THE SAME
> PRODUCT. `216 → 224 → 221`, AND ONLY THE FIRST STEP IS HONEST GROWTH IN WHAT WE OWE.** Rulings **19** and
> **20** (R12, `plan.md` §5, `TWO FURTHER USER RULINGS (R12, 2026-09-07)`). **Every integer in the table above
> was re-derived by re-running §1.2a's own `awk`; not one was edited to fit.**
>
> **THE ARITHMETIC, WRITTEN OUT SO IT IS CHECKABLE RATHER THAN ASSERTED: `216 + 8 − 3 = 221`.**
> - **`+8` — RULING 19. EIGHT VENDOR-ATTESTED CAPABILITIES HAD NO CENSUS ROW AT ALL.** Each was verified twice
>   by name — zero hits in this document, zero in `src/` — so they were **missing rows AND missing features**,
>   not rows filed under another name. **Area 7 gains five** (Attendance Sheet · Pay Head Employee Breakup ·
>   Employee Pay Head Breakup · Payroll Statutory Summary · Income Tax Computation report) and **Area 8 gains
>   three** (Connected Banking · Payment Request · auto-create vouchers from an imported bank statement).
>   **All eight are ABSENT**, which is why the old figure flattered us **in both terms at once** — the eight
>   were missing from the numerator *and* from the denominator. Rows **7.22–7.26** and **8.11–8.13**, each
>   citing the vendor page that attests it. 🔴 **THE USER WAS SHOWN THIS AND CHOSE TO MOVE THE TOTAL RATHER
>   THAN KEEP A NUMBER THAT FLATTERS US.** Ruling 10 had set 216; **the honest figure is 224**, and the goal is
>   now *"all 224"*. ⚠️ **223 if a reader judges auto-create-vouchers to be inside row 8.3's scope; ruling 19
>   ruled it a row of its own, and 8.3's measurement supports that** — `BankStatementImport` exposes only
>   `ParseCsv` and `MatchAndReconcile`, and never creates a voucher.
> - **`−3` — RULING 20, AND IT IS NOT PROGRESS.** Rows **15.9** (Fringe Benefit Tax), **15.7** (Service Tax +
>   Form ST3) and **15.3** (the 2005 four-slab VAT rate structure) are **STRUCK**: they leave §1.2a for
>   **§1.2d**, which holds their full text, their state when struck and the repeal that killed each. They are
>   **out of the in-scope count** the way §1.1 rule 4's exclusions are, and they are **not** given a fifth
>   state token, which would redden `CensusDerivationDriftLockTests` — correctly, because that is the
>   ten-row defect PR #69 exists to stop. 🔴 **ALL THREE WERE `ABSENT`, SO STRIKING THEM LOWERS THE
>   DENOMINATOR AND THE MISSING COUNT WITHOUT SHIPPING ANYTHING.** *"Absent"* falls `50 → 47` and every
>   percentage improves on a day when **no code was written**. **A struck row is not a completed row.**
>   The user's reasoning, recorded because it decides future cases: *"A clone of a dead tax is dead code no
>   user can ever use."* **What survives is what is still levied** — VAT/CST on alcoholic liquor
>   (Art. 366(12A)) and the five petroleum products (s.9(2) CGST), and central excise on tobacco and those
>   petro products (Fourth Schedule, Central Excise Act 1944) — rows 15.1, 15.2, 15.4, 15.5, 15.6 and 15.8,
>   all unchanged in state.
> - **THE NET, STATED HONESTLY:** the split is **unchanged at `52 complete · 122 partial`**; only the absent
>   count and the denominator moved. On **224** it is `52 / 122 / 50`; on the **221** that remains in scope it
>   is `52 / 122 / 47`. **Complete is 23.5% of 221 — it was 24.1% of 216 — so the one figure that measures
>   delivered work went DOWN, which is what an honest denominator move looks like.**
> - 🔴 **THE DRIFT LOCK'S `ScopedCapabilityRows` MOVED 216 → 221 IN THE SAME CHANGE**, because the lock's own
>   failure message requires exactly that of a genuine, user-ruled denominator move and names these very rows:
>   *"earlier verification found vendor-attested capabilities with NO census row at all, in Areas 7 and 8 —
>   that is a real denominator move: record it in §1.2, tell the user the new total, and update
>   `ScopedCapabilityRows` here in the same change."* **That is the one edit outside these two documents, and
>   it is a scope constant, never an expectation bent to fit a state cell.**

> **▶ 🔴 RE-DERIVED 2026-09-07 BY A15 AT `origin/main` (`90413a9`, schema v55). THE DERIVATION HAD BEEN
> SILENTLY BROKEN FOR TEN ROWS, AND IT UNDERSTATED OUR OWN WORK. READ THIS BEFORE THE 2026-09-05 BANNER
> BELOW IT.** No integer below was typed to a target; every one is transcribed from the literal output of
> §1.2a's own `awk`, re-run before and after.
> 1. **THE COUNTING COMMAND COULD NOT SEE TEN ROWS.** The `awk` matches a state cell as the **bare** token
>    `| PARTIAL |`. Ten rows had been written as `| **PARTIAL** |` — bold markdown — so each was counted in
>    its area's row total and **in no state bucket at all**. The literal output was
>    `TOTAL rows=216 C=52 P=110 A=44 U=0 sum=206`: `rows=216` against `sum=206`, ten rows falling through.
>    The ten — **6.26, 7.20, 7.21, 8.4, 8.7, 12.5, 12.6, 12.7, 14.2, 14.9** — are every one of them rows this
>    campaign LANDED, so the published split was smaller than the truth in exactly the direction that
>    flattered nobody. **Only the state cell was normalised; not one word of any evidence cell was touched.**
> 2. **TWO ROWS WERE GRADED `ABSENT` ON EVIDENCE THAT IS NOW FALSE — 6.10 and 6.13**, both corrected in place
>    with the original text kept. These are precisely the two the 2026-09-05 banner below held back because
>    *"the branch carrying them (b2) was held on a red ubuntu leg and is not on `main`"*. **b2 landed. Nobody
>    moved the rows.** Each was re-measured first-hand against the code at this HEAD, to the census's own
>    test — type, route, caller — not copied from any branch's report.
> 3. **§1.2 AND THE AREA HEADINGS WERE A THIRD AND FOURTH STALE ASSERTION.** Six area headings (6, 7, 8, 10,
>    12, 14) and six §1.2 rows still carried the 2026-09-05 figures, and **area 10 had been stale since row
>    10.1 moved** — Credit Limits shipped in PR #62 and neither the heading nor the table followed. All are
>    now transcribed from the same run. `sum` equals `rows` for the first time since the bolding crept in.
> 4. **THE MOVE IS `+0 complete, +17 partial, −17 absent`, `+0 rows`** — `216 · 52 / 105 / 59` →
>    `216 · 52 / 122 / 42`. **Fifteen of the seventeen are bookkeeping, not progress**: those rows were
>    already graded PARTIAL in §1.2a and the arithmetic simply could not see them. Only **two** are a real
>    state change (6.10, 6.13), and both are A15's grade on a read-only pass, owed to the user.
> 5. 🔴 **A DRIFT LOCK NOW GUARDS ALL FOUR SURFACES** —
>    `tests/Apex.Ledger.Tests/CensusDerivationDriftLockTests.cs`. It fails when a §1.2a state cell is not one
>    of the four bare tokens, when `sum != rows`, when an area heading disagrees with its own counted rows,
>    and when a §1.2 row or the TOTAL disagrees with the count. **This class of defect has now bitten this
>    project twice** — a quoted `grep` result that pointed at a blank line, and this — so the lock is the
>    deliverable, not the tidy-up. It is mutation-verified: re-bolding one cell reddens it by name.
> 6. 🔴 **AND THE DENOMINATOR ITSELF IS UNDERSTATED. `216` IS NOT THE HONEST TOTAL, AND THE USER IS OWED THE
>    REAL ONE RATHER THAN THE ONE THAT FLATTERS US.** Everything above makes the count internally consistent;
>    it cannot make it COMPLETE, because a capability with **no row at all** is invisible to every check in
>    this document. **Eight vendor-attested capabilities have no row**, confirmed 2026-09-07 by searching this
>    file for each by name — **every one returned zero** — and searching `src/` for each, which also returned
>    zero, so these are missing rows AND missing features, not rows filed under another name:
>    - **Area 8 (Banking) is short three:** **Connected Banking**, **Payment Request**, and
>      **auto-creating vouchers from an imported bank statement**. The third is NOT hiding inside row 8.3:
>      that row is reconciliation, and its own **Gap** list — CSV only, no manual match/unmatch, no print or
>      export — never mentions creating a voucher.
>    - **Area 7 (Payroll) is short five:** **Attendance Sheet** (7.15 ships the Attendance *Register*; the
>      vendor has both), the **two pay-head breakups** (Pay Head Employee Breakup and Employee Pay Head
>      Breakup), **Payroll Statutory Summary**, and the **Income Tax Computation** report. ⚠️ Stated precisely
>      so nobody over-reads it: the salary-tax ARITHMETIC exists as an engine record
>      (`SalaryIncomeTax.SalaryTaxComputation`, which backs Form 16 Part B) — what is missing is the vendor's
>      per-employee **report** and any census row for it.
>    **So the honest figure is `216 + 8 = 224`** (**223** if a reader judges auto-create-vouchers to be inside
>    8.3's scope), and on it we hold **52 complete, 122 partial, 50 absent**. ~~🔴 **THE ROWS ARE DELIBERATELY
>    NOT ADDED HERE.** The denominator is a **user ruling** — ruling 10 is what made it `200 + 9 + 7 = 216` —
>    and A15 will not move it in passing, which is the very habit that produced four disagreeing surfaces.
>    **This is a decision owed to the user at a gate.** Until they take it, read every percentage in this
>    document as computed on a denominator that is **eight capabilities too small**, and note that all eight
>    are ABSENT, so the understatement flatters us in both terms.~~
>    ✅ **CLOSED THE SAME DAY BY USER RULING 19 — THE GATE WAS PUT AND THE USER TOOK IT. The eight rows ARE
>    now added** (7.22–7.26, 8.11–8.13), *"the honest figure is 224"* is the ruling's own wording, and the
>    goal is now **all 224**. **Then ruling 20 struck three dead-law rows to §1.2d, leaving 221 in scope** —
>    `216 + 8 − 3 = 221`. See the banner at the head of §1.2, which is the derivation. **The refusal above was
>    right and is kept, not deleted: the denominator moved because the USER moved it, at a gate, not because
>    an agent moved it in passing.**
...
**▶ 🔴 EXPECTED RUN FROM 2026-09-07 (A15'S EIGHT-ROW RE-MEASUREMENT AT `origin/main` `cb8cccb`, SCHEMA v56):
`TOTAL rows=221 C=52 P=132 A=37 U=0 sum=221`.** Literal output, transcribed after the edit; the run
immediately before it was `TOTAL rows=221 C=52 P=125 A=44 U=0 sum=221`. **The move is `+0 rows, +0 complete,
+7 partial, −7 absent`, and every one of the seven is BOOKKEEPING, NOT PROGRESS** — the code had already
landed in PRs #52, #53 and #54 and nobody moved the row. Rows **2.4** (Voucher Type master), **2.12** (Multi
Ledger / Multi Group), **2.13** (the derived Show-Unused view), **5.10** (numbering methods), **5.11**
(voucher-type user flags), **14.1** (Go To, Alt+G) and **14.5** (Basis of Values / Scale Factor) moved
`ABSENT` → `PARTIAL`, each only after the capability was **driven to by a user gesture** — a menu row or a
key arm read out of the view/menu wiring, never a view model in isolation. Areas 2, 5 and 14 match their own
headings (`rows=13 C= 0 P=11 A= 2`, `rows=15 C= 6 P= 8 A= 1`, `rows=10 C= 1 P= 9 A= 0`); **Area 14 reaches
`0 absent` for the first time.** 🔴 **AND ONE CANDIDATE WAS LEFT WHERE IT WAS: row 5.5 (Insert Voucher,
Alt+I) IS STILL GENUINELY `ABSENT`** — re-measured, not assumed. A stale ABSENT costs less than a wrong
PARTIAL, and finding seven of eight was the good outcome, not eight.

**▶ 🔴 EXPECTED RUN FROM 2026-09-07 (RULINGS 19 AND 20) — SUPERSEDED BY THE PARAGRAPH ABOVE:
`TOTAL rows=221 C=52 P=122 A=47 U=0 sum=221`.** This was the literal output, transcribed after the edit and
not typed to a target; the run immediately before it was `TOTAL rows=216 C=52 P=122 A=42 U=0 sum=216`.
**The delta is `+8 rows added, −3 rows struck; +0 complete, +0 partial, +5 absent`** — ruling 19 added eight
`ABSENT` rows that had **no row at all** (7.22–7.26, 8.11–8.13) and ruling 20 struck three `ABSENT` rows to
**§1.2d** (15.3, 15.7, 15.9). 🔴 **NOT ONE STATE CELL CHANGED, AND NOTHING WAS BUILT.** Areas 7, 8 and 15
match their own headings (`rows=26 C= 6 P=13 A= 7`, `rows=13 C= 1 P= 6 A= 6`, `rows= 6 C= 0 P= 0 A= 6`), and
`tests/Apex.Ledger.Tests/CensusDerivationDriftLockTests.cs`'s `ScopedCapabilityRows` moved `216 → 221` in the
same change, as that lock's own message requires of a user-ruled denominator move.
🔴 **AND THIS BLOCK THEN WENT ONE ROW STALE, FROM PR #74 UNTIL 2026-09-07 (A15) — CORRECTED ABOVE, NOT HERE.**
PR #74 graded row 14.4 and updated the bottom-line paragraph and Area 14's heading but **not** this block, so
it went on stating `C=52 P=122 A=47` while `main` had already moved to `C=52 P=125 A=44` — a figure
disagreeing with the command printed a few lines above it. **`CensusDerivationDriftLockTests` cannot see this
block**: like the prose restatement under §1.2's table, an "expected run" paragraph is a derived surface the
lock does not parse. **That is now TWO known unguarded derived surfaces**, and the lock's own doc comment is a
third — it still quotes `C=52 P=122 A=47` in its `WAVE 13` remark. A15 edits only this file, so the test's
comment is reported, not fixed.

**▶ 🔴 EXPECTED RUN FROM 2026-09-07 (A15's RE-DERIVATION AT `90413a9`) — SUPERSEDED BY THE PARAGRAPH ABOVE:
`TOTAL rows=216 C=52 P=122 A=42 U=0 sum=216`.** This supersedes the 2026-09-05 expectation restated
immediately below. 🔴 **READ THE `sum` AGAINST THE `rows` EVERY TIME YOU RUN THIS, AND READ THE
`!! UNPARSED:` LINES — THAT IS HOW THIS COMMAND TELLS YOU IT IS BLIND.** Between 2026-09-06 and this run the
literal output was `TOTAL rows=216 C=52 P=110 A=44 U=0 sum=206`: **ten rows had been written with a BOLD
state cell (`| **PARTIAL** |`), which this `awk` cannot match**, so each was counted in `rows` and in no
state bucket, and the published split understated our own landed work by ten. The command printed
`!! UNPARSED:` for every one of them and was read as noise. **A state cell must be the bare token — never
bold, never struck, never annotated.** `tests/Apex.Ledger.Tests/CensusDerivationDriftLockTests.cs` now
fails the suite on a non-bare token, on `sum != rows`, on an area heading that disagrees with its own rows,
and on a §1.2 cell that disagrees with the count.

**▶ 🔴 EXPECTED RUN FROM 2026-09-05 (THE b1/b3/b4/b5 LANDING) — SUPERSEDED BY THE PARAGRAPH ABOVE: `TOTAL rows=216 C=52 P=105 A=59 U=0 sum=216`.**
...
#### Area 1 — Company creation & configuration (F11/F12) · 9 rows · 1 complete / 8 partial / 0 absent
...
| 1.7 | F11 Company Features — the Accounting Features group | PARTIAL | 🟢 **MOVED `ABSENT` → `PARTIAL` 2026-09-08 ON MERGE OF PR #82 (wave K3, no schema). The grade is the build track's own; A12 recorded it.** Three of the vendor's four ACCOUNTING options now exist as real company features with the captions verbatim — *"Enable Bill-wise entry"*, *"Enable Cost Centres"*, *"Enable Interest Calculation"* — on the **existing** F11 page (`GstConfigViewModel`), reached by a real F11 keypress and walked for on the realised visual tree. `Company.EnableBillWiseEntry` / `EnableCostCentres` / `EnableInterestCalculation`, in-memory `bool?` on the established `MaintainBatchwiseDetails` precedent, **no schema column**. 🔴 **NONE OF THE THREE IS A DEAD KNOB: each gates behaviour in SIX places** — the ledger master's bill-wise and Activate-Interest blocks, the Masters → Create *Cost Masters* section **including its header**, the two Reports → Statements of Accounts rows, the *Cost Centres* / *Interest* quick-buttons on the button bar (**the second door**, without which the gate would be cosmetic), and the voucher line's bill- and cost-allocation panels (**the deepest reach**). All six were individually mutation-verified, each reddening with its own written message. **Gaps keeping it off `COMPLETE`:** *"Maintain Accounts"* is deliberately not shipped — this product has no accounts-less mode, so the switch could only ever read Yes and gate nothing, i.e. a dead knob by construction — and the other switches this row's original evidence listed (multi-currency, budgets, credit limits, cheque printing, multi-address) are not options of the vendor's **Accounting** group and are not on this page. **= T1-15, now partly closed.** *(The original evidence read: zero `IntegrateAccountsWithInventory` and `MaintainMode` hits in `src/` or `tests/` — still true; neither identifier exists.)* |
| 1.8 | Global settings — F1 (Help) → Settings, the app-wide configuration surface *(RETITLED IN PLACE 2026-09-09 BY USER RULING 21; was "F12 Configure — the global configuration tree", which named a surface the reference product does not ship)* | PARTIAL | 🟢 **MOVED `ABSENT` → `PARTIAL` 2026-09-14 BY A12 ON MERGE OF PR #100 (W-Q2) — THE GRADE IS A12'S, AND THE EVIDENCE UNDER IT IS A12'S OWN RATHER THAN TRANSCRIBED FROM THE TRACK REPORT.** The surface ruling 21 asked for now ships: **F1 (Help) → Settings**, its own cascading Miller column beside the F12 report-config panel, carrying **Display → Show bottom bar** and **Country → Date and Number Format → Show Quantity and Number in millions**, each gating real rendered behaviour — the window's status strip, and `IndianMoneyFormat.ActiveCulture`, the ONE grouping rule every rendered amount and quantity formats through (report grids and ledger books via `IndianFormat`, the voucher and invoice entry screens, and the printed tax invoice, POS receipt, voucher, certificates and cheque). **The dead `F12` button the ruling also named is gated**: `CanConfigureCurrentScreen`'s five disjuncts and `F12Configure`'s five arms now stand one for one, so where the badge is dim the bare F12 keystroke is inert *and silent* (`Fire` skips a disabled row), and where it is lit both the key and the mouse reach a real panel. 🔴 **AND THE GRADE RESTS ON A DEFECT A12 RE-DERIVED RATHER THAN ACCEPTED: THE BADGE TOLD THE TRUTH TO THE KEYBOARD AND LIED TO THE MOUSE.** The first cut lit F12 on `IsReportContext` and `Screen.PrintPreview` — the two contexts the window's KEY TUNNEL (`MainWindow.axaml.cs`) handles *before* the bar — while `F12Configure()` had an arm for neither. `ButtonBarItem.Invoke` is a `RelayCommand` wrapping the row action directly, so **a mouse click never enters the tunnel**: clicking the lit badge on a report, the most common F12 context in this application, opened nothing and answered *"this screen has no configuration"*. **The branch was correctly WITHHELD for it once**, and this is the case for why a track's own green gate is not the gate — every pre-existing F12 test drove the key route only. The fix **added the two missing arms rather than narrowing the predicate**, so nothing that worked before lost its door. **Mutation-proved by A12 on the merged tree**, not inferred from a passing suite: report arm removed → 2 failures (`The_F12_badge_never_lies_about_the_screen_it_is_on(context: "Report", expectedLit: True)` and `Clicking_the_lit_F12_badge_on_a_report_opens_the_report_configuration`); print-preview arm removed → 1; `AppSettingGroup.RowOpacity` collapsed to the constant `1.0` → 1 (`The_unavailable_setting_groups_render_dimmed_disabled_and_tagged`). 🔴 **PARTIAL AND NOT COMPLETE, AND THE FOUR SHORTFALLS ARE NAMED RATHER THAN LEFT TO BE DISCOVERED.** **(i)** The **setting store is IN-MEMORY for the session** — this slice carried no schema budget, so a restart forgets both switches; the page says so on screen rather than letting the operator believe otherwise. **(ii)** The vendor's group is *"Date **and** Number Format"* and **only the number half ships**: `ApexDate` holds the date rendering in a single `Canonical` const with no switch anywhere, and the row now says so in words, pinned by a test to `ApexDate.Canonical` itself so the claim cannot drift from the constant it describes. **(iii)** The vendor's Settings popup also carries **Startup**, **Language**, **Connectivity** and **Licence** groups and a **Display → Colour & Sound → Display Mode** and **Enable Stripe View for Reports**; none ships, deliberately — this application has no startup company-loading step, no second language, no client/server mode, no licence, no theme and no report row striping, so each could only be a knob with nothing behind it, which is worse than an honestly absent row. They are **listed on the page, dimmed, disabled and tagged "Not in this build"** rather than silently missing. **(iv)** ⚠️ **An R7 citation defect was caught in review and fixed, and it is recorded because it is the kind this project keeps re-filing:** the original citation was a **bare domain that resolves to no page**, and its two quoted phrases are not on the F1 Date-and-Number-Format documentation at all — two *different* vendor options had been spliced into one sentence. `help.tallysolutions.com/stock-items-faq/` carries this build's path and caption (*"Press F1 (Help) > Settings > Country > select Date and Number Format"*, option *"Show Quantity and Number in millions"*); `help.tallysolutions.com/cheque-payments-set-up/` carries the *"1,000,000 instead of 10,00,000"* worked example and the printed-cheque wording, reached by **Alt+K Company > Alter > F12 > Provide Additional Base Currency details**. Both are now cited separately by exact path, and this build's collapse of the two vendor options into one rule is stated as a **divergence labelled as ours**, not implied. ▼ *Prior evidence, unchanged and kept because it is what the build was made against:* 🔴 **RETITLED 2026-09-09 BY USER RULING 21 (R6/R12), AND THE TITLE IS CHANGED IN THE CELL ITSELF RATHER THAN CORRECTED FROM A NOTE — a stale claim left standing with its correction elsewhere has misled this project repeatedly.** **The evidence for WHY, vendor verbatim, re-fetched by wave N and quoted here at the point of the change** (`help.tallysolutions.com/developer-reference/release-notes-whats-new-in-tdl/working-of-tally-erp-9-customisations-with-tallyprime/`): *"Button F12 has been removed from Menu context."*; *"The menu 'Configuration' has been removed. The related configurations have been added under the respective features."*; *"the items in the General Configuration have now been placed in the Popup Menu, F1: Help invoked from the top buttons (F1: Help > Settings)"*. **TallyPrime has NO global F12 configuration tree, so the row as titled described a Tally.ERP 9 artefact and building it would have been inventing one into a TallyPrime clone.** 🔴 **THE USER CHOSE TO KEEP THE CAPABILITY AND DROP THE WRONG NAME:** the global configuration layer genuinely exists and lives at **F1: Help → Settings** (F12 stays *contextual*, overriding the global layer per report — `help.tallysolutions.com/working-with-reports/`), so the row is retitled to the surface the vendor actually ships and **that** is what gets built. **The ruling also gates the dead `F12` button — `ButtonBar.Add(new ButtonBarItem("F12", "Configure", F12Configure));` — which is presently offered to the operator where it does nothing.** ⚠️ **Line reference, recorded honestly rather than quoted:** the ruling named `MainWindowViewModel.cs:11245`; A12 re-measured it on `origin/main` `53d43a1` and it sits at **`src/Apex.Desktop/ViewModels/MainWindowViewModel.cs:11298`** — the only `"F12"` button in the file, so the target is not in doubt and only the line number drifted. **The row stays `ABSENT` and in scope; the retitle moves no state cell and no count.** ▼ *The evidence trail that produced the ruling, kept verbatim:* `F12Configure()` has three real arms and then a fall-through that literally sets a stub message string. No configuration-tree type, no Screen member, no menu row. **= T1-16.** 🔴 **THE STATE IS UNCHANGED, BUT WAVE K3 (PR #82) REPORTED THAT THE ROW ITSELF IS MIS-SPECIFIED, AND NOTHING WAS BUILT FOR EXACTLY THAT REASON. This confirms the census's own open thread U-M(iii), which suspected this and asked for it to be checked first.** TallyPrime has no global F12 configuration tree; building one would have been inventing a Tally.ERP 9 artefact into a TallyPrime clone. Vendor verbatim (`help.tallysolutions.com/developer-reference/release-notes-whats-new-in-tdl/working-of-tally-erp-9-customisations-with-tallyprime/`): *"Button F12 has been removed from Menu context."*; *"The menu 'Configuration' has been removed. The related configurations have been added under the respective features."*; *"the items in the General Configuration have now been placed in the Popup Menu, F1: Help invoked from the top buttons (F1: Help > Settings)"*. And `help.tallysolutions.com/working-with-reports/`: F12 (Configure) overrides the global configuration **per report**. So the global layer exists but lives at **F1 (Help) → Settings**, and F12 is contextual. ~~⚠️ **RE-TITLING THIS ROW TO *"Global settings — F1 (Help) → Settings"* AND RESHAPING T1-16 IS A `plan.md` CHANGE AND THEREFORE AN R6/R12 USER DECISION — IT IS NOT MADE HERE, AND THE ROW STAYS `ABSENT` AND IN SCOPE UNTIL THE USER RULES.**~~ 🔴 **THE USER RULED ON 2026-09-09 (RULING 21) AND ASKED FOR EXACTLY THIS RE-TITLE — struck rather than deleted so the reader can see that the two tracks that refused to build the row were right to refuse, and that the ruling, not an agent, made the change.** ▼ 🔴 **CONFIRMED INDEPENDENTLY 2026-09-08 (wave N, PR #87) — SECOND TRACK IN A ROW TO CARRY THIS ROW IN ITS BRIEF AND DECLINE TO BUILD IT, ON THE SAME GROUND.** The track re-fetched the vendor page itself rather than inheriting this cell, and returned verbatim from `…/working-of-tally-erp-9-customisations-with-tallyprime/`: *"Button F12 has been removed from Menu context."* and *"The menu 'Configuration' has been removed. The related configurations have been added under the respective features."* and *"the items in the General Configuration have now been placed in the Popup Menu, F1: Help invoked from the top buttons (F1: Help > Settings)"*. **Building "the global configuration tree" as titled would INVENT A TALLY.ERP 9 ARTEFACT INTO A TALLYPRIME CLONE** — the global layer exists, but at F1 (Help) → Settings, and F12 itself is contextual. The ruling was **not** pre-empted. ~~**This is now a two-wave-old blocker whose only cost is the user's ruling; it is the cheapest open item in this document.**~~ 🔴 **NO LONGER A BLOCKER — RULING 21 LANDED 2026-09-09 AND THE ROW IS OPEN FOR BUILDING UNDER ITS NEW TITLE.** Build target, stated so no track has to re-derive it: the **F1: Help → Settings** popup carrying the app-wide configuration items, plus the gate on the dead `F12` button at `src/Apex.Desktop/ViewModels/MainWindowViewModel.cs:11298` — the ruling named `:11245`; A12 re-measured it on `origin/main` `53d43a1` and the statement is `ButtonBar.Add(new ButtonBarItem("F12", "Configure", F12Configure));` at **11298**, the only `"F12"` button in the file, so the ruling's target is not in doubt and only the line number drifted. **T1-16 is reshaped by the same ruling.** ⚠️ **What is NOT authorised by ruling 21:** re-introducing a Tally.ERP 9 style global Configuration MENU under any name — the vendor removed it, and cloning a removed surface is inventing, not cloning. |
...
#### Area 2 — Accounting masters · 13 rows · 0 complete / 13 partial / 0 absent
...
| 2.2 | Group behavioural flags — sub-ledger, nett debit/credit, used for calculation, allocation method | PARTIAL | All four identifiers return zero hits over `src/`. `Group` carries only Id, Name, Nature, ParentId, Alias, IsPredefined and Gst. ~~⚠️ Tally's field names for these are themselves **UNVERIFIED** against this corpus (§6 item 6) — the absence from our code is confirmed; what Tally calls them is not.~~ 🔴 **CAVEAT DISCHARGED 2026-09-04 (§1.3 item 19). All four names are VENDOR-VERBATIM on the official Groups page — the guesses were right — and a FIFTH attested flag, *"Does it affect Gross Profits"*, joins this row.** ⚠️ **But note what blocks it by construction:** a new **PRIMARY** group cannot be created at all in our product (`ParentOptions` holds only existing groups; both `Create()` and `GroupService.CreateGroup` refuse a null parent), and *Nature of Group* / *Does it affect Gross Profits* live on the primary-group screen — so **this row cannot be closed without fixing that first** (= **T1-31**). In T2-3. 🟢 **MOVED `ABSENT` → `PARTIAL` 2026-09-08 BY A12 ON MERGE OF PR #90 (W-O1, schema v60) — THE GRADE IS A12'S, RECORDED FROM THE TRACK'S REPORT AND VERIFIED FIRST-HAND BEFORE THE MERGE.** All five vendor fields are captured on the Group master (Gateway → Masters → Create → Group; Ctrl+A accepts) and persist in five new `groups` columns. ⚠️ **AND T1-31 IS CLOSED BY THE SAME COMMIT — I confirmed the defect was real on `6e9b10c` first rather than taking the report's word: `GroupService.CreateGroup` threw *"A parent group (Under) is required"* on a null parent, so no primary group could be created and two of these fields were structurally unreachable.** `ParentOptions` now opens with *"◦ Primary (top-level)"*. 🔴 **PARTIAL AND NOT COMPLETE, FOR A REASON WORTH READING: only ONE of the five is read by anything.** `AffectsGrossProfits` reaches `ProfitAndLoss` (I mutation-verified that arm myself — deleting it reddens `A_custom_primary_head_marked_direct_lands_ABOVE_the_gross_profit_line`); `BehavesLikeSubLedger`, `NettBalancesForReporting` and `UsedForCalculation` are **captured-and-inert and say so on the property**, because the vendor page names each switch without specifying the resulting arithmetic, and `UsedForCalculation` is row 2.6's machinery, which does not exist. `PurchaseAllocationMethod` is stored but the apportionment still reads the Ledger one. |
...
| 2.4 | Voucher Type master — create / alter / display / delete | PARTIAL | 🔴 **STATE MOVED `ABSENT` → `PARTIAL` 2026-09-07 (A15, at `origin/main` `cb8cccb`, schema v56). THE ROW HAD BEEN STALE SINCE PR #52 AND EVERY SEARCH UNDER IT IS NOW FALSE.** The screen exists and a user can reach it. **ROUTE, WALKED RATHER THAN ASSERTED:** Gateway → Masters → **Create** → *Voucher Type* — the row is built in `MainWindowViewModel.BuildCreateColumn` and the label dispatch in `MainWindowViewModel.OnMenuItemActivated`'s `switch (item.Label)` sends it to `MainWindowViewModel.ShowVoucherTypeMaster`, which opens `VoucherTypeMasterViewModel` as a cascade page column; `MainWindow.axaml` renders it (the `ContentControl` bound to `VoucherTypeMaster`) with real `TextBox`/`ComboBox`/`CheckBox` controls, not a stub. **All four verbs are wired:** create = Ctrl+A (`VoucherTypeMasterViewModel.Create`, reached through `MainWindowViewModel`'s master-accept switch and the view's `OnCreateVoucherTypeClick`); alter = Ctrl+Enter on the existing-list row (`MainWindow.axaml.cs`'s `OnKeyDown` arm calling `MainWindowViewModel.AlterHighlightedVoucherTypeRow` → `VoucherTypeMasterViewModel.ForAlter`); display = the Existing list (`RefreshList`, five columns); delete = Alt+D (`MainWindowViewModel`'s delete switch, `Screen.VoucherTypeMaster => RequestDeleteMasterListRow()` → `IMasterListScreen.DeleteMaster` → `VoucherTypeService.Delete`). 🔴 **PARTIAL, NOT COMPLETE — AND THE FRACTION IS THE POINT.** The vendor's Voucher Type screen (help.tallysolutions.com, *"How to Use Voucher Types, Voucher Class, Voucher Type Security, and Voucher Numbering in TallyPrime"*, fetched 2026-09-07) carries roughly **twenty** fields in six blocks; **we ship seven of them** — Name, Type of Voucher, Abbreviation, Method of Voucher Numbering, *Activate this Voucher Type*, *Print voucher after saving*, *Provide narration for each ledger in voucher*. **NAMED MISSING PIECES, every one a zero-hit search over `src/`:** *Use effective dates for vouchers* · *Make this voucher type as 'Optional' by default* · *Enable default accounting allocations* · *Set/Alter additional numbering details* and *Prevent creating duplicate Voucher Nos* **on this screen** (they live only on the separate F12 numbering column, `VoucherNumberingConfigViewModel`) · the whole **Printing block** (*Default template to print*, *Default title to print*, *Default bank*, *Default jurisdiction*, *Set/alter declaration*, *Set/Alter Terms and Conditions*) · the whole **Sharing block** (*WhatsApp voucher after saving*, *Generate Payment link/QR code after saving voucher*) · and **Name of Class** (voucher classes do not exist in this product at all). *Allow zero-valued transactions* and *Track Additional Costs for Purchases* are settable elsewhere, never here. ▼ *Original 2026-08-18 evidence, kept because it is the record of what was true then and every clause of it is now false:* No `VoucherTypeMasterViewModel` among the ~110 view models; no Screen member; no Create-menu row; `MasterCreateKind` has no member for it. Exactly **one** of `VoucherType`'s ~20 configurable properties is settable anywhere in the UI (`TrackAdditionalCosts`, from the purchase-invoice screen). ⚠️ **Area assignment ambiguous** — two surveys named this capability, one under Accounting masters and one under Voucher behaviours. Counted **once, here**; area 5 carries an uncounted cross-reference. 🔴 **EVIDENCE CORRECTED 2026-09-04 (§1.3 item 19) — the grade `ABSENT` is right and the prose under it was FALSE AT HEAD.** The cell said ~~*"exactly **one** … is settable anywhere in the UI"*~~; `grep -n "AllowZeroValuedTransactions" src/Apex.Desktop/ViewModels/VoucherEntryViewModel.cs` returns **994/997/998** — a live get/set on the persisted type. **It is two on that screen and six across two screens.** **= T1-3.** |
...
| 2.6 | Voucher Class | PARTIAL | 🔴 **MOVED `ABSENT` → `PARTIAL` 2026-09-14 (PR #102, W-T2). THE GRADE IS A12's OWN, NOT A REVIEWER'S** — no independent reviewer saw this branch; it is graded on the build agent's evidence plus A12's own re-measurement, and it is deliberately the conservative reading. 🔴 **AND THE OLD EVIDENCE CELL WAS STALE IN EVERY CLAUSE, WHICH IS WHY THIS ROW SAT AT `ABSENT` LONGER THAN IT SHOULD HAVE.** It read ~~*"Zero `VoucherClass` hits in `src/Apex.Ledger` and zero in `src/Apex.Desktop`. No domain type, no persistence table, no view model, no Screen, no menu row."*~~ **Re-measured by A12 on `origin/main` at `9a83de6` before this PR was opened: five domain types (`VoucherClass`, `…AdditionalEntry`, `…CalculationType`, `…LedgerAllocation`, `…RoundingMethod`), three services (`…Posting`, `…Rounding`, `…Service`), the `voucher_class_ledger_allocations` table, and 38 `VoucherClass` hits in `VoucherTypeMasterViewModel` alone.** The **DEFINE** half shipped with schema **v62** (W-Q1) and nobody re-graded the row. **What was genuinely absent was the USE half, and that is what PR #102 built:** `SelectedVoucherClass` and `VoucherClassOptions` returned **zero** across `src/`, and all four `VoucherClassPosting` hits in `src/` were doc-comment cross-references (`<see cref>`/`<c>`) — **an engine with no caller and no picker.** **ROUTE, both halves keyboard-reachable.** *Define:* Gateway → Masters → Alter → Voucher Types → a type → *Name of Class* → *Default Accounting Allocations* / *Additional Accounting Entries*. *Use (new):* Gateway → Vouchers → `F8` Sales or `F9` Purchase → `Ctrl+H` for item-invoice mode → Tab to the **Voucher Class** picker in the invoice header. **EVIDENCE:** 22 VoucherClass tests green in the four-project gate, asserting on `Voucher.Lines` **after** `Accept` rather than on view-model flags; the picker proved by walking the **realised visual tree** of a real `MainWindow` and asserted `Focusable` **and** `IsEnabled`, because a control drawn but not focusable is skipped by Tab and unreachable for a keyboard-only operator. **10 mutations run, 10 killed, no survivors.** A dead inherited test was found and repaired: it called `Accept()`, which refuses every altering screen at its first line for an unrelated reason, so it never reached item-invoice code and **would have stayed green with the class guard deleted**. 🔴 **WHY PARTIAL AND NOT COMPLETE, CONCRETELY:** `VoucherClassCalculationType` has three members; the vendor also ships *On Item Rate* (its own help page) and *On VAT Rate*. Freight (per-unit, `BasedOnQuantity`) works and is tested, but **DISCOUNT — which a class expresses as a rate-based additional entry — cannot be expressed at all.** Also unreached: a class on an **accounting (service)** invoice (`AcceptAccountingInvoice` is a separate path and `ActiveVoucherClass` is gated on `IsItemInvoice` only), and the `F12` *"select common ledger account for Item allocation"* half of the vendor's field-hiding condition, which is not modelled anywhere — so our rule is **narrower** than the vendor's, correct at the vendor's shipped default but not equivalent. ⚠️ Same area ambiguity as 2.4; counted once here. |
...
| 2.12 | Multi-master create (Multi Ledger / Multi Group) | PARTIAL | 🔴 **STATE MOVED `ABSENT` → `PARTIAL` 2026-09-07 (A15, at `origin/main` `cb8cccb`). PR #54 BUILT IT AND NOBODY MOVED THE ROW.** **ROUTE:** Gateway → Masters → **Create** → section header *Multi Masters* → *Multi Ledger* / *Multi Group*, both built in `MainWindowViewModel.BuildCreateColumn` and dispatched by label to `MainWindowViewModel.ShowMultiLedgerCreate` / `ShowMultiGroupCreate` → `ShowMultiMasterCreate` → `MultiMasterCreateViewModel`, opened as a cascade page column and rendered in `MainWindow.axaml`. Ctrl+A commits the whole grid through `MultiMasterCreateViewModel.Accept` (all-or-nothing, a labelled divergence of ours). The header **Under Group** picker carries the vendor's **"All Items"** sentinel and the grid the vendor's own **Name of Ledger / Under / Opening Balance** columns. 🔴 **PARTIAL — THREE NAMED MISSING PIECES, each a zero-hit search over `src/`:** **(a) MULTI ALTER DOES NOT EXIST.** The vendor's Multi Masters offers *Multi Create* **and** *Multi Alter*; `grep -rniE "multi[- ]?alter" src/` returns exactly **one** hit and it is the grounding comment inside `MultiMasterCreateViewModel` recording that the verb exists in the reference product. **(b) TWO FAMILIES OF THE VENDOR'S SIX.** `MultiMasterKind` has only `Ledger` and `AccountGroup`; `Multi Stock Item`, `Multi Godown` and `Multi Cost Centre` return **zero**. **(c) THE Alt+H DOOR IS UNBOUND.** The vendor reaches Multi Masters with **Alt+H** from the Chart of Accounts; the only `Key.H` arm in `MainWindow.axaml.cs` is Ctrl+H change-mode, so our grids are reachable **only** by the Create-column cascade. That is a deliberate UI-contract choice recorded in the view model, not an oversight — but it is a divergence and it belongs in the cell. ▼ *Original evidence, superseded:* ~~Zero `Multi Ledger` / `MultiLedger` hits over `src/`.~~ 🔴 **GREP CORRECTED 2026-09-04 (§1.3 item 19) — the grade `ABSENT` is right and the search targeted a Tally.ERP 9 name the reference product no longer uses.** Replacement, which also returns zero: `grep -rniE "multi[- ]?master|multi create|multi alter|Alt\+H" src/ --include=*.cs --include=*.axaml`. The Create column contains only single-master rows and the label dispatch has no multi-create case. In T2-3. |
| 2.13 | Show Inactive / hidden masters | PARTIAL | 🔴 **STATE MOVED `ABSENT` → `PARTIAL` 2026-09-07 (A15, at `origin/main` `cb8cccb`). AND THE ROW'S TITLE WAS THE PROBLEM — THE VENDOR *DERIVES* THIS, IT DOES NOT *STORE* IT.** The build agent measured that help.tallysolutions.com's Chart of Accounts page carries **no** "Show Inactive" and no inactive-master status; the retrievable feature is **"View and Delete Multiple Unused Masters"**, reached by **Ctrl+J (Exception Reports) → "Show Unused"**, producing **"List of Ledgers (Unused)"**. So an `is_active` column on every master would have been an invention, and none was added (v54 carries nothing for this row). **ROUTE, WALKED:** Gateway → **Chart of Accounts** → **Ctrl+J** — the arm is in `MainWindow.axaml.cs`'s `OnKeyDown`, guarded on `MainWindowViewModel.IsChartOfAccountsScreen` (the same predicate `OpenExceptionReports` refuses on, so the chord is never swallowed for no effect) → `MainWindowViewModel.OpenExceptionReports` → `ExceptionReportsViewModel`, rendered in `MainWindow.axaml` as two radios, *All Masters* / **Show Unused**; choosing the latter sets `ChartOfAccountsViewModel.ShowUnusedOnly`, which re-titles the tree to `UnusedMasters.UnusedLedgersCaption` and filters it through `UnusedMasters.IsLedgerUnused`. **The predicate is real and is written in `src/`, not in a test:** `UnusedMasters.IsLedgerUnused` is the exact complement of BOTH deletion counters (`MasterDeletionRules.CountLedgerTransactions` and `IsLedgerNamedByAnotherMaster`), whole-book scope with no date parameter. 🔴 **PARTIAL — TWO NAMED MISSING PIECES:** **(a) LEDGERS ONLY.** The vendor documents *"Delete Unused Stock Items at once"* beside the ledger arm; it is not built, and the view model's own remarks say why — there is **no stock-item list screen** to host it (`ChartOfAccountsViewModel` walks `Groups` and `Ledgers` only; `StockItemMasterViewModel` is a create/alter form, not a browsable tree). **(b) VIEW-ONLY.** The vendor's feature is *view **and delete***; our pane deletes nothing. **The voucher-type half of the Show-Inactive gesture is counted at 5.11, not here**, and `VoucherType.IsActive` remains the only active flag in this domain. ▼ *Original evidence, superseded:* Zero `Show Inactive` / `ShowInactive` hits over `src/` except one comment in `VoucherTypeResolver.cs` recording that the gesture "meant nothing". Every master's Existing list is an unconditional enumeration. Overlaps the Show-Inactive element of 5.11; the master-level capability is counted here, the voucher-type flag there. |

#### Area 3 — Inventory masters · 15 rows · 2 complete / 13 partial / 0 absent
...
| 3.6 | Alternate units per stock item | PARTIAL | ~~Zero `AlternateUnit` / "Alternate Unit" hits over `src/`. `StockItem` carries a single base unit and `VoucherInventoryLine` has no alternate-unit quantity.~~ In T2-3. 🟢 **MOVED `ABSENT` → `PARTIAL` 2026-09-08 BY A12 ON MERGE OF PR #90 (W-O1, schema v60) — THE GRADE IS A12'S, RECORDED FROM THE TRACK'S REPORT.** Gateway → Masters → Create → Inventory Masters → Stock Item → *"Alternate units"*; choosing a unit reveals *"Base units per 1 alternate"* and a live *"1 Box = 10 Nos"* sentence. Two new `stock_items` columns; Alter reaches it too. Vendor: `help.tallysolutions.com/manage-stock-item-tally/`. 🔴 **THE DESIGN DECISION IS THE LOAD-BEARING PART: THE BASE UNIT IS STORED AND THE ALTERNATE IS ALWAYS DERIVED**, through the single authority `AlternateUnitConversion` — so no stock table gained a quantity column, no second quantity exists, and *"a report and an invoice disagree"* is impossible by construction rather than by test. **PARTIAL because the MASTER is complete and the VOUCHER side is not — a line cannot yet be keyed in the alternate unit.** The seam for that is `InventoryVoucherLineViewModel.UnitOptions` / `ParsedQuantityInBaseUnit`, which already does exactly this for compound units; the only new work is that an alternate unit's factor lives on the ITEM, not the Unit. Deliberately deferred: it is a posting path. |
...
#### Area 5 — Voucher behaviours & edit verbs · 15 rows · 6 complete / 9 partial / 0 absent
...
| 5.5 | Insert Voucher (Alt+I) | PARTIAL | 🔴 **CLAIMED BY A TRACK ON 2026-09-06 AND NOT BUILT — THE GRADE DOES NOT MOVE.** `claude/apex-f2-verbs-limits` (PR #62) carried this row in its brief; the first-time review found **zero lines of its three-dot diff touch it**, and A12 confirmed that before landing the branch for its schema-v54 half. The two comments that discuss the row (~~`MainWindowViewModel.cs:6629-6637`~~ → the `<remarks>` on `MainWindowViewModel.RequestDuplicateHighlightedVoucher`, and ~~`MainWindow.axaml.cs:349-356`~~ → the `🔴 ITS SIBLING IS NOT BUILT` block above the `Key.D2` arm in `MainWindow.axaml.cs`'s `OnKeyDown`) were **already on main** before that branch and are not its work; 🔴 **BOTH LINE CITATIONS WENT STALE AND ARE REPLACED WITH MEMBER NAMES, 2026-09-07 (A15).** Re-checked at `cb8cccb`: `MainWindowViewModel.cs:6629-6637` is now a run of `= null` screen resets and says nothing about Insert Voucher — the note moved down ~1,200 lines as the file grew. **This is the third time a live line citation in this document has rotted**, which is exactly why §1.2a's conventions say *"evidence is written as `file` + `member`, never as a live line citation"*; the rule is now followed here. they state, correctly, that `Alt+I` is spent on the POS tender-mode toggle and that insert-renumbering collides with the IRN/challan freezes in `VoucherAlterationEligibility` — *"needs a user ruling, not a keystroke"*. **Recorded here rather than left to the branch's own summary, because an over-claimed row is the failure mode this census exists to catch** (cf. the 2026-09-04 three-rows-wrong-about-themselves entry). ▼ *Original evidence, unchanged and still true:* Alt+I is spent on the POS tender-mode toggle. No insert-at-position code of any kind, no Screen member, no menu row. Corpus-attested and not built. 🔴 **RE-MEASURED 2026-09-07 (A15) AT `cb8cccb` — STILL ABSENT, AND SAYING SO IS THE POINT.** This row was on A15's own candidate list of "eight rows that shipped weeks ago"; it had not. `grep -rniE "insert.{0,4}voucher\|InsertVoucher" src/` returns only `SqliteCompanyStore`'s `InsertVoucher*` persistence writers — a different sense of the word — and the only `Key.I` arm in `MainWindow.axaml.cs` is still the POS tender-mode toggle scoped to `Screen.PosBilling`. **The grade does not move. A stale ABSENT costs less than a wrong PARTIAL.** ▼ 🟢 **GRADED UP 2026-09-08 BY A12 ON MERGE OF PR #87 (wave N) — AND THIS TIME CODE WAS WRITTEN.** Everything above stands as the record of the false claim; this is the opposite case, and A12 verified it first-hand before landing rather than inheriting the track's own summary. **Route:** Gateway → Reports → Day Book → highlight the row to insert *above* → **Alt+I** → the *Insert Voucher* picker opens as a cascade column with the Day Book still live beneath → pick a voucher type → key the entry → Ctrl+A; advertised on the button bar as *"Alt+I / Insert Vch"*, enabled on the Day Book only. **Engine:** `VoucherInsertion.Plan` computes refusal, inserted number, renumber list and method *before* anything is posted; `Apply` reads the immutable plan and re-derives nothing. **The shell chain was read out end to end** — `ShellChordTable` arm → `MainWindowViewModel.RequestInsertVoucherAtHighlight` → `Plan` → `ApplyPendingInsertion` → `Apply`, plus the button-bar arm — so this is **not** another writer-only feature of the kind this document has filed three times. 🔴 **A12 MUTATION-VERIFIED THE ARBITRATION RATHER THAN TRUSTING IT:** dropping `IsDayBookReport` from the Alt+I predicate (i.e. letting Insert swallow the chord app-wide) reddened **exactly** `AltI_on_the_POS_screen_still_toggles_the_tender_mode` with `Assert.NotEqual() Failure: Values are equal`, 6 of 7 still green; the file was then restored byte-identical with an unchanged CR count of 273. **WHY THIS IS NOT COMPLETE:** the vendor's per-voucher-type *"Numbering behaviour on the insertion/deletion"* switch (Renumber Vouchers vs Retain Original Voucher No.) is a persisted `voucher_types` column, i.e. a migration, and was deliberately NOT taken — only the vendor's stated **default** is implemented, so the alternative cannot be selected at all. **AND THE U-6 CHORD RULING IS STILL OWED:** the row ships *scoped out of* POS rather than owning Alt+I outright, and the mutation above is the proof that scoping is load-bearing rather than decorative. |
...
| 5.10 | Voucher numbering method **Manual** / **None** | PARTIAL | 🔴 **STATE MOVED `ABSENT` → `PARTIAL` 2026-09-07 (A15, at `origin/main` `cb8cccb`). PR #52 CLOSED EVERY GAP THE CELL BELOW NAMES, AND OPENED ONE THE CELL DID NOT KNOW ABOUT.** **What ships:** `NumberingMethod` now has **all five** vendor-attested members (`Automatic`, `Manual`, `None`, `AutomaticManualOverride`, `MultiUserAuto` — the last two APPENDED at ordinals 3/4 because the ordinal is persisted in `voucher_types.numbering`). **The method is SETTABLE by a user:** the picker is `VoucherTypeMasterViewModel.NumberingMethods` / `SelectedNumbering`, rendered as a real `ComboBox` in `MainWindow.axaml`'s Voucher-Type page, reached by row 2.4's route, and committed through `VoucherTypeService.Create` / `Alter`. **The engine honours it end to end:** both post paths ask `VoucherType.AssignsNumberAutomatically` (never `== NumberingMethod.Automatic`, which would have left the two new methods unnumbered), and `VoucherNumberFormatter.Render` returns `""` under `None`. **The Voucher No. is now typeable** where it must be: `VoucherType.AllowsManualNumberEntry` drives `IsVoucherNumberEditable` on **all four** entry view models (`VoucherEntryViewModel`, `InventoryVoucherEntryViewModel`, `MaterialMovementEntryViewModel`, `JobWorkOrderEntryViewModel`), and `MainWindow.axaml` swaps the read-only `<Run>` for a bound `TextBox` at **four** places, one per screen — so the picker is not a label over a broken book. 🔴 **PARTIAL, ON A GAP THE OLD CELL COULD NOT SEE: `Voucher.Number` IS AN `int`.** The vendor's Manual method *"does not check for the sequence of the numbers and **permits you to specify anything that you wish in the voucher number field**"* — i.e. arbitrary alphanumeric text. Ours accepts digits only, so a book that numbers `INV/2026/A-17` by hand cannot be kept in this product. That is a domain + schema change, not a widget change, and it is named here rather than folded away. **Second named piece, already labelled as ours in `NumberingMethod`:** `AutomaticManualOverride` and `MultiUserAuto` are persisted operator CHOICES with **no distinct engine behaviour** — at the engine level all three auto methods are indistinguishable today. ▼ *Original evidence, superseded — every clause of the first three sentences is now false:* The method display is a get-only string, self-described "DISPLAY-ONLY this slice"; there is no setter and no bound control. The Voucher No. on all four entry screens is a `<Run>` inside a `TextBlock`, not a TextBox. The seed hard-codes Automatic throughout. **= T1-5.** 🔴 **THE ROW UNDERSTATES THE GAP, MEASURED 2026-09-04 (wave-2 core-accounting pass) — AND THIS RESIZES THE WORK, NOT JUST THE WORDS.** The official *"Method of Voucher Numbering"* field offers **five** methods (vendor page, verbatim): *Automatic* · **Automatic (Manual Override)** · *Manual* · **Multi-User Auto** · *None*. `src/Apex.Ledger/Domain/NumberingMethod.cs` has **exactly three** members — `Automatic`, `Manual`, `None` — so **two attested methods do not merely lack a picker, they have NO DOMAIN MEMBER AT ALL.** Building this row is therefore an **enum + persistence + migration** change, not "add a picker", and row 2.5's `MethodDisplay` gap inherits the same correction. ⚠️ **And one negative result recorded because it prevented a false divergence:** `None` **IS** attested — the Voucher Type master page omits it, the dedicated numbering-methods page carries it (*"Select this option to disable the voucher numbering"*), and stopping at the first page would have filed `None` as ours. |
| 5.11 | Voucher-type user flags — Use Common Narration, Print after saving, Show Inactive → activate | PARTIAL | 🔴 **STATE MOVED `ABSENT` → `PARTIAL` 2026-09-07 (A15, at `origin/main` `cb8cccb`) — AND THE HONEST PART IS THAT ONLY *ONE* OF THE THREE FLAGS ACTUALLY DOES ANYTHING.** All three are now settable: `MainWindow.axaml`'s Voucher-Type page carries three real `CheckBox`es — *Activate this Voucher Type*, *Print voucher after saving*, *Provide narration for each ledger in voucher* (the vendor's own caption; *"Use Common Narration"* is the Tally.ERP 9 name for the complement) — bound to `VoucherTypeMasterViewModel.IsActive` / `PrintAfterSaving` / `ProvideNarrationForEachLedger`, committed through `VoucherTypeService.Create` / `Alter`, persisted by `SqliteCompanyStore.InsertVoucherTypes` (v53 columns) and round-tripped by `CanonicalXml` / `CanonicalMapper` / `ImportPlan`. **THE SHOW-INACTIVE HALF IS REAL AND REACHABLE END TO END:** `VoucherTypeMasterViewModel.ShowInactive` un-hides deactivated types in the existing list, Space on the highlighted row runs `ToggleActiveOnHighlighted` → `VoucherTypeService.SetActive`, and `VoucherType.IsActive` is genuinely CONSUMED downstream — `VoucherTypeResolver` skips inactive types and `MainWindowViewModel`'s Day-Book add-list filters on it — so the seeded-inactive payroll types can finally be switched on and posted. Before this screen, `IsActive` had **no write route in the product at all** except `JobWorkService`. 🔴 **PARTIAL, AND THIS IS A NEW DEFECT A15 FOUND RATHER THAN A KNOWN GAP: THE OTHER TWO FLAGS ARE WRITE-ONLY.** `grep -rn "PrintAfterSaving\|ProvideNarrationForEachLedger" src/` returns **setters, persistence and export only — not one reader that changes behaviour.** Nothing on any save path opens the Voucher Printing screen when *Print voucher after saving* is ticked, and no entry screen grows a per-ledger narration field when *Provide narration for each ledger* is ticked. **They are checkboxes the operator can tick that do nothing**, which is the same dead-capability shape this document has now filed three times. Two of three flags therefore ship as configuration with no effect; the row stays open on them. ▼ *Original evidence, superseded:* `VoucherType` has no common-narration and no print-after-saving member. "Show Inactive" returns exactly one hit in `src/`, a comment recording that the gesture meant nothing. The two inactive families are flipped only by `JobWorkService`; the other write site is a rollback restore inside a catch, not an activation route. |
...
#### Area 6 — Statutory, current law (GST, TDS/TCS, salary IT) · 42 rows · 18 complete / 23 partial / 1 absent
...
| 6.10 | GSTR-1 / GSTR-3B portal JSON — the artefact that actually gets filed | PARTIAL | 🔴 **CORRECTED 2026-09-07 BY A15. THE `ABSENT` GRADE AND EVERY CLAUSE OF ITS EVIDENCE ARE FALSE AT `origin/main` (`90413a9`), AND THIS ROW IS ONE OF THE TWO THE 2026-09-05 BANNER SAID WOULD MOVE THE MOMENT THE b2 BRANCH LANDED — IT HAS LANDED AND NOBODY MOVED THE ROW.** Measured first-hand, not copied: `Apex.Ledger.Io/GstReturnJson.cs` now exposes **seven** writers, and two of them are the ones this row is about — `Gstr1` (`:221`) and `Gstr3b` (`:306`). The class is **not** callerless: `GstOfflineReturnsViewModel.cs:538-544` dispatches all seven, and a Regular dealer is **offered GSTR-1 and GSTR-3B by name** (`GstOfflineReturnsViewModel.cs:208-209`) from the "Offline Return Files (JSON)" row that `BuildGstAdvancedReturnsColumn` puts on screen (`MainWindowViewModel.cs:2496`) and `MainWindowViewModel.cs:9682` dispatches. Type, route and caller all present — the census's own `COMPLETE`/`ABSENT` test. **PARTIAL, not COMPLETE, and deliberately the conservative reading:** what is proven here is that a JSON artefact is produced and reachable, **not** that it validates against the portal's current offline-tool schema — no source in R7's order has been compared for the emitted shape, so this row must not be read as "filable". ⚠️ **Grade assigned by A15 on a read-only pass, by no reviewer and no fixer; owed to the user for confirmation.** *Original evidence, kept as the record of what was measured before b2 landed:* ~~"The JSON writer class exposes exactly five writers (CMP-08, GSTR-4, 9, 9A, 9C) and **no GSTR-1 or 3B emitter anywhere**; the class itself has **zero production callers** — the only references in `src/` are two doc comments."~~ **= T1-11**, which should be re-walked against this grade. |
...
| 6.13 | GSTR-9A (composition annual return) | PARTIAL | 🔴 **CORRECTED 2026-09-07 BY A15. THE `ABSENT` GRADE IS FALSE AT `origin/main` (`90413a9`), AND THE EVIDENCE CELL NAMES A SEARCH THAT NOW RETURNS THE OPPOSITE. THIS IS THE SECOND OF THE TWO ROWS THE 2026-09-05 BANNER HELD BACK FOR THE b2 BRANCH; b2 LANDED AND NOBODY MOVED IT.** Measured first-hand: `BuildCompositionReturnsColumn` puts a real **"GSTR-9A"** menu row on screen (`MainWindowViewModel.cs:2565`) — the source comment there names *"census rows 6.13 / 6.10"* outright — reached by `ShowCompositionReturnsMenu` for a Composition dealer; `MainWindowViewModel.cs:9681` dispatches that label to `OpenGstOfflineReturns(GstOfflineReturnKind.Gstr9a)`; the page offers it by name with its own distinct description (`GstOfflineReturnsViewModel.cs:227`), projects it through `ProjectGstr9a` → `Gstr4.cs:162 Gstr9a.Build`, and emits it through `GstReturnJson.Gstr9a`. So *"no menu label case"* is now the single most wrong clause in this document. **PARTIAL, not COMPLETE, on the row's own terms:** GSTR-9A is served by the shared offline-returns picker rather than its own screen, and the product itself labels it *"reconciliation only, see the note"* — the portal no longer accepts the form, which is why `Gstr9aApplicabilityText` exists. ⚠️ **Grade assigned by A15 on a read-only pass; owed to the user for confirmation.** *Original evidence, kept:* ~~"The only `Gstr9a` hits are engine-side: an uncalled JSON writer and two report files that mention it. No Screen member, no view model, no menu label case."~~ |
...
| 6.20 | DRC-03 voluntary payment / demand discharge | PARTIAL | 🔴 **BUILT AND LANDED 2026-09-06 — PR #63 (`claude/apex-d3-gst-longtail`). Was ABSENT, and this row is the T2-9 pattern finally closing: the verb was complete and had no door.** Shipped: `Screen.Drc03Payment`, `Drc03PaymentViewModel`, `OpenDrc03Payment()`. The Others/please-specify sub-flow is covered end to end across **all twelve** causes — the blank-specify refusal files nothing, the stated cause actually reaches `GstDrc03.Cause`, and the text is cleared on leaving cause 11 so it can never be appended to a different cause. Unreadable cash cells now render the constant `not read` with a banner naming the failing cell and quoting the exception, never a `0.00` an operator would read as a nil balance; making that branch exercisable required a cash-reader seam, because `GstDepositService.AvailableCash` cannot be made to throw from a `Company` — which is exactly how a wrong answer there would have shipped unexercised. ⚠️ **Grade assigned by A12, not by the track** — the fix report described the work and stated no grade. **Gap holding it below COMPLETE:** no export or print path on the panel (the DRC-03 itself is still not emitted as a filable artefact), and no live portal submission. ▼ *Original ABSENT evidence, kept because it is what the row moved FROM:* 🔴 The **engine verb exists and is complete** — a deposit-service posting method with its own record type — ~~*"and `Drc03` returns **zero hits across all of `src/Apex.Desktop`**"*~~ 🔴 **THAT CLAUSE IS FALSE AND WAS CORRECTED 2026-09-04 (wave-2 breadth pass); THE GRADE IS RIGHT AND MUST NOT CHANGE.** There are two hits, both in one file: `src/Apex.Desktop/ViewModels/VoucherAlterationEligibility.cs:305` (a doc comment) and `:312` (`|| company.GstDrc03s.Any(d => d.VoucherId == id)`). That is a **read-only alteration guard** — it refuses to amend a voucher linked to a DRC-03 — **not a creation route**. There is still no Screen member, no view model and no menu case, so `ABSENT` stands on the census's own test. Reachable only by JSON/XML import. Filed as **T2-9.** ⚠️ **Recorded rather than quietly fixed because this is the exact defect the census already had to correct once, on row 3.13: an evidence sentence falsifiable by one grep invites a reader to distrust the row's grade, which here is correct.** |
...
| 6.23 | Multiple GSTIN registrations for one company (branch / second-State) | PARTIAL | 🔴 **MOVED `ABSENT` → `PARTIAL` 2026-09-09 (W-P1, PR #93, schema v61).** ~~*"Zero `MultiGstin` / `AdditionalGstin` / `BranchGstin` hits in any of the four projects; the config carries exactly one GSTIN field. In T2-2."*~~ **What now exists:** the `GstRegistration` master (`src/Apex.Ledger/Domain/GstRegistration.cs`) with its set-level rules (no two registrations in one State, no shared GSTIN); `GstConfig.AdditionalRegistrations` / `PrimaryRegistration` / `IsMultiRegistration`; `Voucher.GstRegistrationId` (the vendor's F3 Company/Tax Registration attribution); `GstReportSupport.EnsureRegistrationScoped`, which **refuses** to yield anything for a multi-registration company that names no registration, threaded through GSTR-1/3B/4/9/9A/9C, CMP-08, TaxAnalysis, ItcGateView, KeralaFloodCess, CompositionTaxService and Gstr2bReconciler; a Create-menu row **driven to by ArrowDown/Enter through MainWindow's own key handler** in test, not read off a view model; and a registration picker on the filing screen. **WHY IT IS NOT `COMPLETE`, AND THIS IS THE LOAD-BEARING HALF:** **no voucher-entry F3 selector writes `GstRegistrationId`** — the column persists and round-trips but nothing in the UI sets it, so **a second registration's returns are structurally empty** (`VoucherEntryViewModel`, 6805 lines, six `new Voucher(` sites plus the edit/replacement paths). Second, the **Electronic Ledgers / ITC pools remain company-wide** — scoping the movement without scoping the closing balance would be internally inconsistent, and per-registration pools need a separate set of Input/Output tax ledgers that no retrieved vendor page settles. In T2-2. |
| 6.24 | Input Service Distributor (ISD) — credit distribution and the ISD return | ABSENT | The only `Isd` token in `src/` is a GSTR-2B inbound document-type enum member and the parser branch that reads it. No distribution service, no ISD invoice, no ISD return, no Screen, no menu row. 🔴 **RE-CONFIRMED `ABSENT` 2026-09-09 (W-P1, PR #93) — HELD DELIBERATELY WHILE THE TWO ROWS BESIDE IT MOVED.** ISD is credit **distribution** across registrations; it was sequenced behind 6.23 and 6.23 landed `PARTIAL` with no voucher-side registration attribution, so there is no distribution flow for an ISD return to project. Building it now would have fabricated the flow. **A stale `ABSENT` costs less than a wrong `PARTIAL`** — and an invented one here would be a figure filed with a government portal. In T2-2. |
| 6.25 | GST Classification / Nature-of-Transaction master | PARTIAL | 🔴 **MOVED `ABSENT` → `PARTIAL` 2026-09-09 (W-P1, PR #93, schema v61).** ~~*"No such master exists. The ledger GST classification is an engine-managed value object explicitly excluded from user editing. No Screen member, no view model, no Create-menu row. In T2-3."*~~ **The old evidence was re-measured, not assumed stale:** the pre-existing `GstClassification` hits are all the engine-managed `LedgerGstClassification` value object, a different thing, so this is a genuinely new master — `src/Apex.Ledger/Domain/GstClassification.cs`, `GstConfig.Classifications`, the vendor's two sections, `ToDetails()` copy-at-assignment semantics, and Central/State derived as halves of the integrated rate with the odd basis point on the Central half so CGST+SGST == IGST exactly. Reachable at Masters → Create → Statutory Masters → GST Classification, driven to by key gesture in test. **WHY IT IS NOT `COMPLETE`:** the vendor **assigns** a classification via *GST Rate Setup → Alt+S (Set Rate)* over a selected item/ledger list, and **that host screen does not exist here** — the screen of that name in this app (`GstRateSetupViewModel`, 445 lines) is the dated rate-history / cess-window editor, a different thing. The copy is built and tested; the assignment has nowhere to live yet. 🔴 The master is deliberately **NOT** wired in as a live sixth rate-hierarchy level — a test asserts that creating a classification which contradicts a stock item leaves that item's resolved rate at 18%, because the alternative silently re-rates every future invoice on an existing book as a side effect of creating a master. In T2-3. |
| 6.26 | Kerala Flood Cess | PARTIAL | 🔴 **BUILT AND LANDED 2026-09-06 — PR #60 (`claude/apex-f4-gst-tds-tail`), merged at `455efcc`. Was ABSENT; the ten-row gate that withheld it was lifted by USER RULING 15.** Shipped: `Apex.Ledger/Services/KeralaFloodCess.cs` (a pure, clock-free leviability predicate and rate engine), `Apex.Ledger/Reports/KeralaFloodCessReturn.cs` (the Q8 turnover-by-GST-rate return projection) and a Desktop route + view model, with three test files. **The statutory grounding warning carried on the rescue commit is CLOSED**, not carried forward: every figure now cites a PRIMARY Kerala source read by content — the Kerala GST Department FAQ, S.R.O. 436/2019 and S.R.O. 360/2017 on `keralataxes.gov.in` — and the ONE inference (cessation **31-07-2021**, being commencement 01-08-2019 plus the two years of FAQ Q2/Q5) is **labelled as derived** with the single line that would move it named. The levy is date-windowed, so a supply dated today computes ₹0 **by construction**, pinned by test. **Why PARTIAL and not COMPLETE — two boundaries the branch stated rather than faked:** (1) *"in furtherance of business"* (FAQ Q12/Q18/Q19) is a **caller parameter, because no master or voucher in this book records it** — persisting the exception needs a column, i.e. a migration this work does not have and did not fake; the return takes the ordinary reading and says so on the screen. (2) The cess is a **report/return projection only — it is NOT posted onto vouchers and does not appear as a separate invoice line**, which FAQ Q16/Q17 require. It is also deliberately ring-fenced away from the Compensation-Cess ledgers (a Central head), which would have misstated a Central return with a State levy. **A12 records the grade; no reviewer issued a letter grade for this row — the review left it untouched behind the then-live gate.** ⚠️ The 2026-08-18 §3 recommendation below is superseded in substance: **the lapse is now verified** against S.R.O./FAQ text, and the row was BUILT rather than struck, so §3 is moot for it. *Prior measurement, kept: zero `flood` hits over `src/`, the only `Kerala` hit a state-name row in `IndianState.cs`, no cess type, no rate, no computation.* |
...
| 6.35 | The TDS long tail — ~14 further sections, Form 27Q, §197 lower/nil certificates, §234E, Form 16B / 26QB | PARTIAL | 🔴 **STILL ABSENT — RE-CONFIRMED 2026-09-06, and a branch NAME must not be read as coverage.** ⚠️ **AND THAT BRANCH HAS NOW LANDED: PR #60 merged at `455efcc` on 2026-09-06 carrying ZERO TDS files.** A reader who sees `apex-f4-gst-tds-tail` in the merge history must not read it as closing this row — **A12 re-verified the merged tip, not the report. NOT STARTED, grade unchanged by the merge.** The branch `claude/apex-f4-gst-tds-tail` is named for this row and **contains none of it**. Verified by three-dot diff: it touches 11 files, all Kerala Flood Cess plus shell wiring, and `git diff --name-only origin/main...HEAD | grep -iE 'tds\|27q\|form16\|26qb\|natureofpayment\|197\|234e'` returns **nothing**. No TDS files, no nature-of-payment seeds, no Form 27Q, no §197 register, no §234E, no 16B/26QB. **NOT STARTED.** *Prior evidence, unchanged:* Per-term greps return zero for each section identifier; the two hits for "27Q" are a GSTIN inside a comment and a form-number rename entry; §197 exists only as an always-null field with **no writer anywhere**. **In T2-2.** 🟢 **MOVED `ABSENT` → `PARTIAL` 2026-09-08 BY A12 ON MERGE OF PR #91 (W-O3) — THE GRADE IS A12'S, RECORDED FROM THE TRACK'S REPORT.** **THREE sections now ship — §194T, §194R, §194S — and no schema was taken** (every field used already persisted and round-tripped). Reachable at Gateway → Create → Statutory Masters → Nature of Payment, and withheld by the real engine on a voucher, not merely listed. 🔴 **THREE AND NOT FOURTEEN IS THE FINDING, NOT THE SHORTFALL, AND THE REST OF THIS ROW IS STILL `ABSENT` — Form 27Q, §197, §234E and Form 16B/26QB ARE ALL UNSTARTED.** Fifteen candidates went to the bare Act on `incometaxindia.gov.in`; twelve failed one of four gates and are now a **machine-checked rejection register**, so a later pass cannot quietly seed one without clearing its gate. The gates, each of which cost a real candidate: **(G1)** the Act must state the rate — §193/§194/§194B/§194BB/§194D say *"at the rates in force"*, which lives in the Finance Act's First Schedule and is not retrievable, so seeding them would **deepen** the §194A citation gap this file is already flagged for rather than close it; **(G2)** Act text must agree with the Department's chart, else the slug is serving archived text and neither source is trustworthy — §194-O, §194G and §194K (the slug serves the version *omitted in 1999*) all failed here; **(G3)** the threshold limb must read *"does not exceed"*, because `TdsService.ThresholdCrossed` tests strictly greater — §192A and §194EE deduct **at** the figure and a seeded row would **under-deduct on the boundary rupee**; **(G4)** the deductor must plausibly be this product's user — §194N, §194D/§194DA and §194K make an SMB the deductee. ⚠️ **THE REACHABILITY TRAP WAS CAUGHT AND FIXED RATHER THAN SHIPPED:** `EnableTds` seeds only when the nature list is empty, so a new section would have reached **new companies only, for ever**, with re-running F11 doing nothing — the exact shape of the three dead features already filed here. An explicit *Add Missing Predefined* action (button + Ctrl+U, scoped to that one screen) is the fix, and it never overwrites an operator's hand-created row. 🔴 **AND IT SURFACED A PRE-EXISTING DEFECT, REPORTED NOT PAPERED OVER: the notified Form 26Q spells the §194-I codes `4-IA`/`4-IB` WITH A HYPHEN, where this seed has `4IA`/`4IB`.** `FvuWriter`, `Form26Q` and `Form16APdf` all emit that code into a **filed return**. It was deliberately NOT corrected here — the code is persisted per nature, so fixing the seed alone would split the population between old and new books; it needs a migration. ~~**This is owed to the user as an open item.**~~ 🟢 **CLOSED 2026-09-14 BY PR #98 (W-P2), AND WITHOUT THE MIGRATION THE SENTENCE ABOVE THOUGHT IT NEEDED.** The split-population problem was solved at the **point of emission** instead: `NatureOfPayment.NotifiedFvuSectionCode` normalises both spellings, and `Form26Q.cs:294` — verified by reading to be the **single** choke point, since Form 16A derives its rows from `Return26Q.Deductees` and `FvuWriter`, `Form16APdf`, `Form16AViewModel` and `Form26QViewModel` all read Form26Q/Form16A rows — emits the notified code. The stored value is untouched and every reader is tolerant, so legacy `4IA` books keep working and file correctly. A12 mutation-proved it on the merged tree by reverting that one expression to `nature?.FvuSectionCode`: **3 failed / 2739 passed**, all three the LEGACY-BOOK cases, which is what proves the tests exercise the emission normaliser rather than a corrected seed constant. The notified form was fetched first-hand (1,019,024 bytes); its Note 16 reads verbatim `194-I(a) Rent 4-IA` and `194-I (b) Rent 4-IB`, footnoted *"Inserted by the IT (Seventh Amdt.) Rules, 2025, w.e.f. 27-3-2025"*, and every other FVU code in the seed is confirmed in that same table. 🟢 **THE SAME PR CLEARED GATE G3 ABOVE:** §192A and §194EE now carry an inclusive-boundary flag, so neither under-deducts on the boundary rupee, and a neighbour test proves §192, §192B, §194E and §194EEA do **not** inherit it through a loose `StartsWith`. The independent re-audit found **no live wrong-money defect** — no shipped section is exclusive where the statute is inclusive. 🔴 **THE GRADE IS HELD AT `PARTIAL` AND DID NOT MOVE.** Form 27Q, §197, §234E and Form 16B/26QB are still unstarted, and they are most of what this row is named for. ⚠️ **THE W-P2 TRACK RETURNED NO REPORT AT ALL THIS WAVE** — the evidence above is the branch's own recorded verification at `ba28cf3` (five mutants, each with its failure count) plus A12's own re-measurement on the merged tree; it is not transcribed from a track report, because there was none. |
...
| 6.42 | Form 12BA (statement of perquisites) | PARTIAL | 🔴 **BUILT AND LANDED 2026-09-06 — PR #63 (`claude/apex-d3-gst-longtail`). Was ABSENT.** Shipped: `Screen.Form12Ba`, `Form12BaViewModel`, `OpenForm12Ba()` and the rule 26A(2)(b) threshold verdict driven from the employee's real §192 gross. The boundary fixture is **anchored**, not merely present — three rows at Rs.1,49,999 / Rs.1,50,000 / Rs.1,50,001, each asserting `Certificate.PartB.GrossSalary.Amount` **before** the verdict, so a fixture that drifted by a rupee can no longer leave a dead guard reading exactly like a live one. Where there is no §192 activity the screen states an **unmeasured** gross and gives no verdict in either direction, rather than rendering a zero and concluding "not required" from it. ⚠️ **Grade assigned by A12, not by the track** — the fix report described the work and stated no grade. **Gap holding it below COMPLETE:** **no output of any kind** — the view model has no export or PDF path and is not a report page, so the statement cannot be printed or handed to an employee, which is the whole point of a Form 12BA. That is the same *"output dead end"* gap already recorded on rows 6.11, 6.12 and 6.19. ▼ *Original ABSENT evidence, kept because it is what the row moved FROM:* Exactly one `12BA` hit in `src/` and it is a form-number rename entry in the vocabulary map. No report file, no Screen member, no view model, no menu case. In T2-2. |

#### Area 7 — Payroll · 26 rows · 6 complete / 20 partial / 0 absent

> **🔴 THE OLD TABLE ALLOWED THIS AREA ZERO ABSENT CAPABILITIES TOO.** Five are evidenced below.
>
> **▶ 🔴 FIVE ROWS — 7.22 THROUGH 7.26 — WERE ADDED 2026-09-07 BY USER RULING 19 (R12, `plan.md` §5,
> `TWO FURTHER USER RULINGS (R12, 2026-09-07)`). THEY HAD NO ROW AT ALL, WHICH IS THE ONE DEFECT NEITHER
> §1.2's ARITHMETIC NOR THE DRIFT LOCK CAN SEE.** Every check in this document reconciles the §1.2 table, the
> area headings and the published `awk` **against the rows** — so a vendor capability with no row is invisible
> to all four surfaces at once, and it flatters us in **both** terms: it is missing from the numerator *and*
> from the denominator. All five are **ABSENT**, each on a named zero-hit search re-run first-hand on
> 2026-09-07 over `src/`, and each cites the vendor page that attests it. **The area's denominator moves
> `21 → 26`; §1.2's row and the TOTAL were re-derived by re-running the `awk`, never edited.**
...
| 7.18 | NPS pay head (employee §80CCD(1B) / employer §80CCD(2) as a payroll component) | PARTIAL | No NPS member on the pay-head kind enum, no statutory-component analogue, no computation. The three `nps` hits are incidental: one doc comment and two places where employer-NPS is a **declared** figure feeding the §192 estimate — not a pay head that posts. In T2-2. ▼ 🟢 **GRADED UP 2026-09-08 BY A12 ON MERGE OF PR #88 (wave N) — WITH ZERO SCHEMA COST.** Route: **Masters → Create → Pay Head** (vendor: Alt+G → Create Master → Pay Heads) — set *Pay head type* to Employees' Statutory Deductions (employee NPS) or Employer's Statutory Contributions (employer NPS), then pick *"National Pension Scheme (Tier - I)"* or *"National Pension Scheme (Tier II)"* from the Statutory Pay Type picker; the roll-up is at **Reports → Statutory Reports → Payroll → Payroll Statutory Summary**. `IncomeTaxComponent` gained ordinals 11 and 12 riding the **existing** `pay_heads.income_tax_component` column — the precedent `TaxDeductedAtSource = 10` set — so `Schema.CurrentVersion` did not move. Driven end to end through the real shell VM (not a constructed `PayHead`) in `NpsPayHeadReachabilityTests`, reloaded from a real `.db`, with both pay-head names asserted as realised, effectively-visible `TextBlock`s in the Avalonia visual tree. 🔴 **THE ONE GENUINELY NEW GUARD IS THE SIDE RULE, AND A12 MUTATION-VERIFIED IT:** NPS Tier-I is the only statutory tag in this book that is legitimately **two-sided**, which `PayrollComputationService.RequiredStatutoryRole` (a single role) cannot express, so a dedicated guard sits on **both** write paths — `PayHeadService.ValidateNpsComponentSide` and the `CompanyImportService` pre-flight, which bypasses the service. Short-circuiting the service guard reddened **exactly** `An_nps_tier_two_head_is_refused_on_the_employer_side_and_named_in_the_message` with `Assert.Throws() Failure: No exception was thrown`, 17 of 18 still green; restored byte-identical, CR count unchanged at 303. 🔴 **NO ARITHMETIC, AND THAT IS THE VENDOR'S SHAPE RATHER THAN A SHORTCUT** — TallyPrime's NPS head is an ordinary As-Computed-Value head whose Percentage slab the **operator** types on Basic Pay, so this book seeds no NPS rate, ceiling or slab and **this row adds no new payroll-rate exposure at all**. **A SHIPPED DIVERGENCE CLOSES WITH IT:** row 7.25's `UnsupportedTypeNote` used to declare on the report's own face that this book carries no NPS component; it is now empty, and the assertion that required that footnote was deliberately inverted and flagged, because a footnote still saying it would be a lie on a shipped report. **WHY NOT COMPLETE:** nothing computes an NPS amount, and the §80CCD(1B)/§80CCD(2) *deduction* limbs of the income-tax computation are untouched. ⚠️ **CITATION GRADE, OWED TO THE USER:** the two captions are sourced to the vendor's **Tally.ERP 9** documentation (`/docs/te9rel61/…`), not a TallyPrime page — A12 re-fetched it and it returns *"Any contribution by the employer towards NPS will fall under the Tier I account of the scheme"* verbatim. It is still the vendor's own published documentation (R7 tier 1), but of the predecessor product, and the sibling track in this same wave declined row 1.8 precisely because an ERP 9 artefact must not be imported into a TallyPrime clone. The cases differ — NPS statutory pay types persist across both products, whereas the F12 menu was explicitly *removed* — but the distinction is recorded rather than assumed away. |
| 7.19 | Labour Welfare Fund deduction | PARTIAL | 🟢 **MOVED `ABSENT` → `PARTIAL` 2026-09-14 BY A12 ON MERGE OF PR #97 (W-R1), AND THIS ONE IS PROGRESS RATHER THAN BOOKKEEPING — THE STORAGE BLOCKER THIS ROW ITSELF NAMED IS GONE.** The row's own prescription below reads *"it genuinely needs an effective-from/effective-to on the pay-head computation or a month gate"*; schema **v63** adds exactly that — two nullable columns, `effective_from`/`effective_to`, on `pay_head_computation_slabs`, back-filling nothing because BOTH NULL already means "in force in every period", so every existing payslip recomputes to the same paisa. `PayrollComputationService` consults the window, `CanonicalMapper`/`CanonicalXml`/`ImportPlan` carry it through export–import, and it is **reachable by a user**, not merely stored: `LabourWelfareFundReachabilityTests` drives the Pay Head master screen and proves `A_december_only_lwf_head_is_created_through_the_master_screen_and_deducts_once_a_year_after_reload`, that both date boxes are drawn and focusable, that a blank pair means every period, and that an inverted or unparseable window is refused at the screen rather than silently treated as blank. The vendor's documented construction — *"define the Effective From and Value"*, and *"the value will be deducted only for the month (December) specified"* — is therefore now buildable, and the twelve-times over-deduction described below **cannot** happen. 🔴 **PARTIAL AND NOT COMPLETE, AND THE SHORTFALL IS DELIBERATE: NO STATE RATE TABLE IS SEEDED ANYWHERE.** LWF amount, ceiling and periodicity are per-State law; help.tallysolutions.com has the **operator** define Effective From and Value, so seeding a rate would be this product asserting statutory figures no source in R7's order supplies — the §194A / 4% cess mistake again. There is also no LWF-named master and no LWF statutory return or report. **The row moves because a user can now build the deduction correctly, not because the feature is finished.** ▼ *Prior evidence, unchanged and kept because it is what the fix was built against:* Case-insensitive greps for the full phrase and the abbreviation return zero across all four projects. No domain type, no component, no config field, no report. In T2-2. ▼ 🔴 **RE-CONFIRMED 2026-09-08 (wave N, PR #88) — STAYS ABSENT, AND THIS IS A DELIBERATE STOP RATHER THAN AN OMISSION.** The vendor documents LWF as an **ordinary pay head** — *"Select Deductions From Employees in the Pay head type field. In Computation Information section, define the Effective From and Value as applicable."* — with the load-bearing note A12 re-fetched and confirmed verbatim at `help.tallysolutions.com/tally-prime/payroll/payroll-faq/`: *"the value will be deducted only for the month (December) specified in the Pay Head."* 🔴 **THIS PRODUCT'S PAY-HEAD COMPUTATION HAS NEITHER HALF OF THAT.** `PayHeadComputationSlab` carries no effective-from — `pay_head_computation_slabs` holds `from_amount_paisa`/`to_amount_paisa`/`slab_type`/`rate_basis_points`/`value_paisa`/`ord` and **no date** — and there is no month gate anywhere on a pay head, so a FlatValue deduction head fires **every month**. Shipping LWF on today's mechanism would deduct the **annual** contribution **twelve times** on a live payslip: a strictly worse instance of the ₹100/year Karnataka professional-tax defect that already cost a Tier 0 fix plus a v55 back-fill migration. The salary-structure `EffectiveFrom` does not rescue it — a structure is superseded *forward*, so a December structure deducts December onward, forever. **There is no zero-storage construction that is correct**; it genuinely needs an effective-from/effective-to on the pay-head computation or a month gate. Nothing was written and no half-built row was left in the tree. **This row is now BLOCKED ON STORAGE, not on sourcing** — which is a different and more tractable state than it was in before this wave. |
| 7.20 | PF statutory returns beyond the ECR — Forms 3A, 5, 6A, 10, 12A | PARTIAL | 🟡 **Graded 2026-09-06 (PR #58, wave-7 D2).** All five forms are built, routed and tested, as re-presentations of the same `PfEcr` projection the ECR and challan come from — no new PF arithmetic. **PARTIAL, not COMPLETE, for one reason stated by the track itself:** *Refund of Advance* and *Non-Contributing Days* are **structurally unsourced** — `PfEcr.cs:113` hardcodes `RefundOfAdvances: 0` with no parameter to supply it, and no caller in `src/` ever passes `ncpDaysByEmployee`. They print **blank with a footnote** rather than a fabricated `0`, and wiring them needs a non-contributory-days source this book does not maintain. ⚠️ **Grade assigned by A12, not by the fixer** — the fix report described the work but stated no grade; PARTIAL is the conservative reading and is owed to the user for confirmation. *Prior evidence, now historical:* Zero hits for each form identifier. The reports directory holds the ECR projection and nothing else PF-shaped; no Screen member, no menu case. In T2-2. |
| 7.21 | ESI statutory returns — Forms 3, 5, 6 | PARTIAL | 🟡 **Graded 2026-09-06 (PR #58, wave-7 D2).** All three forms built, routed and tested. Form 3 is monthly (Reg. 14); Forms 5 (Reg. 26) and 6 (Reg. 32(1)) run the Apr–Sep / Oct–Mar contribution period. The same PR fixed the defect that made these unusable: the month-walk filtered only on *"a salary structure is in force"*, which never ends at leaving, so **wages were billed for every month after an insured person left** — an 11× overstatement, and Form 5 printed *"still working = No"* beside six months of wages on one row. ⚠️ **Grade assigned by A12, not by the fixer**, on the same conservative basis as 7.20; owed to the user for confirmation. *Prior evidence, now historical:* Zero hits. The only ESI artefacts are the monthly contribution projection and its offline writer; no return form exists. In T2-2. |
| 7.22 | Attendance Sheet — the per-employee attendance matrix (days present/absent, units produced, overtime hours) | PARTIAL | 🟡 **Graded 2026-09-07 (wave J1, PR #80). The grade reported by the building track; A12 verified the route and the counts but did not re-derive the report.** `AttendanceSheet.cs` ships as a pure projection, routed at `Reports > Payroll Reports > [Attendance] > "Attendance Sheet"` — a row of its own, adjacent to but distinct from the Attendance Register that 7.15 ships, which is the fold this row exists to prevent. Wage-month picker; `F12` *remove zero-valued*. The oracle asserts 24 present / 2 absent / 300 units / 12 OT hours **in the realised visual tree**, including the unit-bearing headings *"Units Produced (Nos)"* and *"Overtime (Hrs)"*. 🔴 **Overtime is derived from `PayHead.IsOvertime` + `AttendanceTypeId`, never from a type's NAME** — pinned by a test that clears the flag and watches 12 hrs move into Units Produced. **Gap, and it is why this is not COMPLETE:** no `F4` category/group filter and four other `F12` options unbuilt. ⚠️ A defect this row's own new legibility test caught on its first run and fixed: the heading *"Units Produced (Nos)"* needed 132px in a 96px column and would have been cut to *"Units Produced"* — losing the unit, the only thing distinguishing 300 pieces from 12 hours. *Prior evidence, now historical:* 🔴 **ROW ADDED 2026-09-07 BY USER RULING 19. IT HAD NO ROW AT ALL** — searching this census for *"Attendance Sheet"* before the ruling returned **zero**, and so did `src/`, so this is a missing ROW **and** a missing FEATURE, not a row filed under another name. **Vendor page:** `help.tallysolutions.com/tally-prime/payroll-reports/attendance-sheet-payroll/` — *"the total days an employee was present or absent, along with the number of units produced and the overtime hours worked"*, reached by `Alt+G` (Go To) → **Attendance Sheet**, filtered by `F4` (Employees), with a *Remove zero-valued transactions* `F12` option. ⚠️ **DO NOT FOLD THIS INTO 7.15, WHICH IS THE TRAP THIS ROW EXISTS TO CLOSE.** The vendor publishes **both**, on two different pages — the **Attendance Register** (`…/attendance-register-payroll/`) is what 7.15 ships (`AttendanceRegister`, **23 hits** in `src/`); the **Attendance Sheet** is a different report and `AttendanceSheet` returns **zero** hits in `src/`, as does the phrase *"attendance sheet"*. |
| 7.23 | Pay Head Employee Breakup — for one employee, the group-wise summary of pay-head transactions with closing balance | PARTIAL | 🟡 **Graded 2026-09-07 (wave J1, PR #80). The grade reported by the building track; A12 verified the route and the counts but did not re-derive the report.** Routed at `Reports > Payroll Reports > [Breakups] > "Pay Head Employee Breakup"`, with a wage-month and an **Employee** picker (the vendor's *List of Employees*). Group-banded by the accounting group of the pay head's own ledger, with Opening / Debit / Credit / Closing, Dr/Cr-suffixed through the existing `IndianFormat.Signed`; the prior month carries forward as opening. 🔴 **The employer-contribution expense MIRROR is excluded, or the head would net to zero** — and the same test proves the mirror IS on the voucher, so the exclusion is a projection choice and not a missing posting. **Gap, and it is why this is not COMPLETE:** the vendor drills on `Enter` (Ledger Monthly Summary → Employee Vouchers); the payroll matrix in this product carries no drill stack, so the detail level is rendered as a SECOND BAND under the report — every figure the drill would show is on screen, but the navigation shape is ours, not the vendor's. Also unbuilt: `F5` ledger-wise and `Ctrl+H` Change View. *Prior evidence, now historical:* 🔴 **ROW ADDED 2026-09-07 BY USER RULING 19. IT HAD NO ROW AT ALL** — zero hits by name in this census and zero in `src/`. **Vendor page:** `help.tallysolutions.com/tally-prime/payroll-reports/pay-head-employee-breakup-tally/` — `Alt+G` → **Pay Head Employee Breakup**, then pick the employee from the *List of Employees*. `PayHeadEmployeeBreakup` → **zero** in `src/`: no report kind, no Screen member, no menu case. |
| 7.24 | Employee Pay Head Breakup — for one pay head, the breakup across all employees | PARTIAL | 🟡 **Graded 2026-09-07 (wave J1, PR #80). The grade reported by the building track; A12 verified the route and the counts but did not re-derive the report.** Routed at `Reports > Payroll Reports > [Breakups] > "Employee Pay Head Breakup"`, with a wage-month and a **new Pay Head picker**. It is the transpose of 7.23 over ONE shared engine, banded by employee group — so the two can never disagree — but it is a **second `ReportKind` with a second scope picker**, and a test asserts that 7.24 lists BOTH employees under Basic while 7.23 scoped to one employee shows neither the other employee nor a flat list. Neither row can answer for the other; that is what makes them two rows and not one written twice. **Gap, and it is why this is not COMPLETE:** the vendor drills `Enter` → Employee Summary → Monthly Summary → Vouchers, and this product has no drill stack, so those levels are a SECOND BAND rather than a descent; `Ctrl+H` Change View unbuilt. *Prior evidence, now historical:* 🔴 **ROW ADDED 2026-09-07 BY USER RULING 19. IT HAD NO ROW AT ALL** — zero hits by name in this census and zero in `src/`. **Vendor page:** `help.tallysolutions.com/tally-prime/payroll-reports/payroll-employee-pay-head-breakup-tally/` — select the pay head, then `F2` (Period). `EmployeePayHeadBreakup` → **zero** in `src/`. ⚠️ **7.23 and 7.24 are two reports, not one written twice** — they transpose the same data (one employee × many pay heads, versus one pay head × many employees) and the vendor gives each its own page and its own `Alt+G` entry. Counted as two, with the near-duplication declared, exactly as 16.3/16.4 are. |
| 7.25 | Payroll Statutory Summary — the statutory pay-head details roll-up | PARTIAL | 🟡 **Graded 2026-09-07 (wave J1, PR #80). The grade reported by the building track; A12 verified the route and the counts but did not re-derive the report.** Routed at `Reports > Statutory Reports > Payroll > [Other Payroll Statutory] > "Payroll Statutory Summary"`. 🔴 **No new arithmetic: it READS POSTED VOUCHERS rather than recomputing PF/ESI/PT**, so it can never disagree with 7.10/7.11/7.12, which are the computations this rolls up OVER. *Payable* is the liability the payroll raised; *Paid* is a settlement debited to the same statutory payable ledgers by a **non-payroll** voucher — proven by posting a PT payment and watching only PT's balance clear. The vendor's *Statutory Pay Head Details* is rendered as a second band and asserted on screen. 🔴 **NPS IS OMITTED AND DECLARED ON THE REPORT'S OWN FACE, NOT INVENTED.** The vendor's page names PF, ESI, NPS and PT; this book carries **no NPS statutory component on any pay head**, so there is no figure to roll up. A documented divergence labelled as ours — **and it is owed to the user as a decision** (see §6). **Gap, and it is why this is not COMPLETE:** the vendor drills `Enter` into Payroll Vouchers and this product has no drill stack, so that level is a second band; `Ctrl+J` TDS Variance unbuilt. *Prior evidence, now historical:* 🔴 **ROW ADDED 2026-09-07 BY USER RULING 19. IT HAD NO ROW AT ALL** — zero hits by name in this census and zero in `src/`. **Vendor page:** `help.tallysolutions.com/tally-prime/payroll-statutory-reports/payroll-statutory-summary-tally/` (*"Payroll Statutory Summary and Statutory Pay Head Details"*). `PayrollStatutorySummary` → **zero**; the phrase *"statutory summary"* → **zero** across `src/`. **Not covered by 7.10/7.11/7.12**, which are the PF, ESI and PT computations and their own registers; this is the roll-up **over** them. |
| 7.26 | Income Tax Computation report — the per-employee computation, in Form 16 shape | PARTIAL | 🟡 **Graded 2026-09-07 (wave J1, PR #80). The grade reported by the building track; A12 verified the route, the counts and the non-vacuity of the oracle, but did not re-derive the report.** Routed at `Reports > Statutory Reports > Payroll > "Income Tax Computation"`, gated on *Enable Salary TDS* and sitting beside Form 16, with an Employee picker. **This closes the gap this row was written to name — the arithmetic existed and the REPORT did not.** 🔴 **No second computation was written:** every line is asserted EQUAL to the corresponding `Form24Q.BuildAnnexureII` row, the same call that backs Form 16 Part B, so the report and the return cannot drift. 🔴 **The oracle is NON-VACUOUS, deliberately:** a third fixture employee at **₹18,00,000/yr** was added, because at ₹30,000 every tax line is 0 and the test would have asserted 0 == 0. **Gap, and it is why this is not COMPLETE:** the vendor drills an italic line on `Enter` into that component's computation; with no drill stack the components are a second band carrying real breakdowns instead; `Ctrl+Enter` Income Tax Setup, `F7`, `F8`, `Alt+B` and `Alt+R` unbuilt. ⚠️ **TWO OPEN DEFECTS ARE SURFACED BY THIS REPORT, NOT INTRODUCED AND NOT PAPERED OVER — T1-26, the §192 engine being date-blind (`Slabs` takes no date), and the unverified 4% cess already standing as the top user gate in §6.** Making the computation visible does not make it verified. A defect this row's own legibility test caught on its first run and fixed: the value *"New regime (u/s 115BAC)"* needed 158px in a 114px cell and would have been cut to *"New regime (u/s"* — leaving the two regimes, the one thing that decides which deductions apply, reading almost identically. *Prior evidence, now historical:* 🔴 **ROW ADDED 2026-09-07 BY USER RULING 19. IT HAD NO ROW AT ALL** — zero hits by name in this census and zero in `src/`. **Vendor page:** `help.tallysolutions.com/tally-prime/payroll-income-tax-reports/tax-computation-tally/` — *Gateway → Display More Reports → Payroll Reports → Statutory Reports → Income Tax → Computation*, or `Alt+G` → **Computation**; employee header, deduction snapshot, each income-tax component, `Enter` on an italic line to drill into its computation, `F4` to change employee. ⚠️ **STATED PRECISELY SO NOBODY OVER-READS IT, AND THIS PRECISION IS THE USER'S OWN IN RULING 19: THE ARITHMETIC EXISTS AND THE REPORT DOES NOT.** `SalaryIncomeTax.ComputeAnnual` returns a `SalaryTaxComputation` record (`src/Apex.Ledger/Services/SalaryIncomeTax.cs`, 4 hits) carrying slab tax, rebate, surcharge and cess, and it backs **Form 16 Part B / Annexure II**. What is missing is the vendor's **per-employee report** — no `ReportKind`, no route, no drill-down, no `F4` employee switch. A capability no user can reach is not complete; this row is ABSENT on the census's own bar, not PARTIAL. |

#### Area 8 — Banking · 11 rows · 1 complete / 10 partial / 0 absent

> **▶ 🔴 THREE ROWS — 8.11, 8.12 AND 8.13 — WERE ADDED 2026-09-07 BY USER RULING 19 (R12, `plan.md` §5,
> `TWO FURTHER USER RULINGS (R12, 2026-09-07)`). THEY HAD NO ROW AT ALL.** Same defect and same reasoning as
> Area 7's five: a vendor capability with no row is invisible to §1.2's arithmetic, to the area headings, to
> the published `awk` and to `CensusDerivationDriftLockTests`, all four at once. All three are **ABSENT** on a
> named zero-hit search re-run first-hand on 2026-09-07 over `src/`. **The area's denominator moves `10 → 13`.**
...
| 8.4 | Cheque printing — print a cheque from a bank payment | PARTIAL | 🟡 **Re-graded 2026-09-07 (wave J2, schema v57) — THE LEAF IS NOW REACHABLE.** The confession this cell carried is discharged: `Ledger.ChequeLayout` had **zero writers in `src/`** and no table to load one from, so `ChequePdf.Validate` refused every render on every loaded company and ~625 lines of shipped, tested, deterministic cheque-rendering code could be reached by nobody. v57 adds `cheque_layouts` plus six `ledgers` columns, and the bank ledger master gains a **Cheque Dimensions** block — the vendor's own screen (`help.tallysolutions.com/cheque-payments-set-up/`, "Specify User Defined Cheque Format"; field list from `…/docs/te9rel51/…/Creation_Mode.htm`, "Cheque Dimensions") — so an operator measures a real leaf, keys the offsets in millimetres, saves, and `Ctrl+P` on the drilled voucher inks the cheque. Units are **tenths of a millimetre as INTEGER, never REAL**, so one stored layout renders one byte stream on every machine. The two latent guards went live with it and both fire against a persisted layout: a layout that would **place nothing** refuses rather than spending a pre-numbered leaf on a blank sheet, and an amount-in-words that will not fit refuses rather than truncating. **Gap, and it is real:** we ship **exactly one format, "User Defined"** — the vendor's per-bank predefined dimension table is served from its own subscription service and not one millimetre of it is publishable, so guessing was refused (a documented divergence, not an omission); `ChequePdf.RenderCalibrationSheet` exists and **still has no route in the UI**, so the bootstrap from an all-zero layout is measure-by-hand; the `printed` flag has storage and no writer, so `F8 Include Printed` reads every cheque as unprinted; no A/C-payee crossing (the vendor's field list has no such element and one was not invented). *Prior evidence, now historical:* a Cheque Printing report under Banking drilling to the paying voucher, with `ChequePdf`, a `PrintKind.Cheque` and validation — but the leaf path NOT REACHABLE BY AN OPERATOR, and no schema version taken. **= T1-14.** ⚠️ **Area assignment ambiguous** — one survey placed this under Printing; counted **once, here**, because the vendor's own Banking menu carries it. Area 12 holds an uncounted cross-reference. |
| 8.5 | Cheque Register | PARTIAL | 🟡 **Graded 2026-09-07 (wave J2, schema v57).** The register ships in both halves: a summary of one row per cheque book with its six status counts, drilling to a leaf-by-leaf list, under `Transactions > Banking > Cheque Management > Cheque Register`. All six vendor buckets are produced — **Available · Unreconciled · Reconciled · Blank · Cancelled · Out of Period** (`help.tallysolutions.com/cheque-register/`, `…/docs/te9rel65/Banking/Cheque_Register.htm`) — with `F4` bank scope, `F8` status filter, `Alt+A` Alter Status on a leaf, and the vendor's *"cheques which do not belong to any cheque range"* case reported in its own **Not in Range** section rather than dropped. 🔴 **THREE OF THE SIX BUCKETS NEEDED STORAGE AND THAT IS WHY THIS ROW MOVED:** Available / Blank / Cancelled are facts about *paper you hold*, not about entries you posted, so v57 adds `cheque_books` + `cheque_status_overrides`; the operator records a book on the bank ledger master and cycles a leaf's status with `Alt+A`. Reconciled / Unreconciled / Out-of-Period stay **computed**, deliberately — a stored copy would lie the moment a Bank Date is keyed. **Gap:** no `Alt+B` Alter-Cheque-Book chord from inside the report (books are maintained on the ledger master only, so the master is one route, not two); no `Alt+R` reconcile-from-the-register; the printed flag has storage and no writer, so `F8 Include Printed` on the Cheque PRINTING report still reads every cheque as unprinted; a lettered cheque series reports a leaf count of 0 rather than enumerating. *Prior evidence, now historical:* Zero hits for the identifier and the phrase; no report kind, no Screen member, no menu row. **= T1-14.** |
| 8.6 | Deposit Slip (the bank pay-in slip) | PARTIAL | 🟡 **Graded 2026-09-07 (wave J2, schema v57).** Both vendor slips ship as one `ReportKind` with the vendor's **F5** mode switch — **Cash Deposit Slip** and **Cheque Deposit Slip** (`help.tallysolutions.com/deposit-slips/`) — under `Transactions > Banking > Slips & Advices > Deposit Slip`, with `F4` bank scope, `Ctrl+P`, export and a drill to the receipt voucher. Receipt-side only by construction: a payment out of the same account can never reach a pay-in slip. The four header fields print — *Account Number · Bank Name · Branch Name · Account Holder Name* — the first three from the v57 `ledgers` columns (`bank_account_number`, `bank_branch`, `bank_ifsc`), captured on the bank ledger master; the fourth is the company's mailing name, deliberately not a fourth stored field that could contradict the company master. 🔴 **TWO VENDOR FIELDS ARE DELIBERATELY NOT PRINTED, AND THE SOURCE-SILENCE IS THE REASON:** the **company's telephone number** has no column anywhere in this schema (adding one is a Company-master change), and the **cash denomination breakdown** is a tally of notes being carried to the bank that is not in the books and cannot be derived. Neither is captioned — a caption over permanent blank is the dead-field defect this project has filed three times. **Gap:** those two fields; no `F8` include-printed; no `Alt+H` alter-cheque-details; no Top Margin print-config field; no per-receipt spacebar selection. *Prior evidence, now historical:* Zero hits for the identifier and the phrase — no type, no report kind, no Screen, no menu row, no print projector. **= T1-14.** |
| 8.7 | Banking Payment Advice (the supplier-payment advice letter) | PARTIAL | 🟡 **Graded 2026-09-06 (PR #59, wave-7 D1).** A supplier payment advice now exists under Banking as its own `ReportKind`, with a `PaymentAdvicePdf` renderer that paginates, foots each sheet with its own page number, and repeats the bill-table caption band across a break. The same PR fixed two defects that made it unsafe to send: the letter **ran off the paper** (an 80-bill advice placed Gross, Less: TDS, Net Amount Paid and the signatory at negative y, in a PDF that stayed valid and reported one page), and **Net Paid was `gross − tds`, not what left the bank** — it now derives from the party debit, so a bank-charge leg cannot tell a supplier it was paid more than it was. **PARTIAL:** an unattributable non-tax credit on a **multi-supplier** voucher is deliberately left out of both advices rather than apportioned. *Prior evidence, now historical:* the only payment advice in the tree was the **payroll** bank advice, with employee/bank/IFSC/net-pay columns, surfaced under Payroll Reports. Nothing under Banking produces a supplier advice. **= T1-14.** |
...
| 8.10 | e-Payments / bank payment-instruction file | PARTIAL | 🟢 **MOVED `ABSENT` → `PARTIAL` 2026-09-08 ON MERGE OF PR #83 (wave K2, no schema). The grade is the build track's own; A12 recorded it.** Gateway → Banking → Payments → e-Payments (`Reports/EPayments.cs`, `ReportKind.EPayments`), reached by arrow keys; `F4` scopes one remitting bank, `F2`/`Alt+F2` the period, rows drill to the payment, `Ctrl+A` writes the payment-instruction file and a status line names the path. Beneficiary account/IFSC is captured at Masters → Ledgers → (party) → Beneficiary Bank Details, riding the v57 `ledgers.bank_account_number` / `bank_branch` / `bank_ifsc` columns — the only change needed was a visibility gate, since the columns are on every ledger already. The vendor's three shipped section names are cloned verbatim. Optional vouchers are excluded by the same shared predicate as 8.13, so **an unreviewed auto-created voucher can never reach a bank instruction**. 🔴 **THE FILE LAYOUT IS OURS AND IS LABELLED AS OURS IN THE SOURCE (ruling 9), BECAUSE THE VENDOR PUBLISHES NONE** — `help.tallysolutions.com/export-upload-e-payments/` names no format, no field list and no bank's layout, and refers the reader to their own bank. **This is the open half of decision D-6 (*"e-Payments file format"*) and it is still owed to the user.** **Gaps keeping it off `COMPLETE`:** the *"Sent to Bank (Unreconciled)"* section and its In Progress / Successful / Unsuccessful split are absent rather than faked — every one is a fact about what happened *after* an export and nothing in this schema records it, and the report says so on its own face; the *"Mismatch in Bank Details (With Masters)"* bucket presupposes the transaction carries its own copy of the payee's account to compare against the master, and a `BankAllocation` carries transaction type, instrument number and dates only, so with one copy there is nothing to mismatch; and the reverse-file import and `Alt+Z` → Exchange → Send Payments (a connected-banking path) are not built. |
*(Rows **8.11 Connected Banking** and **8.12 Payment Request** stood here until 2026-09-14. 🔴 **STRUCK BY USER RULING 24 — they are out of the denominator and are NOT complete.** Their full text, their state when struck and the reason are in **§1.2d**. Ruling 19's own row 8.11 predicted this exact case and said whose call it was: the scope question was flagged there, unanswered, and it is now answered by the user rather than by an agent. **Do not re-add them to this area** — see §1.2d's mechanism note for why a struck row does not get a fifth state token.)*
| 8.13 | Auto-create vouchers from an imported bank statement (optional vouchers, reviewed then regularised) | PARTIAL | 🟢 **MOVED `ABSENT` → `PARTIAL` 2026-09-08 ON MERGE OF PR #83 (wave K2, no schema). The grade is the build track's own; A12 recorded it. THE ROW WAS `ABSENT` FOR ONE DAY.** Gateway → Transactions → Banking → Import Bank Statement; pick a bank, import the CSV, tick unmatched lines and choose a **Ledger Name** (the vendor's own column), then `F7` (*Create Vch/Multi-Vch*) or `Alt+F7` (*Create Voucher-Consolidate*), then **R** (*Mark as Regular & Reconcile*) on the *"Bank Reconciliation - Optional Vouchers"* band. Engine: `Banking/BankStatementVoucherCreation.cs` plus `LedgerService.MarkOptionalAsRegular`. 🔴 **THE SAFETY PROPERTY IS STRUCTURAL, NOT A FLAG, AND IT IS ASSERTED AS ARITHMETIC.** Every created voucher is `Voucher.Optional`, which `LedgerBalances.CountsAsOf` excludes, so nothing reaches the Trial Balance, the Balance Sheet, the bank closing balance or the BRS until a human presses R — pinned by a test that reads three independent figures and requires them byte-identical after creation. Mutating `optional: true` to `false` reddens five tests, the key one failing **on the bank balance**: the mutation is caught as money moving, not as a boolean flipping. `Alt+F7` also proved that the global Stock-Journal chord steals the keystroke without the page-scoped arm. **Gaps keeping it off `COMPLETE`:** no `Ctrl+J` exception reports from that screen, no per-voucher pre-save field editor (bank allocations / cost centres), and no Excel statement import — row 8.3's parser is CSV-only while the vendor allows either. **The 8.3 boundary is pinned by its own test** (`Matching_alone_never_creates_a_voucher_which_is_why_8_13_exists`). — 🔴 **ROW ADDED 2026-09-07 BY USER RULING 19. IT HAD NO ROW AT ALL** — zero hits by name in this census and zero in `src/`. **Vendor page:** `help.tallysolutions.com/auto-create-vouchers/` — import an Excel/CSV statement, **create multiple vouchers**, which are *marked optional so you can review before finalizing*, checking bank allocations and cost centres, then reconcile once regularised. 🔴 **THIS IS GENUINELY NOT INSIDE ROW 8.3, AND THE MEASUREMENT SAYS SO RATHER THAN THE ARGUMENT.** 8.3 is reconciliation: `src/Apex.Ledger/Banking/BankStatementImport.cs` exposes exactly `ParseCsv` and `MatchAndReconcile`, and `CreateVoucher`, `AddVoucher`, `PostVoucher` and `new Voucher` return **zero** in that file — it matches statement lines to vouchers that **already exist** and stamps a Bank Date. 8.3's own **Gap** list (CSV only, no manual match/unmatch, no print or export) never mentions creating a voucher either. ⚠️ **A reader who judges auto-create to be inside 8.3's scope would put the honest denominator at 223 rather than 224; ruling 19 ruled it a row of its own.** |

#### Area 9 — Inventory / manufacturing / job work (post-7.2) · 9 rows · 3 complete / 6 partial / 0 absent
...
| 9.6 | Job Costing | PARTIAL | 🟢 **MOVED `ABSENT` → `PARTIAL` 2026-09-08 ON MERGE OF PR #81 (wave K1, schema v58). The grade is the build track's own; A12 recorded it. Code was written to move it.** F11 → *"Enable Job Costing"*; Masters → Create → Godown → *"Set job/project for job costing"*, which picks a **cost centre** — the vendor's own model, so this reuses the existing cost-centre machinery instead of adding a parallel cost dimension (`godowns.job_cost_centre_id`). Report at Reports → Inventory Reports → Job Costing → *Job Work Analysis* (`Reports/JobWorkAnalysis.cs`), which splits revenue from cost **by the ledger's nature** rather than by allocation sign, applies a godown-link membership rule, and folds one job spanning two godowns into a single row naming both. Mutation-verified: replacing the membership rule with *"every cost centre is a job"* reddens three tests. **Gaps keeping it off `COMPLETE`:** no `Ctrl+H` alternative views (Monthly Summary / Breakup of Job-Project / Job-Project Summary), no `Ctrl+B` scale factor, no `Ctrl+J` negative ledgers, no `Alt+C` comparison columns, no drill-down, and the vendor's godown-level F12 *"Set Job Costing details"* toggle is absent — the field is gated on F11 alone. |
| 9.7 | Item Cost Tracking | PARTIAL | 🟢 **MOVED `ABSENT` → `PARTIAL` 2026-09-08 ON MERGE OF PR #81 (wave K1, schema v58). The grade is the build track's own; A12 recorded it.** F11 → *"Enable Cost Tracking"*; a **Cost Tracking No.** box on every stock line (notes, both arms of the Stock Journal, item invoices), persisted on BOTH stock-line tables. Reports → Inventory Reports → Item Cost Analysis → *Stock Item Cost Analysis* / *Stock Group Cost Analysis* / *Cost Track Break-up* (`Reports/ItemCostAnalysis.cs`), drawing the vendor's four column captions verbatim. **The load-bearing arithmetic is the balance subtraction** — profit is `Revenue − (Cost − BalanceAtCost)`; a naive build reports the pinned fixture as a ₹300 LOSS instead of a ₹200 profit, and mutating it back reddens the test that pins ₹1,000 / ₹700 / ₹500 / ₹200. **Gaps keeping it off `COMPLETE`:** the vendor's *List of Cost Tracking Numbers* picker with its *New Number* entry is not shipped — the field is free text — and the *Item Movement Summary* sub-report (Order / Receipt / Billing / Manufacturing details) inside Cost Track Break-up is absent. |
| 9.8 | Tracking Numbers linking Receipt Note ↔ Purchase and Delivery Note ↔ Sales | PARTIAL | 🟢 **MOVED `ABSENT` → `PARTIAL` 2026-09-08 ON MERGE OF PR #81 (wave K1, schema v58). The grade is the build track's own; A12 recorded it.** F11 → *"Use tracking numbers (enables delivery and receipt notes)"*; then Receipt Note (`Alt+F9`), Delivery Note (`Alt+F8`), Rejection In/Out and item-mode Purchase/Sales each draw a **Tracking No.** box, persisted on both `inventory_allocations` and `voucher_inventory_lines`. Reports → Inventory Reports → Bills Pending → *Purchase Bills Pending* / *Sales Bills Pending* (`Reports/BillsPending.cs`), which nets `Received − Billed`: no bill leaves 40 pending, a part bill of 25 leaves only the unbilled 15, and a full bill clears the item entirely. **There is now an operator-entered tracking datum, so order fulfilment is no longer inferred by a FIFO walk.** 🔴 **The most important evidence on this row is a mutation that did NOT redden:** deleting the Tracking No. field from the item-invoice (**bill**) grid left all eight reachability tests green, so half the mechanism could have shipped dead — goods keyed, bills not, and Purchase Bills Pending would then report every receipt as permanently unbilled, which is *wrong* rather than merely empty. A ninth test was written against the live mutation before the XAML was restored. **Gaps keeping it off `COMPLETE`:** the vendor's default (*"By default, the invoice number appears"*) is not implemented — the box opens blank; there is no *List of Tracking Numbers* picker, no order-number linkage, and no drill from a pending row to its voucher. **= T1-8, now partly closed.** |
| 9.9 | **Voucher Classes on Stock Journal** — the Transfer Class (*"Use Class for Inter-Godown Transfers"*) and the Job-Costing Consumption Class | PARTIAL | 🟢 **MOVED `ABSENT` → `PARTIAL` 2026-09-08 ON MERGE OF PR #81 (wave K1, schema v58). The grade is the build track's own; A12 recorded it. HALF THE ROW SHIPPED, AND THE ROW TITLE SHOULD BE READ AS HALF-MET.** The **Transfer Class** ships: Masters → Alter → Voucher Type → Stock Journal → *Voucher Classes* → *"Name of Class"* + *"Use Class for Inter-Godown Transfers"*, then Stock Journal (`Alt+F7`) → *Class* picker → *Destination Godown*, which replaces the hand-keyed destination grid. `InterGodownTransfer.Mirror` carries batch, rate, unit and cost-tracking number across, refuses a same-godown transfer, and balances the source by construction; the service refuses the flag on a non-Stock-Journal type and refuses a duplicate class name. 🔴 **The Job-Costing Consumption Class is NOT shipped, and that is a source-silence decision, not an oversight** — no vendor page reached enumerates its fields, so inventing them would have been the *"clone, never invent"* failure. ⚠️ **AND THIS DID NOT HALF-BUILD ROW 2.6.** The general Voucher Class machinery (ledger pre-maps, default accounting allocations, additional-ledger rules, rounding) stays `ABSENT` and is not this track's: a Stock Journal posts no ledger entry, so `voucher_type_classes` stores only the type link, the vendor's *"Name of Class"* and the one attested flag. 🔴 **THE TWO MEASUREMENTS BELOW ARE NOW STALE AND ARE KEPT ONLY AS THE ROW'S HISTORY** — *"this product has exactly one voucher class and it is the hard-wired POS tender pre-map"* and *"zero hits for the phrase and the identifier"* were both true when written and are both false now. — 🔴 **RE-TITLED 2026-09-04 (§1.3 item 23) — THE STATE IS UNCHANGED AND THE ROW'S OLD PREMISE WAS NOT A TALLYPRIME FACT. It previously read** ~~*"Transfer Journal as a **named** voucher kind"*~~**, which is a Tally.ERP 9 artefact:** TallyPrime's own inventory-voucher page lists nine kinds and does not include it; the mechanism is a user-defined **voucher class**, of which this product has exactly one and it is the hard-wired POS tender pre-map (`Domain/PosConfig.cs`, `Schema.cs:1498`). **A row that names the wrong target cannot be closed by building the right thing.** **Shares its single missing mechanism with 9.6.** Zero hits for the phrase and the identifier. **Read this precisely:** the *function* is partly covered — an inventory line carries a godown with an in/out direction and the posting service handles the Stock Journal base kind with its own balance guard, so inter-godown movement **is** expressible. What is absent is the separately named kind. |

#### Area 10 — Accounting features (post-7.2) · 2 rows · 0 complete / 1 partial / 1 absent
...
|---|---|---|---|
| 10.1 | Credit Limits on a ledger, with the over-limit block on voucher save | PARTIAL | 🔴 **BUILT AND LANDED 2026-09-06 — PR #62 (`claude/apex-f2-verbs-limits`), merged at `e081317`. Was ABSENT.** Shipped: **schema v54** (three `ledgers` columns — `credit_limit_paisa`, `check_credit_days_on_entry`, `override_credit_limit_post_dated`), the `CreditLimitRules` save-blocking guard in the validator, and the **Credit Limits** block on the party-ledger master, gated on `IsPartyGroup`. The migration was verified complete by A12 in all three respects before landing — migration constant, byte-identical `CreateV1` twin, and `SchemaDowngrade.V54ToV53` — and `credit_limit_paisa` is deliberately **NULLable with no DEFAULT**, because `NOT NULL DEFAULT 0` would have declared that every pre-existing party may buy nothing. ⚠️ **Grade assigned by A12, not by the track** — the first-time review reported findings but no grade; `PARTIAL` is the conservative reading and is **owed to the user for confirmation**. **Three gaps hold it below COMPLETE, all review-measured:** **(a)** the refusal message tells the operator to turn on *"Override credit limit using post-dated transactions"*, but `CreditLimitRules.cs:213` exempts only `voucher.PostDated && party.Override…` — their own test asserts an ordinary voucher is **still refused**, so the app's advice provably does not work; **(b)** the vendor's **F11 › Accounting Features › "Maintain budgets and controls"** prerequisite (help.tallysolutions.com, *Credit Limits*) has **no flag anywhere in `src/`** — the rule and the master block are unconditional, a wrong-refusal vector for books that never opted in; **(c)** `ResetForNextEntry`'s credit-limit reset is **unguarded** — the reviewer deleted the three lines and the full Desktop suite stayed green, so a regression would stamp one party's limit onto every subsequent ledger in a keying run. ▼ *Original ABSENT evidence, kept because it is what the row moved FROM:* Case-insensitive search of every `.cs` and `.axaml` in `src/` for the identifier and the phrase returns **zero**: no domain property, no persistence column, no view-model field, no guard in the validator. ⚠️ Also named under Accounting masters by a second survey; counted **once, here**, per §1.1 rule 3 (earliest product that shipped it). In T2-3. |
...
#### Area 12 — Printing · 9 rows · 1 complete / 8 partial / 0 absent
...
| 12.5 | Physical printer output (printer selection, print job, spooler) | PARTIAL | **Was ABSENT; moved by PR #64 (`claude/apex-f1-printing`), merged as `99b3c3d`.** Now ships a real spooler hand-off, not a file write: `src/Apex.Desktop/Services/PlatformPrinting.cs` P/Invokes `winspool.drv` (`OpenPrinter` → `StartDocPrinter` → `StartPagePrinter` → `WritePrinter` → `EndPagePrinter` → `EndDocPrinter` → `ClosePrinter`, with an `AbortPrinter` unwind) on Windows and shells `lp`/`lpstat` on CUPS; plus `PrinterDevices` (enumeration + default queue), `PrintJob`/`PrintJobBuilder`, `PrinterSelectionViewModel` and the `MainWindow`/`GatewayColumn` wiring. Gate on the merged head, read from **each OS leg's own job log** on run `34044686846` (headSha = PR headRefOid `29264fe`): **638 · 2348 · 266 · 3089**, 0 failed, **0 skipped on all three runners** — windows ran 16 min and genuinely completed. **🔴 WHY PARTIAL AND NOT PRESENT: the spooler call itself is never exercised by the suite, and cannot be.** No CI runner has a print queue, so the tests can only prove the *refusal* path (`OpenPrinter` fails → honest failure, no throw), the off-thread dispatch, enumeration robustness and the platform shim. Two shipped fixes are structurally untestable here and the track said so rather than faking coverage: the **CUPS stdout drain** (no `lp` on a Windows runner) and the **Windows `AbortPrinter` unwind** (`OpenPrinter` fails first for a nonexistent queue, so the unwind is unreachable). Their absence from the suite is **not a defect**, and must not be recorded as covered. Ink on paper is therefore **unproven by automation** and needs a manual run against a real queue before this becomes PRESENT. **⚠️ Grade issued by A12 on merge — PR #64 drew no reviewer review and no reviewer comment, so no reviewer grade exists for this row.** A12 mutation-verified the suite rather than trusting it: inverting the `OpenPrinter` refusal to report success reddened `Submitting_to_a_queue_that_does_not_exist_reports_failure_without_throwing` with its own assertion text, then restored byte-exact (clean status, unchanged CR count) and went green. **🔴 KNOWN FLAKE THIS ROW SHIPPED WITH, on `main` now: `PlatformPrintingTests.Enumeration_survives_a_tool_that_floods_stderr` failed the **windows** leg on the very next run after PR #64 merged (`Assert.Contains` — stdout came back `""`, run `34061735405`), then passed on rerun of the same commit, so it is non-deterministic, not broken.** Cause: the test spawns `powershell.exe` as its flooding child but is bounded by `CupsPrinterDevices.TimeoutMs = 4000`, a **production** budget sized for `lpstat`; the failing run took exactly 4 s, i.e. it hit that timeout and `RunTool` returned empty. Locally the same child costs 250–500 ms, so the margin is ~8× and only disappears under runner contention (it runs alongside 3089 desktop tests). **This is a defect in the test's choice of a heavyweight host process, not in the shipped drain** — and note the irony that the one test touching the CUPS drain is the flaky one, on the platform where CUPS enumeration never runs in production. Needs a lighter child or a test-only timeout; not yet fixed. **= T2-5.** |
| 12.6 | Multi-account printing / multi-voucher (range) printing | PARTIAL | 🟡 **Graded 2026-09-06 (PR #56, wave-7 C4).** Multi-account printing exists: a projector builds a set of documents from a chosen set of accounts, a `PrintKind.ReportSet` renders them as **one collated job** (one PDF, each document on its own sheet, numbering running across the whole job), and a page range is honoured by every renderer. **PARTIAL:** the multi-**voucher** (range) half of this row is not addressed — the range knob selects sheets within a job, not a range of vouchers to print. *Prior evidence, now historical:* Zero grep hits for the identifiers and phrases. Nothing iterates a set of accounts or vouchers into one print job — the opener builds exactly one preview from exactly one report or one drilled voucher. **In T1-14.** |
| 12.7 | Delivery challan, reminder letter, confirmation of accounts | PARTIAL | 🟡 **Graded 2026-09-06 (PR #56, wave-7 C4).** **Two of the three are delivered**, and in the right shape — as multi-account print outputs, exactly as the 2026-09-04 correction below said they must be, not as three standalone document templates. The **reminder letter** and the **confirmation of accounts** are produced by `MultiAccountPrintProjector`, gated by one shared eligibility rule: a reminder letter goes only to **receivable** accounts (Sundry Debtors ancestry — the letter's own text is unconditionally receivable-direction), while a confirmation of accounts is direction-neutral and goes to either party side. The gate that offers a document and the gate that produces one are literally the same predicate, so they cannot drift. 🔴 **PARTIAL: the DELIVERY CHALLAN is still ABSENT** — it is the Delivery Note voucher printed, our projector is inventory-blind, and no drill route reaches it. *Prior evidence, now historical:* Zero hits for the reminder-letter and confirmation-of-accounts phrases; the nine "Delivery Challan" hits are all e-Way-bill document-kind prose, and no printable challan exists. 🔴 **THE EVIDENCE CELL DESCRIBES THE WRONG SHAPE FOR TWO OF THE THREE — CORRECTED 2026-09-04 (§1.3 item 22). Grade `ABSENT` unchanged.** The cell reads as *"no printable challan/letter/confirmation exists"*, which scopes the work as three new documents. **It is not.** In the reference product the **reminder letter** and the **confirmation of accounts** are not standalone documents at all — they are **multi-account print/export outputs reached from the `Alt+P` / `Alt+E` menus** (*Print Multi-Account Reports* → *Print All Ledger Accounts*, *Print Confirmation of Accounts*; *Export Reminder Letters*, *Export Confirmation of Accounts*). **They are therefore blocked behind the missing menu shell (T2-20), not behind three document templates**, and the **delivery challan** is the **Delivery Note voucher** printed — our projector is inventory-blind and no drill route reaches it. **In T1-14.** *(Deposit slip and banking payment advice are 8.6 and 8.7.)* |
...
#### Area 14 — TallyPrime-only capabilities · 10 rows · 1 complete / 9 partial / 0 absent
...
| # | Capability | State | Evidence · gap |
|---|---|---|---|
| 14.1 | Go To (Alt+G) — the jump-anywhere overlay | PARTIAL | 🔴 **STATE MOVED `ABSENT` → `PARTIAL` 2026-09-07 (A15, at `origin/main` `cb8cccb`). PR #53 BUILT IT AND NOBODY MOVED THE ROW.** **ROUTE, WALKED:** `MainWindow.axaml.cs`'s `OnKeyDown` opens with the Go To block — `Key.G` + Alt fires `MainWindowViewModel.ToggleGoTo` from **anywhere**, deliberately ahead of every screen guard, and while the overlay is up the same block owns Up / Down / Enter / Escape so the operator cannot drive the cascade hidden behind it. The **same door** is on the button bar as the `Alt+G · Go To` badge (`MainWindowViewModel.BuildButtonBar` binds `ToggleGoTo` with the one `CanOpenGoTo` predicate the key arm shares, so bar and chord can never disagree). `OpenGoTo` builds a fresh index by `BuildGoToIndex` → `WalkGoTo`, which walks the **real Gateway menu** rather than a hand-written table — so a row added to any menu builder is findable automatically — and `ActivateGoTo` replays the destination's Group path to land on the screen. Typed search + ranked results live in `GoToViewModel`; the ranking (prefix → substring → section) is labelled **OURS** under ruling 9 because the vendor states the search but not the match rule. 🔴 **PARTIAL — THREE NAMED MISSING PIECES.** The vendor's Go To offers categorised actions, not only a report index: **(a) `Alter Master` is not there.** The vendor gives *"Alt+G (Go To) > Alter Master > Voucher Type"* verbatim; `grep -rn "Alter Master" src/` returns **two hits and both are doc comments**, never a menu group — alteration in this product is Ctrl+Enter on a list, so Go To cannot reach it. **(b) `Show Opened Reports` does not exist** — zero hits. **(c) `MenuItemKind.Action` rows are deliberately excluded** from the index (a Go To that ran *Quit — Change Company* would close the book out from under the screen); that is ours, labelled, and it means Go To navigates but never runs verbs. **Ctrl+G "Switch To" is row 14.2, not this row.** ▼ *Original evidence, superseded:* **Zero occurrences of `Key.G`** in the whole of `src/Apex.Desktop`, and zero for the identifiers and phrases. No view model, no Screen member, no menu row, no key arm. **In T2-7.** |
| 14.2 | Switch To (Ctrl+G) — ~~between simultaneously loaded companies~~ **to a different report** | PARTIAL | 🔴 **THIS ROW'S TITLE WAS A MISREADING, CORRECTED 2026-09-06 (PR #57, wave-7 D4), AND THE CORRECTION CHANGES WHAT IS OWED.** `help.tallysolutions.com/tally-prime/keyboard-shortcuts-tally/` gives `Ctrl+G` verbatim as *"To switch to a different report, and create masters and vouchers in the flow of work"*; the one documented difference from `Alt+G` (Go To) is that **Switch To does not return you to where you were**, which is why the screen REPLACES the cascade. **It is not a multi-company feature.** Company switching is the vendor's `F3` / `Alt+F3` / `Ctrl+F3` family and belongs to **14.9**. ⚠️ **Consequence for user ruling U-10**, which gates this row as part of the *"multi-company shell (14.2 Switch To, 16.7 Group Company)"*: **the 14.2 half of that gate rests on the misreading.** What shipped is the documented report-switcher — a 99-row destination list with prefix filtering and auto-scroll — and **no multi-company shell**. **PARTIAL** because the row is graded against a corrected title, not the one the gate was written against; the user should confirm both the retitle and that U-10 now covers only 16.7. ⚠️ Grade assigned by A12: the fixer reported the row as *"advanced"* without stating a grade. *Prior evidence, now historical:* Absent **and structurally impossible**: the shell exposes a single nullable `Company` and the open path **replaces** it. There is no collection of loaded companies to switch between — zero hits for either candidate field. **In T2-7.** |
| 14.3 | Graphical dashboard / any chart at all | PARTIAL | 🔴 **BUILT AND LANDED 2026-09-07 — PR #67 (`claude/apex-f3-nav-finish`). Was ABSENT.** Shipped: `Screen.Dashboard`, `DashboardViewModel`, `ChartGeometry`, all three vendor dashboard types (`DashboardKind` Default / Sales / Purchase) and **two mark types** — a Bar tile now builds no line vertices and a Line tile no bars, which it did not do on the first cut. No pie chart, matching the vendor's documented deliberate non-feature. Every figure is read from the already-tested `VoucherRegister`; nothing is re-computed. ⚠️ **Grade assigned by A12, not by the track** — the fix report described outcomes finding-by-finding and stated no grade. **Two gaps hold it below COMPLETE, both now SOURCED rather than unknown, which is itself a correction this PR made:** **(a)** the Alt+C **Configure Tile** panel is **read-only and configures nothing** — the vendor ships eight options there (Name of Tile, Type of Value, Display type, Graph Type, Show Percentages, Scale Factor, Sorting Method, Position of Tile, per `help.tallysolutions.com/dashboard-in-tallyprime/`) and **we ship none of the eight**; the panel stays read-only deliberately, because controls that write nowhere are the dead-capability shape this project has filed three times, and a test now forbids its operator-facing string from ever again claiming the option set "has not been sourced"; **(b)** no **startup preference** — the vendor's `F1 > Settings > Startup` "Open Dashboard on loading a Company" needs a Settings host that does not exist in `src/`. ⚠️ **The ROUTE is NOT a gap, and the code used to say it was.** A comment claimed the vendor reaches the dashboard from `F1 > Settings > Startup` and that our placement diverged; the vendor's documented route is **"Gateway > Dashboard"** (or Alt+G > Dashboard) and our Dashboard row sits on the root Gateway column, so it **matches** — the invented divergence has been struck. **Known residual, reported not buried:** the chart is not responsive (460x150 pinned in six XAML places plus two consts); 125% DPI untested. ▼ *Original ABSENT evidence, kept because it is what the row moved FROM:* Zero chart identifiers in `src/`, and **zero `<Polyline>`, `<Path>`, `<Canvas>` or `<PathGeometry>` elements in the ~16,000-line main view**. The only "dashboard" hits are IMS prose and a comment describing Ratio Analysis as a flat label/value dashboard. **Directly contradicts `plan.md`'s claim of a delivered graphical dashboard** (already a Tier 3 row). **In T2-7.** |
| 14.4 | More Details (Ctrl+I) — the optional-field side panel | PARTIAL | 🔴 **BUILT AND LANDED 2026-09-07 — PR #72 (`claude/apex-g2-more-details`). Was ABSENT, and the ABSENT evidence — *"zero identifiers and zero phrase hits; the chord is taken"* — is now spent: user ruling 17 gave Ctrl+I to this panel and the item-invoice toggle moved onto the Ctrl+H mode cycle.** Shipped and keyboard-reachable: `Screen.MoreDetails`, `MoreDetailsViewModel`, the `Ctrl+I` arm of `ShellChordTable` gated on `CanOpenMoreDetails`, arrow navigation and an Enter that reveals the field **on this voucher only** without changing the screen option. **PARTIAL, not COMPLETE, and the product itself says why:** only **two** vendor field groups are offered — Bill-wise Details and Batch / Lot Details — and `MoreDetailsViewModel.WithheldFields` names the three it cannot reach, each with the reason (per-line **Ledger Narration**, because `EntryLine` has no narration field and adding one is a schema change this track did not take; **Select Unused Voucher Nos.**, because nothing in this build tracks the gaps behind the next number; **PAN / CIN**, because `Company` has neither field — the `Pan` on `Employee` and the `Cin` on `GstChallan` are different identifiers on different aggregates). A test locks the footnote against that list, so a later slice cannot quietly drop the disclosure and claim the row COMPLETE. When the panel has nothing to offer it **opens and says so** rather than swallowing the key — the dead-chord defect this row was graded ABSENT for. ⚠️ **Grade assigned by A12 on merge, not by the track.** **Still in T2-7 for the remaining three groups.** |
| 14.5 | The standard report button-bar options — Change View, Basis of Values, Monthly Summary, Value Range, Scale Factor, Vertical Balance, number-of-decimals, Alt+U Unhide | PARTIAL | 🔴 **STATE MOVED `ABSENT` → `PARTIAL` 2026-09-07 (A15, at `origin/main` `cb8cccb`). TWO OF THE EIGHT NOW EXIST — AND SIX STILL RETURN ZERO, SO THE ROW STAYS WIDE OPEN.** **What ships:** **Basis of Values (Ctrl+B)**, carrying the **Scale Factor**. `MainWindow.axaml.cs`'s `OnKeyDown` binds `Key.B` + Control → `MainWindowViewModel.OpenBasisOfValues` → `BasisOfValuesViewModel`, pushed as its own Miller cascade column and rendered in `MainWindow.axaml`. 🔴 **The key arm is guarded on the EXACT predicate the button-bar row is enabled on** (`IsReportContext && Reports is { SupportsScaleFactor: true }`), so the chord is never swallowed on a report that cannot scale — a badge honestly dimmed beside a key that fires nothing is the disagreement this project has caught before. `ReportScales.All` offers the vendor's six factors (Default · Hundreds · Thousands · Lakhs · Millions · Crores) and `Apply` re-runs the engine over the UNSCALED books, dividing only display cells. **The build agent's own remarks state the fraction and this measurement agrees with them: two of eight.** 🔴 **THE SIX THAT ARE STILL ABSENT, each a zero-hit search over `src/` re-run at this HEAD:** *Change View* · *Value Range* · *Vertical Balance* · number-of-decimals · **Alt+U Unhide** (there is **not one `Key.U` arm** in `MainWindow.axaml.cs`) · and *Monthly Summary* **as a bar option on the open report** — a Ledger Monthly Summary exists (row T1-32) but it is a drill-down report on Account Books, not a button-bar option that re-shapes whatever report is in front of you. **Also NOT carried inside our Ctrl+B, named rather than implied:** the vendor's *Stock Valuation Method*, *Godown type* and *Stock Position* on stock reports, *Type of Voucher Entries*, *Forex Transactions*, and the free-entry **#New Number** divisor (carved out deliberately — an unbounded user-typed divisor on a financial statement). ▼ *Original 2026-08-18 evidence, superseded in its first clause only — the survey disagreement it records is still the right reading of the other six:* 🔴 **SURVEY DISAGREEMENT, RECORDED.** ~~Both surveys agree **none of the eight exists**~~ (true then; Ctrl+B was bound afterwards): per-term greps return zero, and ~~the only "Basis of Values" hits are three doc comments explaining why Ctrl+B was deliberately **not** bound~~ *(**false since PR #53** — Ctrl+B IS bound; see the 2026-09-07 note above)*. Alt+U is unbound **and still is at `cb8cccb`**; the Ctrl+H report arm does not exist (that chord is voucher-mode only). One survey graded the row **PARTIAL** on the ground that a context-rebuilt button bar **does** exist and that two adjacent options have real counterparts — **but those two are counted separately at 11.16 (F12 report config) and 11.17 (Alt+N Auto Columns)**, and the bar's actual contents are our own app-wide quick-jumps, not the open report's options. **ABSENT is recorded for the eight named options**, with the disagreement stated rather than reconciled away. **In T2-7.** |
...
| 14.9 | Company menu (Alt+K: Create / Alter / Select / Shut Company) | PARTIAL | 🟡 **Graded 2026-09-06 (PR #57, wave-7 D4).** A `CompanyMenuViewModel` is built and routed, and the same PR closed the two ways this menu destroyed work: **Ctrl+F3 (Shut) silently discarded a half-keyed voucher**, and so did **Alt+K**, which clears sub-screens *before* building the menu and so would have defeated a Shut-row guard entirely. Both now refuse and say why. It also fixed a blank window — `Alt+F3` then `Ctrl+G`/`Alt+K` raised a panel into a hidden cascade, because the company-select screen keeps `Company` non-null while holding no cascade. **PARTIAL:** the chord collision this row records is real and **unresolved** — `Alt+K` is also Saved Views (14.7); which owner wins is a user ruling, not a build agent's call. ⚠️ Grade assigned by A12; the fixer reported the row as *"advanced"* without stating a grade. *Prior evidence, now historical:* No company-menu column was built anywhere in the root builder, and **Alt+K is already consumed by Saved Views** (14.7). Create and Alter are reached instead from Company Select and a Masters row. §1.3 row 9 records the company menu as **owed, not refused**; this confirms it is still owed at HEAD. |
| 14.10 | WhatsApp sharing of a document | PARTIAL | 🔴 **BUILT AND LANDED 2026-09-07 — PR #67 (`claude/apex-f3-nav-finish`). Was ABSENT.** Shipped: `Screen.WhatsAppShare`, `WhatsAppShareViewModel`, a `W` / Ctrl+W chord on any printable page, and a prepared `wa.me` link beside the saved document. ⚠️ **Grade assigned by A12, not by the track.** **What we do NOT do, said on the panel's own face and locked by a test:** nothing is sent and the file is not attached — the notice reads *"Apex Solutions does not send the message and cannot attach the file — WhatsApp's link carries text only."* That is a **sourced** limit, not a shortcut: `help.tallysolutions.com/share-documents-using-whatsapp-for-business-faq/` requires WhatsApp Business API registration through a commercial BSP (Interakt), and Meta forbids re-using a number already on a WhatsApp Business account, so there is no account-free path. **Gap holding it below COMPLETE:** the vendor genuinely *sends* the document through that WABA; we hand over a link the operator completes by hand. 🔴 **AND ONE R12 DECISION IS OPEN AND OWED TO THE USER (register row IV-64):** we put this on our own `W` / Ctrl+W. The justification on file said the vendor's chord "could not be reused" — **false**. The vendor's chord is **Alt+M** (Share > WHATSAPP) and `Alt+M` is **unclaimed** in this application: the only `Key.M` arm explicitly excludes Alt. Moving the Share access point onto Alt+M as a channel picker would match the vendor's shape *and* keystroke and **costs nothing** — the original framing concealed that. ▼ *Original ABSENT evidence, kept because it is what the row moved FROM:* Case-insensitive search for the name across every `.cs` and `.axaml` in `src/` returns zero. No service, no button-bar row, no Screen member. |


#### Area 15 — Statutory, obsolete by law (pre-GST) · 6 rows · 0 complete / 4 partial / 2 absent

> **▶ 🔴 THREE ROWS WERE STRUCK FROM THIS AREA ON 2026-09-07 BY USER RULING 20 (R12, `plan.md` §5,
> `TWO FURTHER USER RULINGS (R12, 2026-09-07)`): 15.3 (the 2005 four-slab VAT rate structure), 15.7 (Service
> Tax + Form ST3) and 15.9 (Fringe Benefit Tax). THE AREA WAS NINE ROWS AND IS NOW SIX.**
> **THE MECHANISM, STATED EXPLICITLY BECAUSE IT MOVES THE DENOMINATOR:** a struck row **leaves §1.2a
> altogether** and lives in **§1.2d**, below §1.2c, with its full text, its state history and the repeal that
> killed it. It is therefore **out of the in-scope count** — exactly as §1.1 rule 4's exclusions are — and it
> is **not** given a fifth state token inside §1.2a, which would redden
> `tests/Apex.Ledger.Tests/CensusDerivationDriftLockTests.cs` and break the very derivation this document
> exists to keep honest. **The three rows are not deleted and they are not completed.**
> 🔴 **AND THE THING A LATER READER MUST NOT MISREAD: STRIKING A ROW LOWERS THE DENOMINATOR WITHOUT SHIPPING
> ANYTHING.** All three were `ABSENT`, so the *"absent"* count falls by three and every percentage improves —
> **and not one line of code was written.** §1.2's banner says so in the same words; §1.2d says it a third
> time on each row. **Nothing about this move is progress.**
> **▶ THE REMAINING SIX ARE STILL BUILD ROWS UNDER RULING 10, AND RULING 20 SAYS WHAT IS LEFT TO BUILD:**
> what is **still levied** — **VAT and CST on alcoholic liquor for human consumption** (Art. 366(12A) puts it
> permanently outside GST) and on the **five petroleum products** (s.9(2) CGST Act defers GST on petroleum
> crude, HSD, petrol, natural gas and ATF), and **central excise on tobacco and those petroleum products**
> under the **Fourth Schedule** to the Central Excise Act 1944. Rows 15.1, 15.2, 15.4, 15.5, 15.6 and 15.8
> are re-read against that narrower target; **nothing in them is re-graded by this edit.**
>
...
| # | Capability | State | Evidence · gap |
|---|---|---|---|
| 15.1 | State VAT — enable, dealer type, TIN, registration date | PARTIAL | `\bVAT\b` case-sensitive over `src/` → **zero**; `EnableVat`, `VatEnabled`, `VatDealer`, `VatTin`, `VatRegistration` → **zero**; the identifier shapes `\bTin[A-Z]`, `[A-Za-z]Tin\b`, `\bTin\b` → **zero**. The F11 statutory screen carries **GST / TDS / TCS / Payroll only**; the sole TIN-shaped field in the product is `GstConfig.Gstin`. ▼ 🟢 **GRADED UP 2026-09-08 BY A12 ON MERGE OF PR #86 (wave N, schema v59).** Every zero above is now stale and is kept only as the baseline. Route: **F11 (Features) → "Enable Value Added Tax (VAT)" → Company VAT Details**, with the scope warning rendered *above* the switch. TIN, Interstate sales tax number, VAT applicable from, Type of Dealer, Periodicity and CST Rate Against Form C persist in v59 and reload; pinned by `The_F11_VAT_registration_persists_and_reloads` (which asserts the RELOADED company re-opens with the block on) and `Company_VAT_registration_round_trips_and_an_unset_CST_rate_stays_null`. **WHY NOT COMPLETE:** the state filing block (LVO/VSO code, authorised person, status, place) and the VAT-commodity master are NOT built — both only mean anything against a state return form, which is the half of 15.5 that could not be sourced. 🔴 **NO STATUTORY RATE IS ASSERTED ANYWHERE IN THIS SLICE.** The concessional Form-C rate has **no default and none may be added**: `indiacode.nic.in` refused every copy of the CST Act 1956, so the field is operator-supplied and the report says the rate is unset rather than showing a figure nobody sourced — deliberately unlike the open T0-6 exposure. |
| 15.2 | VAT & Tax Classifications (`Input VAT @ 4%`, `Output VAT @ 4%`) — 🔴 **TITLE CONTESTED, RETITLE OWED (R6/R12), NOT RENAMED HERE** | PARTIAL | Four independent regexes — `tax ?classification`, `taxclassification`, `input ?vat`, `output ?vat` — all **zero**. The product's tax-head masters are the six GST ledgers `GstService.EnableGst` auto-creates. ▼ 🟢 **GRADED UP 2026-09-08 BY A12 ON MERGE OF PR #86 (wave N, schema v59).** Routes: **Masters → Stock Item → VAT Details** (class of goods + Tax rate, the rate box disabled until the class permits) and **Masters → Ledger → VAT Details** (two mutually exclusive blocks — party identity vs sales/purchase treatment). Pinned by `The_item_master_gates_the_VAT_rate_on_the_class_of_goods_and_persists_it` and `Item_goods_class_and_the_two_ledger_VAT_blocks_round_trip`. 🔴 **THE GATE IS THE CLASS OF GOODS, BY CONSTRUCTION, AND IT REFUSES RATHER THAN HIDES.** `VatService.SetItemVatRate` **throws** on ordinary GST goods — a hidden-but-settable field is exactly how an ordinary item acquires a liquor rate — and tobacco is refused with a *different* reason (inside GST; central excise also applies). **A12 MUTATION-VERIFIED THIS** rather than trusting the track: guarding the refusal with `false &&` reddened exactly `A_VAT_rate_is_refused_on_ordinary_GST_goods` and the `None` and `Tobacco` cases of `Only_the_six_classes_outside_GST_accept_a_VAT_rate` with `Assert.Throws() Failure: No exception was thrown`, the other 20 staying green; restored byte-identical, CR count unchanged at 220. 🔴 **THE ROW TITLE IS WRONG AND A RETITLE IS OWED — IT WAS DELIBERATELY NOT RENAMED HERE.** The title names `Input VAT @ 4%` / `Output VAT @ 4%` *named classifications*; the vendor's current screens do not ship that list, carrying instead **"VAT Applicable"**, **"Set/Alter VAT details"** and **"Tax rate"** on the ledger and stock-item masters, and **"Type of Dealer"** / **"TIN/Sales Tax No."** / **"CST No."** on a party ledger. What ships is the vendor's fields. Retitling the row is a `plan.md`/census scope change and therefore a **user decision (R6/R12)**; A12 records it as owed rather than quietly renaming the thing the grade is measured against. |
| 15.4 | VAT Composition scheme | ABSENT | `composition` returns **920 hits across 83 files and 100% of them carry a GST section reference** (§10, §10(2A), Sch-II 6(b), CMP-08, GSTR-4). 🔴 **RE-CONFIRMED 2026-09-08 (wave N, PR #86) — STAYS ABSENT, AND THE REASON IS SOURCING, NOT CAPACITY.** The track that shipped 15.1/15.2/15.5/15.6 went looking and **no vendor page it could reach attests a VAT composition SCHEME separately**: everything returned is either GST composition (current law, already in this product as `CompositionTaxService`/CMP-08/GSTR-4) or the VAT dealer **type**. The one piece that IS attested is the dealer-type value *"Composite"* — `help.tallysolutions.com/.../configuring-vat-masters-tally/` states the *"VAT TIN (Composition)"* field *"can only be entered when the Type of Dealer is selected as Composite"* — so `VatDealerType.Composite` ships as part of 15.1/15.2 and **nothing computes a composition rate, base or return anywhere**. An unsourced tax scheme is what this project has already had to strip out of shipped code (T0-6), so the honest smaller thing was shipped and this row was left alone. ▼ *Original evidence:* Zero reference a State VAT composition scheme. Adjacent-but-different, **not counted**: `CompositionTaxService`, `CompositionThreshold`, `CompositionSubType`, `Cmp08`, `Gstr4` — all current law. |
| 15.5 | VAT Reports (VAT Computation + state return forms) | PARTIAL | `vat ?computation` → **zero**; `state ?return` → **zero**; and the `ReportKind` enum — the **single dispatch point for every report in the product** — was enumerated in full and carries **no VAT member**. No route can reach a VAT report. ▼ 🟢 **GRADED UP 2026-09-08 BY A12 ON MERGE OF PR #86 (wave N) — THE COMPUTATION HALF ONLY.** Route: **Reports → Statutory Reports → VAT Reports → VAT Computation**, reachable only while F11's VAT switch is on. The `ReportKind` enum now carries the VAT members, and **A12 read the reachability out of the shell wiring rather than a view model in isolation** — the menu column, the dispatch case and the enum arm are all present, and `CstDeclarationFormsReport`/`VatComputation` are consumed by `ReportsViewModel`, `MainWindowViewModel` and `MainWindow.axaml`. The report carries the vendor's Particulars of Computation Details: Sales and Purchases each split local/inter-State × taxable/exempt, VAT Payable or Refundable, and CST Payable. Pinned by `VAT_Computation_counts_only_the_goods_outside_GST` (an ordinary GST item on the **same invoice** contributes nothing) and `The_VAT_Computation_report_carries_its_own_disclosure_rows`. 🔴 **WHY NOT COMPLETE, AND THE REASON IS SOURCING NOT TIME:** the ~30 **state return forms** are the other half and are NOT built. **AND NOTHING ON THIS REPORT IS A POSTED LEG** — VAT liability is computed and reported, never posted, so a figure here is not in the Trial Balance; the report says so on its own face. |
| 15.6 | Central Sales Tax — 2% interstate, C/F/H declaration forms | PARTIAL | `\bcst\b` case-insensitive over `src/` and `tests/` → **zero**; `c-?form`, `form ?[cfh]\b`, `concessional` → **zero**; the one `declaration ?form` hit describes a **C# method declaration form**. The **432** `interstate` hits are IGST place-of-supply routing, not CST against a C-form. ▼ 🟢 **GRADED UP 2026-09-08 BY A12 ON MERGE OF PR #86 (wave N) — BOTH CAPABILITIES SHIPPED.** Voucher-side declaration-form **capture** (form type, series, number and date on the voucher, written through the vendor's own Alt+S panel) **and** the **Forms Receivable / Forms Issuable** tracking registers, split three ways NotRecorded / Pending / Completed. Route: **Reports → Statutory Reports → VAT Reports → Declaration Forms → Forms Receivable** or **Forms Issuable**, then **Alt+S** = *Set/Alter Form No.* Pinned end to end by `Alt_S_on_Forms_Receivable_records_a_declaration_form_that_survives_a_reload` — it drives the report, selects the row, presses Alt+S, keys the form, accepts and **reloads from SQLite** — and by `The_form_number_alone_moves_a_row_out_of_pending`, `A_sale_is_receivable_and_a_purchase_is_issuable_and_never_both` and `A_cancelled_sale_is_not_on_the_declaration_forms_report`. A blank form number must **not** coerce to an empty string, or a pending form reloads as completed and the exposure vanishes; that is pinned in the schema tests. **A12 VERIFIED THE VENDOR CITATION FIRST-HAND** at `help.tallysolutions.com/tally-prime/reports/forms-receivables-tally/`, which returns verbatim *"The Forms Receivable report displays the declaration forms to be received from the dealer at any given point of time"*, the date / CST ledger / gross-amount-inclusive-of-CST / form-type columns, the series-number, form-number and form-date fields, and the **Alt+S (Set/Alter Form No)** button. **WHY NOT COMPLETE:** the concessional rate itself is operator-supplied — see 15.1 — so CST Payable is null until someone supplies a rate, and no 2% figure is asserted by this build. |
| 15.8 | Excise — the F12 invoice-format route; Excise for Dealers (RG23D / Form 2); Excise for Manufacturers | ABSENT | `excise`, `rg23`, `cenvat`, `modvat`, `form ?2\b` → **five hard zeros**. The word does not appear even in a comment in `src/` or `tests/`. `tariff` → 4, all GST rate-notification Schedule citations; `duty` → 3, all the "Duties & Taxes" group or §200's *duty to deposit*. ▼ 🔴 **RE-CONFIRMED 2026-09-08 (wave N, PR #86) — STAYS ABSENT, AND THE REASON IS SOURCING, NOT TIME.** The **route** and the **report names** are attested: `help.tallysolutions.com/tally-prime/dealer-excise-reports/ed-rg-23d-register/` gives *"Display More Reports > Statutory Reports > Central Excise Reports > Dealer > Form RG23D"* and `…/ed-export-form2-tally/` gives Form 2; the Enable-Excise-for-Dealers field list (Unit name, Registration type, ECC number, Range/Division/Commissionerate via F12, Enable RG23D numbering) is attested too. **But the RG 23D register's COLUMN SET is published only as screenshots** — the page carries no text listing of the column headings — so building the register would have meant **inventing the columns of a statutory register**, and the track stopped rather than fabricate. ▼ **GROUNDWORK LANDED AND LABELLED:** `NonGstGoods.AttractsCentralExcise` already returns the correct and **different** set (the five petroleum products **+ tobacco**, and **not** alcoholic liquor, which is a State excise subject), so a later excise slice cannot re-derive a wrong set from the same enum. ⚠️ **A12 NOTE:** that predicate currently has **no caller in `src/`** — it is deliberate, labelled groundwork rather than a reachable capability, and it is recorded here so a later reader does not mistake it for shipped excise support. |

#### Area 16 — Formerly excluded by decision (security, audit, data structure) · 7 rows · 0 complete / 6 partial / 1 absent
...
| # | Capability | State | Evidence · gap |
|---|---|---|---|
| 16.1 | The company vault — company-data encryption behind a user passphrase | PARTIAL | 🔴 **MOVED `ABSENT` → `PARTIAL` 2026-09-14 (PR #104, W-T1, user ruling 23). THE GRADE IS A12's OWN, NOT A REVIEWER'S** — no independent reviewer saw this branch. **Unlike row 2.6, the old evidence cell was ACCURATE and current: A12 re-measured it on `origin/main` at `9a83de6` and `passphrase` across `src/` and `tests/` really did return zero, with zero `CompanyVault`/`CompanyRegistry` files.** It read: ~~*"`TallyVault` and `passphrase` over `src/` and `tests/` → both zero. The 24 crypto hits are one non-matching capability: `SqliteNicCredentialStore` AES-CBC over the four `nic_*_enc` columns, keyed from a hard-coded application pepper its own file calls an 'obfuscation-grade placeholder'."*~~ **ROUTE (de-branded to *Data Vault* per R7/ruling 9 — the shipped app must never say the vendor's name):** `Alt+K` (Company) → *Data Vault* → three masked boxes → `Ctrl+A` sets or changes the passphrase; `Alt+D` leaves. *Reopening:* Company Select → the asterisked row → passphrase → `Ctrl+A`. SQLCipher page-level encryption, passphrase never stored, changing it exports to a new file rather than rekeying in place. **EVIDENCE:** `CompanyVaultReachabilityTests` (8 tests, a new file — the feature previously had **zero** tests that any user could reach it and **none** that a vaulted company could be reopened) drives the real `MainWindow` through the real keystrokes and then asserts **against the file system** that the book is encrypted and the name is gone from every filename; `CompanyRegistryTests` (15) including a raw-byte scan of every file; `CompanyVaultTests` (19) including live-native KDF pragmas. 🔴 **THE FEATURE WAS BROKEN ON ARRIVAL IN EXACTLY THE WAY IT EXISTS TO PREVENT, AND THAT IS WHY THIS ROW IS GRADED CAUTIOUSLY:** vaulting a company left its name **legible in plaintext** in `Companies/companies.index`, because the `UPDATE` of `display_name` to `NULL` frees the old b-tree cell without erasing it. Fixed with `PRAGMA secure_delete = ON` plus a VACUUM scrub, and mutation-verified. **A negative mutation result is recorded rather than hidden:** removing the VACUUM scrub left all 15 registry tests green — the guard was **dead as written** — so a test was added for the case it actually defends (bytes freed *before* the pragma was ever on), after which both halves are independently load-bearing. **WHY PARTIAL:** 🔴 **backup is vault-unaware — a vaulted company cannot be backed up**, pinned by its own test rather than left to be discovered. **VENDOR GROUNDING (R7, ruling 14):** `help.tallysolutions.com/tallyvault-for-company-tally/` and `…/data-security-faq/`, re-read 2026-09-14 and confirmed verbatim for the `Alt+K` route, the three fields, that the company **name itself** is encrypted, that there is **no recovery**, and that a vaulted company lists as asterisks with its number still visible. **NO SCHEMA WAS TAKEN and v64 was released back to the pool:** the registry is a separate SQLite file with its own `CREATE TABLE IF NOT EXISTS`, must be readable *before* any company is open, and a vaulted book cannot be read at all without a passphrase — so the company schema ladder would be the wrong place in principle as well as unnecessary. |
| 16.2 | Security Control — users, roles, security levels, password policy | PARTIAL | 🔴 **MOVED `ABSENT` → `PARTIAL` ON 2026-09-07 BY PR #75 (schema v56). THE GRADE IS A12's, AND `PARTIAL` RATHER THAN `COMPLETE` IS THE POINT OF IT.** ~~The 2026-08-18 measurement: `SecurityLevel`, `PasswordPolicy`, `IUserRepository`, `UserId`, `security level`, `user account`, `LoginUser`, `Authenticat` → all zero, and zero of the 182 tables named a user, role or permission.~~ Struck, not deleted: every one of those searches now returns. **WHAT SHIPPED** — `Apex.Ledger.Security.PasswordHash` (PBKDF2-HMAC-SHA256, per-password 128-bit salt, 600,000 iterations stored beside the hash, `FixedTimeEquals`); the `SecurityControl` aggregate with both seeded level names verbatim, the six access types, the allow/deny rule algebra and both back-dating limits; `CompanyUser`; the password policy; three tables and three `companies` columns at v56; and two screens on the vendor's own route — *"Press **Alt+K** (Company) > **Users and Passwords**"* and *"Press **Alt+K** (Company) > **Password Policy**"* (`help.tallysolutions.com/manage-users-in-tallyprime/`), each proved reachable by **keyboard through the realised visual tree**, not by method call. **WHY NOT `COMPLETE` — THE RULES ARE RECORDED, NOT ENFORCED.** `SecurityLevel.IsAllowed`, `SecurityLevel.AllowsVoucherDate` and `SecurityControl.Authenticate` all decide correctly and are tested, and **none of the three has a caller outside the security screens and the tests**: there is no sign-in step, and no report, master or voucher screen asks a level for permission before opening. That is disclosed **on the screen** (`SecurityControl.EnforcementNotice`, pinned by a test) rather than left to be discovered. Also outstanding: the **edit-log actor** (the design's slice S5, deferred to wave 14). **A12 DID NOT TAKE THE GREEN SUITE AS EVIDENCE** — three mutations: disabling the last-Owner **demote** guard reddens `The_last_owner_user_cannot_be_demoted`; swapping `FixedTimeEquals` for `SequenceEqual` reddens the IL-reading constant-time test **while every functional password test stays green**; drifting the v56 `DEFAULT 0` to `1` reddens four independent tests. See **IV-65** for the divergences and the irrecoverable-lockout consequence. |
...
| 16.5 | Split Company Data by financial year | PARTIAL | 🔴 **MOVED `ABSENT` → `PARTIAL` ON 2026-09-07 BY PR #76. THE GRADE IS A12's.** ~~`SplitCompany` → zero; every `\bsplit\b` in `src/` was a different concept and all Split-Company-Data mentions were docs-only.~~ Struck, not deleted. **WHAT SHIPPED** — `Apex.Ledger/Services/CompanySplit.cs` (the three vendor modes, the checks, the two shaping functions; no UI, no persistence), `Apex.Desktop/Services/CompanySplitService.cs`, and a Split Data panel reached by arrows and Enter through the Gateway's Data section. Vendor: `help.tallysolutions.com/split-company-data-tally/` and its FAQ (read 2026-09-07) — the two quoted ranges **partition the book with no overlap and no gap**, so a voucher dated **on** the split date goes to the from-book, and *"the original company will remain as it is"*. The source is proved untouched **three independent ways**: structurally (the service re-loads the `.db` and never sees the open aggregate), at engine level over both fixtures, and by comparing the source file's **bytes** across a real end-to-end split. Reconciliation is asserted as **identities against the unsplit book**, not as expected figures, and the opening carry is computed from **ledger balances, never from a report**, so the from-book's opening trial balance balances by arithmetic under every closing-stock basis. **WHY NOT `COMPLETE`, two reasons.** (a) **A large class of real books cannot be split at all**: the engine **refuses**, naming the reason, when the book carries bill-wise references still open at the cutoff (a `Ledger` has an opening balance but **no opening bill-wise breakdown** — that needs a schema version), payroll attendance (each spans a date *range*), voucher-linked statutory/e-document rows that would straddle, or a stock rate that would not carry paisa-exact. The vendor splits regardless and carries them. (b) **The route is a Back-press off the vendor's**: `Alt+Y` still opens the sibling *Backup / Restore* column, not a Data menu containing Split. **A12's mutation check**: relaxing the boundary from `>=` to `>` reddens `A_voucher_dated_on_the_split_date_lands_in_the_from_book` — **and the whole reconciliation Theory over both fixtures stays green through it**, so that boundary rests on the one dedicated test, not on the reconciliation gate; disabling the bill-wise refusal reddens `A_book_with_open_bill_wise_references_is_refused`, whose non-vacuity partner proves the guard does not over-fire. See **IV-66 · IV-66a · IV-67 · IV-68 · IV-69 · IV-70**. |
| 16.6 | Repair / Rewrite / Verify company data | PARTIAL | 🔴 **SURVEY DISAGREEMENT, RECORDED, AND THE STATE MOVED.** §1.2a's area-13 held-out note said **ABSENT** on 2026-08-18, reasoning *"never a user command"*. The 2026-08-19 measurement finds a real `CompanyBackup.IntegrityCheck` running `PRAGMA integrity_check`, **called on both write paths** — `Create` refuses to archive an already-corrupt database, and `Restore` validates the payload with it plus a SHA-256 digest and a schema-version cross-check — reachable through the Gateway's Data → Backup / Restore rows. **The ABSENT bar is *"no type, no route, no caller; every absent row rests on a search that returned zero"*, and this search did not return zero**, which is what decides it. **NAMED MISSING PIECES, two: (a) no standalone Verify verb** — `verify.{0,25}(company\|data\|book)` over `src/` → **zero**; the check is only ever a gate *inside* backup and restore. 🔴 **PIECE (a) IS CLOSED AS OF 2026-09-07 BY PR #76 (A12's grade); THE ROW STAYS `PARTIAL` BECAUSE (b) DOES NOT MOVE.** `CompanyBackup.Verify(path)` is now a verb of its own, reached at Gateway → Data → Split Company Data → **Verify Data**: it opens the file **read-only** (a test compares the file's bytes *and* its last-write time across a verify) and checks existence, SQLite-ness, our schema stamp, readability by this build, `PRAGMA integrity_check`, **`PRAGMA foreign_key_check`** — the orphan-row check no other path in this codebase runs — and exactly one company row. **(b) Nothing repairs or rewrites** a database found bad — `RepairCompanyData` → **zero**, no `VACUUM` or rebuild path; the only `Repair*` member is `VoucherTypeResolver.RepairSupersededSeedShortcuts`, a load-time seeded-shortcut fixup its own doc calls *"not a schema migration"*. **The area-13 note is corrected in place.** |
...
#### 1.2d 🔴 STRUCK BY USER RULINGS 20 (2026-09-07) AND 24 (2026-09-14) — OUT OF SCOPE, AND **NOT** COMPLETE

> **▶ 🔴 SECOND STRIKE, 2026-09-14 — USER RULING 24. TWO MORE ROWS LEAVE THE DENOMINATOR AND NOTHING WAS BUILT.**
> **`221 → 219`**, and the absent count falls from 9 to 5 — **two of those four came from here, not from work.**
> Everything the ruling-20 banner below says applies again word for word, and it is not restated at length:
> a struck row is **not** a completed row, and every percentage in this document improves for free.
>
> **Ruling 19's own row 8.11 predicted this case and named whose call it was**, which is why it is recorded as
> an answered question rather than a reversal. It read, in the row's last sentence: *"Ruling 19 counted it in
> scope, so it is counted; **if the user later rules it a subscription feature it leaves the denominator, and
> that is a ruling, not an agent's call.**"* The user has now so ruled. **The reason given: both capabilities
> are gated behind a vendor subscription and a vendor-operated service, so there is nothing we could build
> that a user of this product could ever reach** — which puts them squarely under §1.1 rule 4's exclusion of
> *edition/subscription features*, the rule 8.11 was already flagged against.
>
> **The arithmetic was re-derived, not typed.** §1.2a's own `awk` returns `TOTAL rows=219 C=52 P=162 A=5
> U=0 sum=219`; Area 8's heading moved `13 rows · 1 / 10 / 2` → `11 rows · 1 / 10 / 0`. **The rows below are
> counted nowhere.** ⚠️ **And Area 8 now reads `0 absent` — which is NOT an achievement and must never be
> quoted as one.** It is what remains after its only two absent rows were struck.

| Struck # | Capability, as §1.2a carried it | State when struck | The reason — and why this cannot be built to this project's standard |
|---|---|---|---|
| **8.11** | Connected Banking — connect a bank account, then **Get Balance** and **Get Statement** from inside the product | `ABSENT` (`ConnectedBanking`, `GetBankBalance` and `GetStatement` → **zero** in `src/`; the phrase *"connected banking"* → **zero**) | **STRUCK BY USER RULING 24, 2026-09-14, as a subscription-gated capability under §1.1 rule 4.** The vendor gates it on a **Tally.NET ID and a valid TSS subscription**, and the feature is not a computation we could clone — it is a **client for the vendor's own bank-connection service**. `Alt+Z` (Exchange) → *Get Balance*, and `Alt+Z` → *All Banking Options* → *Get Statement* with a period, per `help.tallysolutions.com/connected-banking/`, `…/get-balance/`, `…/get-statement/`, `…/bank-connections/` and `…/enable-connected-banking/`. **There is no endpoint we may call and no behaviour we could compare a clone against**, so building it would mean inventing a protocol and calling the result fidelity. **The vendor pages are kept above so that a future reader can see exactly what was excluded**, and so that this row can be revived intact if the user ever reverses the call. |
| **8.12** | Payment Request — generate and share a UPI/gateway **payment link or QR code** for a transaction, a party or a whole report | `ABSENT` (`PaymentRequest` → **zero** in `src/`; the phrases *"payment request"* and *"payment link"* → **zero**) | **STRUCK BY USER RULING 24, 2026-09-14, on the same ground.** Enabled at `F11` → *Show more features* → *Enable Payment Request to share payment link/QR code*, with a UPI ID / Merchant Name profile (`help.tallysolutions.com/payment-request/`, `…/banking-utilities/payment-request-faq/`). The link and QR are minted by **a payment gateway the vendor contracts with**, not by the product; a clone could draw a QR but could not make it collect money, and a payment instrument that looks real and does not work is worse than an absent one. 🔴 **THE NEAR-COLLISION IS CARRIED ACROSS WITH THE ROW, BECAUSE A RE-RUN WILL HIT IT AND MAY WRONGLY CONCLUDE THIS SHIPPED:** `\bUPI\b` returns **22** hits in `src/` and **not one is this feature** — they are the **GST B2C dynamic QR** on an invoice (`GstConfig.B2cDynamicQrEnabled`, the payee VPA and its `name@handle` validator) plus one `BankTransactionType` doc comment listing IMPS/UPI as transfer kinds. A supplier-side invoice QR under GST is a **different capability** from a Banking payment request. |

---

#### 1.2d-i 🔴 TWO FURTHER USER RULINGS OF 2026-09-14 THAT CHANGE NO STATE TOKEN — RECORDED HERE SO THEY ARE NOT LOST

> **▶ READ THE HEADING LITERALLY. Neither ruling below moves a grade, and neither was allowed to.** A12
> recorded them without touching any state cell, because there is no grade evidence that would justify one —
> the rulings set **scope and design**, and the build that satisfies them has not happened yet. If a later
> reader finds a state token that moved on the strength of one of these, that is the defect.

**(a) THE 4% HEALTH & EDUCATION CESS — RULED 2026-09-14: IT SHIPS AS A DATED, COMPANY-CONFIGURABLE RATE
DEFAULTING TO 4 PER CENT.** This has been the project's **highest-priority standing user decision** and it is
now taken. The build owed is a rate that (i) carries an **effective-from date** so a past payroll re-computes
on the rate that was live when it ran, (ii) is **editable per company** rather than compiled in, and
(iii) **defaults to 4%** so no existing book changes behaviour on upgrade. 🔴 **A correction that belongs with
the ruling, because the census row that raised it is now STALE and would otherwise keep being re-raised:**
§1.2's **T0-5** still describes the rate as one *"the code itself says it could not verify"*. **A12 re-read
`src/Apex.Ledger/Services/SalaryIncomeTax.cs:48-56` on `origin/main` at `9a83de6` and that is no longer
true** — the rate is cited to the **Income-tax Department's own page**, `https://www.incometaxindia.gov.in/w/tax-rates`,
quoted verbatim for AY 2026-27 and AY 2025-26: *"Health and Education Cess is levied at the rate of 4% on the
amount of income-tax plus surcharge."* That is a **tier-2 official statutory source under ruling 14**, so the
**sourcing half of T0-5 is closed**; what the user has now ruled on is the **configurability** half, which was
always a separate question and is still open. **T0-5's own cell is deliberately NOT edited here** — it is a
defect register row, not a census grade, and rewriting it belongs with the build.

**(c) ROW 16.7 GROUP COMPANY CONSOLIDATION IS IN SCOPE — RULED 2026-09-14.** It stays in §1.2a and in the
denominator; it is **not** struck, and it keeps its `ABSENT` state, which its evidence still supports
(`parent compan`, `member compan` and `multi-?compan` → **zero**; the two `GroupCompany` hits are a test
helper building **one** ordinary company whose account *groups* net to zero — a false positive the cell
already names). 🔴 **The ruling's second half is the part that must not be lost: the multi-company primitive
is scheduled as its OWN future wave, ahead of 16.7 and separate from it.** Group Company consolidation is a
*report over several companies*; we do not yet have the ability to hold **more than one company open at
once**, and consolidating books we cannot open together is not a smaller version of the feature — it is a
different one. **Sequencing 16.7 behind that primitive is therefore the ruling, not an agent's deferral**,
and 16.7 must not be picked up as a standalone build item before the primitive lands.

---

> **▶ READ THIS BEFORE READING THE ROWS. STRIKING A ROW LOWERS THE DENOMINATOR WITHOUT SHIPPING ANYTHING.**
> All three rows below were `ABSENT` in §1.2a. Removing them takes the in-scope total from **224 to 221** and
> the absent count from **50 to 47**, so every percentage in this document improves — **and not one line of
> code was written on 2026-09-07.** A struck row is **not** a completed row. If a later reader sees the
> *"missing"* figure fall and reads it as progress, this section is the correction, and it is deliberately
> placed where the arithmetic is derived rather than in a footnote.

**THE MECHANISM, DECIDED AND STATED EXPLICITLY, BECAUSE IT IS WHAT MOVES THE DENOMINATOR.** A struck row
**leaves §1.2a and lives here**, alongside §1.1 rule 4's excluded set and above §1.3. It is **not** given a
fifth state token inside §1.2a. Two reasons, in order of weight:

1. **A fifth token would break the derivation.** §1.2a's published `awk` classifies on the four **bare**
   tokens and `tests/Apex.Ledger.Tests/CensusDerivationDriftLockTests.cs` fails the suite on any state cell
   that is not one of them, on `sum != rows`, on a stale area heading and on a stale §1.2 cell. A `STRUCK`
   token in an area table would redden all of that — and it would redden it **correctly**, because a row that
   is counted in `rows` and in no state bucket is exactly the ten-row defect PR #69 was written to stop.
2. **The census already has this structure and it should be used, not extended.** §1.1 rule 4 excludes rows
   from the denominator entirely; §1.2 (superseded) holds a snapshot that is kept rather than deleted; §3 and
   §4 are retained as the record of decisions that were later reversed. **This section is the same pattern:
   the full text is kept, the row is out of the count, and the reason is on the row.**

**The arithmetic still closes, and it was re-derived rather than typed.** §1.2a's own `awk` returns
`TOTAL rows=221 C=52 P=122 A=47 U=0 sum=221`; Area 15's heading moved `9 rows · 0/0/9` → `6 rows · 0/0/6`;
`224 − 3 = 221` and `50 − 3 = 47`. **The rows below are counted nowhere.**

**The user's reasoning, quoted, because it decides future cases too:** *"A clone of a dead tax is dead code no
user can ever use."*

| Struck # | Capability, as §1.2a carried it | State when struck | The repeal — and why this cannot be built to this project's standard |
|---|---|---|---|
| **15.9** | Fringe Benefit Tax | `ABSENT` (`\bfbt\b` and `fringe` over `src/` and `tests/` → **both zero**) | **Abolished with effect from AY 2010-11 by s.115WM of the Finance (No.2) Act 2009**, which provides that Chapter XII-H shall not apply in respect of any assessment year beginning on or after 1 April 2010. It was **never in the 7.2 baseline** either, and current vendor documentation does not mention it — so there is **no reference-product behaviour to clone and no live law to clone it against**. This closes the FBT half of owed ruling **U-5**, whose own recommendation was *"strike it — a build agent handed this row will invent a feature"*. |
| **15.7** | Service Tax + Form ST3 | `ABSENT` (`servicetax` → **zero**; `\bst-?3\b` → **zero**; the three `service tax` hits are *"a service **tax invoice**"* under `IsServiceAccountingInvoice`, a GST word-boundary accident) | **Chapter V of the Finance Act 1994 was displaced ENTIRELY on 1 July 2017** by the GST regime, with **no surviving class of supply** left to it — unlike VAT/CST and excise, which survive on named goods. Form ST-3 is the return under a chapter that no longer levies anything. **There is no live official source stating a current service-tax rate for any supply**, because there is none. |
| **15.3** | The 2005 four-slab VAT rate structure (1% / 4% / 12.5% / exempt, ~550 commodity categories) | `ABSENT` (`four ?slab`/`fourslab` → **zero**; `commodit` → **1**, an e-way-bill doc comment in a test; every `12.5` in the tree is a `FontSize`, a GST half-rate money amount or a pay-head percentage) | **Repealed.** The 2005 empowered-committee four-slab structure was a **per-State** schedule regime, and it was superseded State by State on 1 July 2017 except for the goods Art. 366(12A) and s.9(2) CGST keep outside GST. 🔴 **AND THE REASON IT CANNOT BE BUILT TO THIS PROJECT'S STANDARD IS STATED PLAINLY, BECAUSE IT IS THE REAL ONE: the ~550 per-State commodity schedules HAVE NO LIVE OFFICIAL SOURCE.** There is no current, citable, content-resolving publication of them under R7's grounding order; a build agent handed this row would have to take the rates from a blog or a chart — precisely the citation this project has already had to strip out of shipped code (T0-6). **Under "clone, never invent", the honest answer is that the source is silent, so we ship nothing.** |

**▶ 🔴 WHAT RULING 20 DID *NOT* STRIKE, SO THE REMAINING SIX ARE NOT MISREAD AS DEAD TOO.** The user ruled
*"build only what is still levied"*, and named it: **VAT and CST on alcoholic liquor for human consumption**
(Art. 366(12A) of the Constitution excludes it from the definition of GST permanently) and on the **five
petroleum products** (s.9(2) of the CGST Act 2017 defers GST on petroleum crude, high-speed diesel, motor
spirit, natural gas and aviation turbine fuel), plus **central excise on tobacco and those petroleum
products** under the **Fourth Schedule** to the Central Excise Act 1944. Rows **15.1, 15.2, 15.4, 15.5, 15.6
and 15.8 stay in §1.2a, still `ABSENT`, still build rows under ruling 10** — but their target is now that
narrower live surface rather than the whole pre-GST stack. **No state was re-graded by this edit.**
...
> **As of 2026-09-14 (user ruling 24 struck two rows, moving §1.2's denominator; NOT ONE ITEM GRADE CHANGED HERE), against §1.2's 219 denominator:
> 21 shipped-and-compared · 22 grounded · 198 uncompared as shipped · 197 with no sourced verification of any
> kind.**
> **▶ 🔴 MOVED 2026-09-14 BY A DENOMINATOR CHANGE, NOT BY A COMPARISON — THE SAME WAY, AND FOR THE SAME REASON,
> AS THE 2026-09-07 MOVE RECORDED BELOW.** The block read ~~*"As of 2026-09-07 … against §1.2's 221 denominator: 21 shipped-and-compared · 22 grounded · 200 uncompared as shipped · 199 with no sourced verification of any kind"*~~ *(kept on one line deliberately: the derivation test strips `~~…~~` LINE BY LINE, so a struck quotation wrapped across two lines is read as a LIVE statement of the figures)*. **Figures (1) and (2) are UNTOUCHED at 21 and 22 — wave T compared nothing new.** Figures (3) and (4) are `denominator − (1)` and `denominator − (2)`, so they follow the denominator mechanically: `219 − 21 = 198` and `219 − 22 = 197`. 🔴 **AND THE HONEST READING, BECAUSE BOTH FIGURES WENT DOWN AND *"FEWER UNCOMPARED"* SOUNDS LIKE VERIFICATION AND IS NOT:** ruling 24 struck **two** rows that were uncompared, so the uncompared count fell by exactly two. **Nothing was verified. The thing to be verified got smaller.** ⚠️ **The mirror of that is the part to keep hold of: `21 of 219` is a LARGER fraction than `21 of 221`, and not one capability moved to earn it.** 🔴 **AND WAVE T DID VERIFY THINGS THAT THESE FOUR FIGURES DO NOT SHOW:** rows 2.6 and 16.1 were both built against re-read `help.tallysolutions.com` pages, and 16.1's route, field names and no-recovery warning were confirmed **verbatim**. They are not added to figure (1) here because figure (1) counts capabilities whose **shipped behaviour** has been compared end to end, and both rows shipped `PARTIAL` with named gaps. **Whoever next audits this block should decide whether that bar is still the right one rather than inheriting it silently.**
> **▶ 🔴 MOVED 2026-09-07 BY A DENOMINATOR CHANGE, NOT BY A COMPARISON — AND THIS IS THE ONE WAY THESE FIGURES
> CAN MOVE WITHOUT ANYONE VERIFYING ANYTHING, SO IT IS SAID FIRST.** The block read ~~*"against §1.2's 216 denominator: 21 shipped-and-compared · 22 grounded · 195 uncompared as shipped · 194 with no sourced verification of any kind"*~~ *(kept on one line deliberately: the derivation test strips `~~…~~` LINE BY LINE, so a struck quotation wrapped across two lines is read as a LIVE statement of the figures)*. **Figures (1) and (2) are UNTOUCHED at 21 and 22 — no capability was compared to any source on 2026-09-07.** Figures (3) and (4) are `denominator − (1)` and `denominator − (2)`, so they follow the denominator mechanically: `221 − 21 = 200` and `221 − 22 = 199`. 🔴 **AND THE HONEST READING, BECAUSE BOTH FIGURES WENT UP AND *"MORE UNCOMPARED"* SOUNDS LIKE A REGRESSION AND IS NOT ONE:** ruling 19 added **eight** vendor-attested capabilities that had no census row at all, and ruling 20 struck **three** dead-law rows to §1.2d — `216 + 8 − 3 = 221`. **The uncompared count rose by five because we admitted five net capabilities we had never been counting, not because anything got worse.** ⚠️ **The mirror of that is the part to keep hold of: `21 of 221` is a SMALLER fraction than `21 of 216`. Verification did not move; the thing to be verified got bigger.** `tests/Apex.Ledger.Tests/CensusFidelityDerivationTests.cs` caught this block the moment §1.2 moved and before any of it was written down — **the third time that lock has bitten**, and the first time it bit a denominator move rather than a hand-typed digit.
...
| **T1-8** | ~~**No Tracking Numbers.** Receipt Note↔Purchase and Delivery Note↔Sales cannot be linked.~~ 🟢 **PARTLY CLOSED 2026-09-08 (PR #81, wave K1, schema v58) — see census row 9.8, which is now `PARTIAL`.** A Tracking No. is keyed by the operator on both the goods side and the bill side and persists on both stock-line tables; Purchase / Sales Bills Pending net `Received − Billed`. **What is still open:** the vendor's invoice-number default, the *List of Tracking Numbers* picker, order-number linkage, and drill-down from a pending row. | 🔴 **THE ORIGINAL EVIDENCE — *"zero `TrackingNumber` hits in `src/`"* — IS NOW FALSE AND IS STRUCK RATHER THAN DELETED**, so a reader can see what the claim was and when it stopped being true. Re-measure before citing it. | Order fulfilment can now be tracked on the entered datum rather than inferred by a FIFO walk. **The WF-8 prerequisite is met in substance; the row stays open for the four gaps named on the left.** |
...
| **T1-15** | ~~**No F11 Accounting Features group at all.**~~ 🟢 **PARTLY CLOSED 2026-09-08 (PR #82, wave K3, no schema) — see census row 1.7, now `PARTIAL`.** The group exists with three of the vendor's four options, each gating real behaviour in six places (both ledger-master blocks, the Cost Masters section and its header, two Statements-of-Accounts rows, two button-bar quick-buttons, and both voucher-line allocation panels), every gate individually mutation-verified. **Still open:** *"Maintain Accounts"* (deliberately not shipped — this product has no accounts-less mode, so the switch could only read Yes and gate nothing), `Integrate Accounts with Inventory`, and the Accounts-only vs Accounts-with-Inventory maintain mode. ⚠️ **`IntegrateAccountsWithInventory` is behaviour-bearing and D-6 requires an oracle harness first** — three negative-stock attempts produced three *different* unbounded Balance-Sheet errors that each passed the full suite. The remaining switches this row listed (multi-currency, budgets, credit limits, cheque printing, multi-address) are **not options of the vendor's Accounting group** and were mis-attributed to it here. | 🔴 **PARTLY STALE:** *"`GstConfigViewModel.cs` — 21 booleans"* has moved (the three Accounting flags were added to that page). **Still true and re-measured 2026-09-08:** zero hits for `IntegrateAccountsWithInventory` and `MaintainMode`. | Every one of those capabilities is always-on with no way to turn it off. `Integrate Accounts with Inventory` in particular changes how the Balance Sheet sources closing stock — it belongs in wrong-figures territory, not a checkbox. |
| **T1-16** | **No global F12 configuration tree.** Four context panels only; everywhere else F12 sets a stub string. | `MainWindowViewModel.cs:6628-6653` — the fall-through is literally `Message = "F12 Configure — display options (Phase 1 defaults)."` | Blocks the entire display/entry configuration layer. Entangled with gap-decision D10. 🔴 **BUT THE TARGET IS WRONG, AND WAVE K3 (PR #82) MEASURED IT AGAINST THE VENDOR RATHER THAN ARGUING IT: TallyPrime REMOVED the global Configuration menu and the F12 button from menu context; the global layer now lives at F1 (Help) → Settings, and F12 is per-report.** Building the tree this row names would have been inventing a Tally.ERP 9 artefact into a TallyPrime clone. **A row that names the wrong target cannot be closed by building the right thing** — ~~re-titling it to *"Global settings — F1 (Help) → Settings"* and reshaping this defect is a `plan.md` change and therefore an **R6/R12 user decision, owed to the user and not taken here.**~~ 🔴 **RESHAPED 2026-09-09 BY USER RULING 21 (plan.md §5, `THREE FURTHER USER RULINGS (R12, 2026-09-09)`): census row 1.8 is retitled and T1-16's target moves with it. THIS DEFECT IS NO LONGER "no global F12 configuration tree" — the reference product has no such tree and never will. It is now: THE APP-WIDE CONFIGURATION LAYER IS UNREACHABLE BECAUSE F1: Help → Settings IS NOT BUILT, AND THE `F12` BUTTON IS OFFERED WHERE IT DOES NOTHING** — the ruling gates that button as part of the same work. See census row 1.8 for the three vendor quotations. |
...
> **▶ 🔴 UPDATED 2026-09-07 — RULINGS 15–20 HAVE CLOSED PART OF THIS BATCH. THE LIVE, ANSWERABLE-IN-ONE-SITTING
> VERSION IS NOW `plan.md` §5 (`TWO FURTHER USER RULINGS (R12, 2026-09-07)` → the decision sheet immediately
> below it). THIS TABLE IS THE LONG-FORM RECORD; the sheet is what goes to the user.** Closed since this table
> was written: **U-0** and **U-3** (ruling 14) · the whole **ten-row gate** (ruling 15) · the **`Ctrl+I` arm of
> U-6** (ruling 17) · the **FBT arm of U-5** (ruling 20 — struck; see §1.2d) · **U-11's denominator question**
> (rulings 19 and 20 moved it `216 → 221`, though U-11's own two rows, Migrate-as-an-operator-action and
> Extract/Share, are **still unanswered** and were **not** among ruling 19's eight). **Rows are struck in place
> rather than deleted so the batch can be seen to shrink.**
...
| **U-5** | 🔴 **PARTLY CLOSED 2026-09-07 BY RULING 20 — THE FBT ARM IS DECIDED: STRUCK.** Row 15.9 left §1.2a for **§1.2d**, cited to **s.115WM of the Finance (No.2) Act 2009** (Chapter XII-H does not apply from AY 2010-11). The same ruling struck **15.7** (Service Tax + ST-3, Chapter V of the Finance Act 1994 displaced entirely on 1-Jul-2017) and **15.3** (the 2005 four-slab VAT structure, ~550 per-State schedules with **no live official source**). 🔴 **STILL OPEN, AND IT IS THE ARM NOBODY HAS ANSWERED: ROW 15.5's *state return forms*.** *"VAT Computation"* is buildable against the live liquor/petroleum surface ruling 20 preserved, but *"state return forms"* is **~30 State-specific forms with no single official publisher** — the same sourcing problem that killed 15.3, one row further on. **Nothing may be sliced into 15.5's return-form half until this lands.** The original question is kept verbatim below. — **Fringe Benefit Tax (row 15.9) — build it, or strike it?** | Ruling 10 said build Area 15. But FBT was abolished by the Finance Act 2009, **was never in 7.2**, and is the one row in the area with **no reference-product behaviour to clone**. Current vendor documentation does not mention it. | **Strike it.** **A build agent handed this row will invent a feature** — which is the failure mode this project has the most scar tissue about. |
...
| **U-9** 🔴 **CLOSED 2026-09-09 BY USER RULING 23** *(plan.md §5, `THREE FURTHER USER RULINGS (R12, 2026-09-09)`)* — **the storage-identity change is ACCEPTED: SQLCipher, opaque company ids, and the company name held INSIDE the encrypted book. The user accepted the named costs — a new native dependency with its own licence, three-OS CI risk, and `CompanyStorage` losing its filename-enumeration listing. ROW 16.1 IS UNBLOCKED.** ⚠️ **Row 16.7 (Group Company / the multi-company primitive) was explicitly NOT ruled and stays open under D-6; the 8.11 / 8.12 denominator question was explicitly NOT ruled and stays open. The in-scope denominator does not move: 221 of an honest 224.** ⚠️ **Brand rule: what WE ship must not carry the vendor's name; "TallyVault" here is the reference-product identifier only.** | **TallyVault (row 16.1).** | Company-data encryption behind a passphrase means SQLCipher or an equivalent crypto dependency. 🔴 **And there is an architecture collision nobody had recorded: the source says a TallyVault password encrypts the company *"including the company name"*, while our companies live in a `.db` NAMED AFTER THE COMPANY** — the same constraint item 9 records as the reason rename is out of scope. **It cannot be bolted on: the filename leaks the plaintext the feature exists to hide.** | Decide the dependency **and** the storage-layout change together, or defer both. R13 (secrets) applies. |
...
**Bottom line for the user** *(every figure below is as of **2026-09-05** and copied from its derivation — the capability split from §1.2, which is itself summed from §1.2a; the fidelity numbers from §1.3; the TIER 0 count from §2 TIER 0. §1.3's anchor block pins the last two. If a figure here disagrees with its derivation, the derivation is right and this paragraph is stale)*. A perfect clone needs ~~**216**~~ **221 named capabilities — and this is the first version of this document in which you can read which ones**. We have **52** whole, ~~**122**~~ ~~**123**~~ ~~**125**~~ **132** partial, ~~**42**~~ ~~**47**~~ ~~**46**~~ ~~**44**~~ **37** missing. 🟡 **RE-DERIVED 2026-09-07 (A15, at `origin/main` `cb8cccb`, schema v56) — `TOTAL rows=221 C=52 P=132 A=37 U=0 sum=221`, transcribed from §1.2a's own `awk`, never typed. SEVEN ROWS MOVED `ABSENT` → `PARTIAL` AND NOT ONE LINE OF CODE WAS WRITTEN TO DO IT: 2.4, 2.12, 2.13, 5.10, 5.11, 14.1, 14.5 had all SHIPPED — in PRs #52, #53 and #54 — and the census simply never followed them. `complete` did not move, so the one figure that measures finished work is unchanged at 23.5% of 221; what moved is how much of our own delivered work this document could see. Area 14 reaches `0 absent` for the first time.** 🔴 **AND THE CHECK THAT MAKES THAT CREDIBLE: an eighth candidate, row 5.5 (Insert Voucher, Alt+I), was re-measured and LEFT `ABSENT` — `Alt+I` is still the POS tender-mode toggle and there is no insert-at-position code anywhere. Seven of eight, not eight of eight.** 🟢 **THE LAST MOVE BEFORE THAT WAS THE ONLY KIND THAT IS PROGRESS, AND IT IS ONE ROW: 14.4 More Details went `ABSENT` → `PARTIAL` on 2026-09-07 when PR #72 landed, because a user can now press Ctrl+I and reach the panel. The denominator did not move with it — 221 is still 221 — which is exactly the difference between building something and re-scoping.** 🔴 **THE DENOMINATOR MOVED 2026-09-07 BY USER RULINGS 19 AND 20: `216 + 8 − 3 = 221`, re-derived by re-running §1.2a's own `awk` (`TOTAL rows=221 C=52 P=122 A=47 U=0 sum=221`), never edited. `+8` is ruling 19 — eight vendor-attested capabilities that had NO CENSUS ROW AT ALL, all eight ABSENT, so the old 216 flattered us in BOTH numerator and denominator; the honest figure is 224 and the goal is now "all 224". `−3` is ruling 20 — three dead-law rows struck to §1.2d, and 🔴 STRIKING LOWERS THE DENOMINATOR WITHOUT SHIPPING ANYTHING: nothing was built on 2026-09-07, `complete` did not move, and as a fraction it FELL from 24.1% of 216 to 23.5% of 221. A struck row is not a completed row.** *(RE-DERIVED 2026-09-07 BY A15 at `origin/main` `90413a9` — it read ~~"**52** whole, **105** partial, **59** missing"~~. Transcribed from §1.2a's own awk, whose literal output is now `TOTAL rows=216 C=52 P=122 A=42 U=0 sum=216`; before the fix it was `… C=52 P=110 A=44 U=0 sum=206`, ten landed rows falling into no state bucket because their state cell had been bolded. **Fifteen of the seventeen moved partials are bookkeeping, not progress** — only 6.10 and 6.13 are a real state change. **And this denominator is itself eight capabilities too small — see §1.2 point 6.**)*. *(🔴 Re-derived 2026-09-05 **a second time, in the b1/b3/b4/b5 landing**, by re-running §1.2a's own counting command; its LITERAL output is `TOTAL rows=216 C=52 P=105 A=59 U=0 sum=216`. Earlier the same day it read ~~*"**46** whole, **102** partial, **68** missing"*~~ and before that ~~*"**47** whole, **96** partial, **73** missing"*~~ — every one of those was stale in turn, and **no digit here was typed to a target.** The move is `+6 complete, +3 partial, −9 absent`; the nine rows are named in §1.2's banner. **It would have been eleven** — rows 6.10 and 6.13 are written, reviewed and gate-green — **but the branch carrying them was held on a red CI leg and is not on `main`, so on `main` those capabilities are still unreachable and the rows did not move.** That is the standard this pass held: a row moves when a USER can reach the capability, not when the code exists.)* *(The **115 · 42 / 44 / 21** you may have seen quoted elsewhere is the superseded 2026-08-10 snapshot, and **200 · 47 / 95 / 58** is the 2026-08-18 one; §1.2b explains the first move — mostly granularity, and an absent column that was provably too small — and §1.2's banner explains the second, which is a **scope decision**: user ruling 10 brought §3's nine and §4's seven into the denominator, `200 + 9 + 7 = 216`.)* Only **21** capabilities have ever been checked against a source for correctness as shipped, so the fidelity denominator is ~~**195**~~ **200** wide open; one of those — voucher alteration — has its grounding banked ahead of the slice that builds it, which leaves ~~**194**~~ **199** with no sourced verification of any kind. 🔴 **THOSE TWO MOVED 2026-09-07 WITHOUT ANYONE VERIFYING ANYTHING: rulings 19 and 20 took §1.2's denominator `216 → 221`, and figures (3) and (4) are `denominator − 21` and `denominator − 22`. Compared is STILL 21. `21 of 221` is a SMALLER fraction than `21 of 216` — verification did not move; the thing to be verified got bigger.** See §1.3's anchor block, which is the single derivation and is re-derived by `CensusFidelityDerivationTests` on every run. *(🔴 Re-derived 2026-09-05 in the same merge by re-running §1.3's grade command; its LITERAL output is `21 [GRADE: COMPARED]` / `1 [GRADE: GROUNDED-AHEAD]` / `1 [GRADE: METHOD-NOTE]`, which is the anchor block `21 · 22 · 195 · 194`. It read ~~*"**11** … **205** … **204**"*~~ here.)* **▶ And as of 2026-08-19 that gap is the goal itself: user ruling 9 makes "done" mean FULL PARITY *and* CORPUS VERIFICATION — with the honest limit that where the corpus is silent a capability ships as a documented divergence labelled as ours, and can never join the 21.** 🔴 **AMENDED BY RULING 14 (2026-09-04): read *"CORPUS VERIFICATION"* above as **VENDOR-DOCUMENTATION VERIFICATION** and *"where the corpus is silent"* as *"where no source in R7's order speaks"*. Only the source's name changed; the bar did not.** The most urgent items are not the missing ones: they are the **open TIER 0 defects** 🔴 **(COUNT DELIBERATELY NOT RESTATED, 2026-09-05. It read ~~*"eleven"*~~ and was stale, but unlike the capability split and the §1.3 grades the TIER 0 register carries **no single machine-readable state token** - a row's closure is prose (`✅ CLOSED`, `**CLOSED 2026-08-17**`, `~~struck~~`), so there is no command to re-run and any digit here would be typed to a target. The register held **27 rows** at this merge, counted by `awk` over the §2 TIER 0 table. **Re-walk it and classify the rows yourself; adding a bare state token to that table is filed as an open item.**)** — of which **nine are confirmed wrong-money-or-invalid-document defects a business would suffer today**, and **two (T0-5's 4% cess, T0-6's blog-cited TDS rates) are statutory figures the product applies to money on sourcing nobody can stand behind — confirmed unsourced, not confirmed wrong**. Two of the nine are new today and both are payroll: **recording an attendance period twice silently doubles the pay**, and **a leaver accrues gratuity and bonus for ever**. All of it sits on top of a book that still cannot be fully corrected: ~~**no voucher can be ALTERED at all**~~ 🔴 **— FALSE, AND STALE SINCE PHASE 10.11 S5d/S5e (`a34d989` / `b89213e`). CORRECTED IN PLACE 2026-09-07 (A15), NOT DELETED, because it is the same defect the banner at the head of this file records: row 5.1 moved to `PARTIAL` on 2026-08-20 and this closing paragraph was never re-read. Measured at `cb8cccb`: `MainWindow.axaml.cs`'s `OnKeyDown` carries a `Ctrl+Enter OPENS THE HIGHLIGHTED POSTED VOUCHER FOR ALTERATION` arm on three surfaces, `MainWindowViewModel.IsVoucherAlterTargetPage` gates it, and `VoucherAlterationEligibility` decides per voucher. What is TRUE is narrower and is row 5.1's own text: the SALES ITEM INVOICE family is still refused, and `LedgerService` refuses a renumber inside Alter.** And **eight of the classic eighteen voucher kinds can be neither cancelled nor deleted nor even listed in the Day Book** (T1-17) — cancellation and deletion shipped for the other ten.
