...
| **T0-19** | 🔴 **NEW 2026-09-03. Both POS rate resolutions use the DATE-BLIND two-argument overload, so the dated `GstRateHistory` override never fires at the counter — while every accounting screen passes the date.** `src/Apex.Desktop/ViewModels/PosBillingViewModel.cs:452` and `src/Apex.Desktop/ViewModels/PosBillingViewModel.cs:494` call `_gst.ResolveRate(item, SelectedSalesLedger)`; all four `VoucherEntryViewModel` sites — `src/Apex.Desktop/ViewModels/VoucherEntryViewModel.cs:5182`, `src/Apex.Desktop/ViewModels/VoucherEntryViewModel.cs:5232`, `src/Apex.Desktop/ViewModels/VoucherEntryViewModel.cs:5417` and `src/Apex.Desktop/ViewModels/VoucherEntryViewModel.cs:5873` — call the three-argument form with `Date`. ⚠️ **All six locators were RE-DERIVED on 2026-09-04** (they read `:428/:465` and `:5073/:5119/:5304/:5760` before): the T0-16 slice edited both files, and a citation this row itself warns about going stale is exactly the kind that does. ⚠️ **Every locator here is written in full deliberately: the bare `` `:NN` `` shorthand this document used elsewhere is checked by NEITHER citation guard** (both key on `File.ext:NN`), which is how a section of stale pointers survived a repair pass on 2026-08-21. | **[V] 2026-09-03**, all six call sites enumerated at HEAD by one grep over `src/Apex.Desktop`. The two-argument overload forwards `voucherDate: null`, and the dated override is gated on `voucherDate is { } d`, so it is skipped entirely. | The same item sold over the counter and on a Sales item invoice on the same day carries **different tax** whenever a dated rate-history row is in force — and the counter takes the pre-revision rate for ever. ~~*"Sits beside **T0-16** (the POS also collects zero cess), so the POS is now known to diverge from the accounting screen on two tax dimensions."*~~ **SUPERSEDED 2026-09-04: T0-16 is CLOSED, so this is now the LAST known tax divergence between the counter and the accounting screen** — and closing T0-16 deliberately did NOT close it, because `ResolveCess` has no date-blind overload while `ResolveRate` does, so the cess is already resolved as of `Date` while both rate resolutions still are not. Changing that moves the tax on existing counter sales and belongs to its own slice. **OPEN.** |
