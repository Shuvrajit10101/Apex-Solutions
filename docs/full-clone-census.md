...
| | **TOTAL** | **221** | **52** | **139** | **30** | **0** |

**A full clone requires 221 named capabilities. We have 52 complete, 139 partial, 30 absent, 0 undetermined.**
...
#### Area 8 — Banking · 13 rows · 1 complete / 8 partial / 4 absent
...
| 8.13 | Auto-create vouchers from an imported bank statement (optional vouchers, reviewed then regularised) | ABSENT | 🔴 **ROW ADDED 2026-09-07 BY USER RULING 19. IT HAD NO ROW AT ALL** — zero hits by name in this census and zero in `src/`. **Vendor page:** `help.tallysolutions.com/auto-create-vouchers/` — import an Excel/CSV statement, **create multiple vouchers**, which are *marked optional so you can review before finalizing*, checking bank allocations and cost centres, then reconcile once regularised. 🔴 **THIS IS GENUINELY NOT INSIDE ROW 8.3, AND THE MEASUREMENT SAYS SO RATHER THAN THE ARGUMENT.** 8.3 is reconciliation: `src/Apex.Ledger/Banking/BankStatementImport.cs` exposes exactly `ParseCsv` and `MatchAndReconcile`, and `CreateVoucher`, `AddVoucher`, `PostVoucher` and `new Voucher` return **zero** in that file — it matches statement lines to vouchers that **already exist** and stamps a Bank Date. 8.3's own **Gap** list (CSV only, no manual match/unmatch, no print or export) never mentions creating a voucher either. ⚠️ **A reader who judges auto-create to be inside 8.3's scope would put the honest denominator at 223 rather than 224; ruling 19 ruled it a row of its own.** |
