...
- **Slices (schema-clean first, then the version chain in strict order; rationale in `memory.md`):**
  ...
  5. **S4 — GST five-level hierarchy** (WF-1) — **XL / HIGH / owns v51** — the worst row in the register.
     ...
     **The fix is the writer's three-way fallback — `src/Apex.Persistence.Sqlite/SqliteCompanyStore.cs:5722`** (re-pointed 2026-09-14 from ~~`:5689`~~, schema **v63** (W-R1 census 7.19 Labour Welfare Fund) added the v61→v63 ladder step and the slab effective-window reader/writer above it; itself re-pointed 2026-09-09 from ~~`:5576`~~, schema **v61** (W-P1) added the v60→v61 ladder step and the GST registration / classification reader-writer above it; itself re-pointed 2026-09-08 from ~~`:5529`~~, schema **v60** (W-O1) added the five `groups` behavioural columns to the group reader/writer above it; itself re-pointed the same day from ~~`:5440`~~, schema **v59** (W-N1) added the seven company VAT columns to this INSERT's parameter block above it)
     ...
     **The index replay is `src/Apex.Persistence.Sqlite/SchemaDowngrade.cs:922`** (re-pointed 2026-09-14 from ~~`:886`~~, schema **v63**'s `V63ToV61` (W-R1 census 7.19 Labour Welfare Fund) was added above it — this is the `DropColumns` copy of the replay, **not** the `RebuildPreservingShape` copy that now sits at `:852`; itself re-pointed 2026-09-09 from ~~`:835`~~, schema **v61**'s `V61ToV60` was added above it, when the `RebuildPreservingShape` copy sat at ~~`:816`~~; itself re-pointed 2026-09-08 from ~~`:791`~~, schema **v60**'s `V60ToV59` was added above it; itself re-pointed the same day from ~~`:747`~~, schema **v59**'s `V59ToV58` added lines above it; itself re-pointed the same day from
