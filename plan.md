...
> **`src/Apex.Persistence.Sqlite/Schema.cs:287-288`** — **re-pointed 2026-09-24 from ~~`:264-265`~~ (A12, wave 32: TWO stale `"v59"` restatements were removed — one in the `Schema` type doc and one in `CurrentVersion`'s OWN doc comment, which claimed `"v59 is the latest bump"` while the constant read 64 — each replaced by a note that DELETES the restated digit rather than updating it, adding 23 lines above this citation in total; the cited CODE is unchanged, only its line number moved — and note that `CurrentVersion` itself now sits at `:289`, not `:266`), itself re-pointed 2026-09-08 from ~~`:245-246`~~ (schema **v59**, W-N1 State VAT & CST, added lines above it), itself re-pointed the same day from ~~`:232-233`~~ (schema **v58**, W-K1 inventory costing & tracking, added lines above it), itself re-pointed 2026-09-07 from ~~`:207-208`~~ (schema v56, Security Control, added lines above it), itself re-pointed the same day from ~~`:187-188`~~ (schema v55, the Karnataka PT back-fill, added lines above it), itself re-pointed 2026-09-05 from ~~`:166-167`~~ (W2-03's v53 doc comment added lines above it), itself re-pointed 2026-08-19 from ~~`:157-158`~~** (the voucher edit log added lines above it, so it drifted exactly as `:144-145` had, and stayed green for exactly the same reason) — a **content drift, not a dangling citation**: `:144-145`
...
> moved three times already (~~`:146`~~ → ~~`:159`~~ → ~~past `:167`~~ → past `:178`), so grep `public const int CurrentVersion` in `src/Apex.Persistence.Sqlite/Schema.cs` and read the number off the match. **WF-1 took v51 in `e49b88e`**, exactly as this ruling
> authorises and as **W0-2b's own `▶ SCHEMA` note already records**. **v52 WAS TAKEN 2026-08-19 by the voucher edit log; ~~v53 was RESERVED by Phase 10.10's binding allocation (WF-3)~~ — **v53 WAS TAKEN 2026-09-05 by W2-03 (the Voucher Type master)**, pre-allocated to that track alone by the wave-2 dispatch and used for exactly two additive `voucher_types` columns (`print_after_saving`, `provide_narration_for_each_ledger`) with a forward migration `MigrateV52ToV53`, a `SchemaDowngrade.V53ToV52`, and round-trip + migration-parity tests. **The next free version is v54, and WF-2 is unallocated.** Read W0-2b's
...
> verbatim so nobody over-reads it:** *"the salary-tax ARITHMETIC exists (`SalaryIncomeTax.cs:264`
> `SalaryTaxComputation`, which backs Form 16 Part B). What is missing is the vendor per-employee REPORT."*
> *(⚠️ **Address re-pointed 2026-09-15 from ~~`:310`~~ — the ONLY edit ever made inside this verbatim quote, and it
> is made rather than left because leaving it would make the user's own sentence FALSE: defect T1-26 rewrote
> `SalaryIncomeTax.cs` to take a dated `SalaryTaxRates` table and the file shrank to 308 lines, so `:310` now
> points past EOF. The user's WORDS and their claim are untouched; only the address under `SalaryTaxComputation`
> moved. A12: revert this if the verbatim rule is meant to bind even against a citation that no longer resolves.)*
...
- **Slices (schema-clean first, then the version chain in strict order; rationale in `memory.md`):**
  ...
  5. **S4 — GST five-level hierarchy** (WF-1) — **XL / HIGH / owns v51** — the worst row in the register.
     ...
     **The fix is the writer's three-way fallback — `src/Apex.Persistence.Sqlite/SqliteCompanyStore.cs:5921`** (re-pointed 2026-09-15 from ~~`:5864`~~, schema **v64** (defect T1-26 / the 4% cess ruling) added the v63→v64 ladder step and the dated cess-rate reader-writer above it; the cited text, the `storedSourceOrders?.Hsn` limb of the fallback, is unchanged; itself re-pointed 2026-09-14 by A12 from ~~`:5851`~~ at the W-T1 vault merge — the vault's registry and unpooled-connection work added thirteen lines above it and the cited text, the `storedSourceOrders?.Hsn` limb of the fallback, is unchanged; itself re-pointed 2026-09-14 from ~~`:5856`~~, the v63 ladder step above it was re-pointed from 61→63 to 62→63 once v62 landed, and the twenty-line merge-action note it carried was replaced by a five-line record of the completed re-point; itself re-pointed 2026-09-14 from ~~`:5689`~~, schema **v62** (W-Q1 census 2.6 Voucher Class) added the v61→v62 ladder step and the voucher-class allocation / additional-entry reader-writer, and schema **v63** (W-R1 census 7.19 Labour Welfare Fund) added its own ladder step and the slab effective-window reader/writer, above it — both landed in this tree at the same merge, so neither branch's own figure survived; itself re-pointed 2026-09-09 from ~~`:5576`~~, schema **v61** (W-P1) added the v60→v61 ladder step and the GST registration / classification reader-writer above it; itself re-pointed 2026-09-08 from ~~`:5529`~~, schema **v60** (W-O1) added the five `groups` behavioural columns to the group reader/writer above it; itself re-pointed the same day from ~~`:5440`~~, schema **v59** (W-N1) added the seven company VAT columns to this INSERT's parameter block above it)
     ...
     **The index replay is `src/Apex.Persistence.Sqlite/SchemaDowngrade.cs:1010`** (re-pointed 2026-09-15 from ~~`:968`~~, schema **v64**'s `V64ToV63` (defect T1-26 / the 4% cess ruling) was added above it as the new TOP rung and `V63ToV62`'s "top rung" note was re-worded to point at it — the `RebuildPreservingShape` copy of the replay moved with it, from ~~`:898`~~ to `:940`; the cited CODE is unchanged, only its line number moved; itself re-pointed 2026-09-14 from ~~`:959`~~, the v63 downgrade rung above it was renamed `V63ToV61` → `V63ToV62` and re-documented — it now stamps 62 and reverses v63 alone, `V62ToV61` reversing v62 after it — and the repair also fixed two call sites that had been invoking `V62ToV61` alone on a v63 book; itself re-pointed 2026-09-14 from ~~`:886`~~, schema **v62**'s `V62ToV61` and schema **v63**'s `V63ToV62` (W-R1 census 7.19 Labour Welfare Fund) were BOTH added above it at the same merge, so neither branch's own figure survived — this is the `DropColumns` copy of the replay, **not** the `RebuildPreservingShape` copy that then sat at ~~`:898`~~ and now sits at `:940` (itself moved from ~~`:889`~~ by that re-point); itself re-pointed 2026-09-09 from ~~`:835`~~, schema **v61**'s `V61ToV60` was added above it, when the `RebuildPreservingShape` copy sat at ~~`:816`~~; itself re-pointed 2026-09-08 from ~~`:791`~~, schema **v60**'s `V60ToV59` was added above it; itself re-pointed the same day from ~~`:747`~~, schema **v59**'s `V59ToV58` added lines above it; itself re-pointed the same day from
     ...
     is the `DropColumns` one, the second of the two.
...
- **Work items (id — one-line; the evidence for every row is in `docs/full-clone-census.md` §2, not here):**
  ...
  - **W0-2b (S2 / T1-6, T0-8 write half) Company Create/Alter screen** — ✅ **BUILT 2026-08-16, REVIEWED AND
    ...
    *"a postal State typed into `Company.State` goes nowhere"* was **wrong**, and it is the sentence the choice
    ...
    `CanonicalXml.cs:55` (write), `CanonicalXml.cs:1026-1027` (read), `ImportPlan.cs:1213-1214` (assign) — and
...
- **Schema: NONE expected** — every item is UI over persisted state. ~~Any slice that finds it needs a column
  ...
  **forward migration**, **round-trip tests** and the **existing migration-equivalence check**
  ...
  `src/Apex.Persistence.Sqlite/Schema.cs:287-288` — **re-pointed 2026-09-24 from ~~`:264-265`~~ (A12, wave 32: two stale `"v59"` restatements were removed — the `Schema` type doc's and `CurrentVersion`'s own — each replaced by a note that deletes the restated digit, adding 23 lines above this citation; the cited code is unchanged; `CurrentVersion` is now at `:289`), itself re-pointed 2026-09-08 from ~~`:245-246`~~ (schema **v59**, W-N1 State VAT & CST), itself re-pointed the same day from ~~`:232-233`~~ (schema **v58**, W-K1), itself re-pointed 2026-09-07 from ~~`:207-208`~~ (schema v56, Security Control, added lines above it), itself re-pointed the same day from ~~`:187-188`~~ (schema v55, the Karnataka PT back-fill), itself re-pointed 2026-09-05 from ~~`:166-167`~~ (W2-03's v53 doc comment), itself re-pointed 2026-08-19 from ~~`:157-158`~~, and guarded from here on by `LoadBearingCitationContentTests`** — and **`Schema.CurrentVersion` is `53`**,
