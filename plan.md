...
- **Slices (schema-clean first, then the version chain in strict order; rationale in `memory.md`):**
  ...
  5. **S4 — GST five-level hierarchy** (WF-1) — **XL / HIGH / owns v51** — the worst row in the register.
     ...
     **The index replay is `src/Apex.Persistence.Sqlite/SchemaDowngrade.cs:923`** (re-pointed 2026-09-14 from ~~`:886`~~, schema **v62**'s `V62ToV61` was added above it as the new top rung — this is the `DropColumns` copy of the replay, **not** the `RebuildPreservingShape` copy that now sits at `:853`; itself re-pointed 2026-09-09 from ~~`:835`~~, schema **v61**'s `V61ToV60` was added above it; itself re-pointed 2026-09-08 from ~~`:791`~~, schema **v60**'s `V60ToV59` was added above it; itself re-pointed the same day from ~~`:747`~~, schema **v59**'s `V59ToV58` added lines above it; itself re-pointed the same day from
